...
class ProductivityUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('ProductivityInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('ProductivityKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('ProductivityTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('ProductivityFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['productivity_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('ProductivityCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['productivity_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('ProductivityCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['productivity_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/productivity/"; // Upload directory
                $imgCamp='';
                $errors='';
