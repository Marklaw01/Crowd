//
//  AddProductivityViewController.m
//  CrowdBootstrap
//
...
#import "AddProductivityViewController.h"
...
@interface AddProductivityViewController ()

@end

@implementation AddProductivityViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Add Productivity";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Productivity Tool Title", @"Productivity Tool Description", @"Productivity Tool Availability Start Date", @"Productivity Tool Availability End Date", @"Target Market", @"Productivity Tool Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kProductivityAPI_Title, kProductivityAPI_Description, kProductivityAPI_StartDate, kProductivityAPI_EndDate, kAddProductivityAPI_Target_Market_Keywords, kAddProductivityAPI_Keywords, kAddProductivityAPI_IndustryKeywords, kProductivityAPI_Image];
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kProductivityAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kProductivityAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kProductivityAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createProductivity {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addProductivityWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddProductivity_SuccessMessage withResultType:@"0" withDuration:1] ;
