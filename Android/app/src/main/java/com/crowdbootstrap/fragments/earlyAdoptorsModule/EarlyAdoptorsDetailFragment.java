...
public class EarlyAdoptorsDetailFragment extends Fragment implements View.OnClickListener, AsyncTaskCompleteListener<String> {
    ...
    public EarlyAdoptorsDetailFragment() {
        super();
    }
            ...
            case R.id.cbx_Follow:
                ...
                if (((HomeActivity) getActivity()).networkConnectivity.isInternetConnectionAvaliable()) {
                    if (cbx_Follow.getText().toString().trim().equalsIgnoreCase("Follow")) {
                        ((HomeActivity) getActivity()).showProgressDialog();
                        AsyncNew asyncNew = new AsyncNew(getActivity(), (AsyncTaskCompleteListener<String>) getActivity(), Constants.EARLY_ADOPTORS_FOLLOW_TAG, Constants.EARLY_ADOPTORS_FOLLOW_URL, Constants.HTTP_POST_REQUEST, followObj);
                        asyncNew.execute();
                    } else {
                        ((HomeActivity) getActivity()).showProgressDialog();
                        AsyncNew asyncNew = new AsyncNew(getActivity(), (AsyncTaskCompleteListener<String>) getActivity(), Constants.EARLY_ADOPTORS_UNFOLLOW_TAG, Constants.EARLY_ADOPTORS_UNFOLLOW_URL, Constants.HTTP_POST_REQUEST, followObj);
                        asyncNew.execute();
                    }
                } else {
                    ((HomeActivity) getActivity()).utilitiesClass.alertDialogSingleButton(getString(R.string.no_internet_connection));
    ...
    public void onTaskComplete(String result, String tag) {
        if (result.equalsIgnoreCase(Constants.NOINTERNET)) {
            ...
            if (tag.equals(Constants.EARLY_ADOPTORS_DETAILS_TAG)) {
                ...
                try {
                    ...
                    if (jsonObject.optString(Constants.RESPONSE_STATUS_CODE).equalsIgnoreCase(Constants.RESPONSE_SUCESS_STATUS_CODE)) {
                        ...
                        if (jsonObject.has("early_interest_keywords_id")) {
                            StringBuilder sb = new StringBuilder();
                            for (int i = 0; i < jsonObject.getJSONArray("early_interest_keywords_id").length(); i++) {
                                if (sb.length() > 0) {
                                    sb.append(", ");
                                }
                                sb.append(jsonObject.getJSONArray("early_interest_keywords_id").getJSONObject(i).getString("name"));
                            }
                            et_interestKeywords.setText(sb.toString());
                        }

                        if (jsonObject.has("early_keywords_id")) {
                            StringBuilder sb = new StringBuilder();
                            for (int i = 0; i < jsonObject.getJSONArray("early_keywords_id").length(); i++) {
                                if (sb.length() > 0) {
                                    sb.append(", ");
                                }
                                sb.append(jsonObject.getJSONArray("early_keywords_id").getJSONObject(i).getString("name"));
                            }
            ...
            } else if (tag.equals(Constants.EARLY_ADOPTORS_LIKE_TAG)) {
                ((HomeActivity) getActivity()).dismissProgressDialog();
                try {
                    JSONObject jsonObject = new JSONObject(result);
                    if (jsonObject.optString(Constants.RESPONSE_STATUS_CODE).equalsIgnoreCase(Constants.RESPONSE_SUCESS_STATUS_CODE)) {
            ...
            } else if (tag.equals(Constants.EARLY_ADOPTORS_DISLIKE_TAG)) {
                ((HomeActivity) getActivity()).dismissProgressDialog();
                try {
                    JSONObject jsonObject = new JSONObject(result);
                    if (jsonObject.optString(Constants.RESPONSE_STATUS_CODE).equalsIgnoreCase(Constants.RESPONSE_SUCESS_STATUS_CODE)) {
            ...
            } else if (tag.equals(Constants.EARLY_ADOPTORS_FOLLOW_TAG)) {
                ((HomeActivity) getActivity()).dismissProgressDialog();
                try {
                    JSONObject jsonObject = new JSONObject(result);
                    if (jsonObject.optString(Constants.RESPONSE_STATUS_CODE).equalsIgnoreCase(Constants.RESPONSE_SUCESS_STATUS_CODE)) {
            ...
            } else if (tag.equals(Constants.EARLY_ADOPTORS_UNFOLLOW_TAG)) {
                ((HomeActivity) getActivity()).dismissProgressDialog();
                try {
                    JSONObject jsonObject = new JSONObject(result);
                    if (jsonObject.optString(Constants.RESPONSE_STATUS_CODE).equalsIgnoreCase(Constants.RESPONSE_SUCESS_STATUS_CODE)) {
