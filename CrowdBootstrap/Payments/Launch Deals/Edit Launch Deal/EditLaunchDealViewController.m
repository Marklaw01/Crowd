//
//  EditLaunchDealViewController.m
//  CrowdBootstrap
//
...
#import "EditLaunchDealViewController.h"
...
@interface EditLaunchDealViewController ()

@end

@implementation EditLaunchDealViewController
...
-(void) awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(SetViewEditingNotification:) name:kNotificationSetLaunchDealViewEditing
                                               object:nil];
...
-(void)SetViewEditingNotification:(NSNotification *)notification {
    if ([[notification name] isEqualToString:kNotificationSetLaunchDealViewEditing]) {
...
-(void)resetUISettings {
    ...
    launchDealData = [[UtilityClass getLaunchDealDetails] mutableCopy];
    NSLog(@"LaunchDeal Data: %@",launchDealData) ;
    ...
    self.title = [NSString stringWithFormat:@"%@",[launchDealData valueForKey:kLaunchDealAPI_Title]] ;
    [self getLaunchDealDetails] ;
...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Created By", @"Launch Deal Title", @"Launch Deal Description", @"Launch Deal Availability Start Date", @"Launch Deal Availability End Date", @"Target Market", @"Launch Deal Keywords", @"Interest Keywords", @"Upload Image", @"View Document", @"Play Audio", @"Play Video"] ;
    NSArray *parametersArray = @[kLaunchDealAPI_CreatedBy, kLaunchDealAPI_Title, kLaunchDealAPI_Description, kLaunchDealAPI_StartDate, kLaunchDealAPI_EndDate, kAddLaunchDealAPI_Target_Market_Keywords, kAddLaunchDealAPI_Keywords, kAddLaunchDealAPI_IndustryKeywords, kLaunchDealAPI_Image, kLaunchDealAPI_Document, kLaunchDealAPI_Audio, kLaunchDealAPI_Video] ;
    ...
    [self getLaunchDealKeywordsList];
    [self getLaunchDealIndustryKeywordsList];
...
- (void)checkKeywordsListFound:(int)keywordType {
    switch (keywordType) {
        ...
        case 6:
        {
            if (launchDealKeywordsArray.count > 0)
                [self openTagsPopup] ;
            else
                [self presentViewController:[UtilityClass displayAlertMessage:kNoLaunchDealKeywordsFoundMessage] animated:YES completion:nil];
        }
            break;
        default:
            break;
...
-(NSString*)convertTagsArrayToStringforArray:(NSMutableArray*)array withTagsArray:(NSArray*)tagsArray tagType:(NSString *)type {
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kLaunchDealAPI_Target_Market_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kLaunchDealAPI_Target_Market_ID])
                    selectedValue = [obj valueForKey:kLaunchDealAPI_Target_Market_ID] ;
                else
                    selectedValue = [obj valueForKey:kLaunchDealAPI_Target_Market_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag)
                        tagsStr = [dict valueForKey:kLaunchDealAPI_Target_Market_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kLaunchDealAPI_Target_Market_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
    } else if ([type isEqualToString:@"LaunchDealkeyword"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kLaunchDealAPI_KeywordID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kLaunchDealAPI_KeywordID])
                    selectedValue = [obj valueForKey:kLaunchDealAPI_KeywordID] ;
                else
                    selectedValue = [obj valueForKey:kLaunchDealAPI_KeywordID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kLaunchDealAPI_KeywordID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kLaunchDealAPI_KeywordID]] ;
                    isFirstTag = NO ;
                }
            }
        }
        ...
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kLaunchDealAPI_Industry_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kLaunchDealAPI_Industry_ID])
                    selectedValue = [obj valueForKey:kLaunchDealAPI_Industry_ID] ;
                else
                    selectedValue = [obj valueForKey:kLaunchDealAPI_Industry_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kLaunchDealAPI_Industry_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kLaunchDealAPI_Industry_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
...
- (IBAction)CheckUncheck_ClickAction:(id)sender {
    ...
    if([btn.accessibilityLabel isEqualToString:CHECK_IMAGE ]) { // Check
        ...
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            if(selectedKeywordType == LAUNCHDEAL_INDUSTRY_KEYWORDS_SELECTED)
                dictKeyword = [industryKeywordsArray objectAtIndex:btn.tag];
            else if (selectedKeywordType == LAUNCHDEAL_KEYWORDS_SELECTED)
                dictKeyword = [launchDealKeywordsArray objectAtIndex:btn.tag];
            else
    ...
    if(selectedKeywordType == LAUNCHDEAL_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (NSMutableDictionary *obj in industryKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedIndustryKeywordNames addObject:[obj valueForKey:kLaunchDealAPI_Industry_Name]] ;
                [selectedIndustryKeywordIds addObject:[obj valueForKey:kLaunchDealAPI_Industry_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_INDUSTRY_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        
    } else if(selectedKeywordType == LAUNCHDEAL_KEYWORDS_SELECTED) {
        [selectedLaunchDealKeywordsArray removeAllObjects];
        [selectedLaunchDealKeywordNames removeAllObjects];
        [selectedLaunchDealKeywordIds removeAllObjects];
        
        for (NSMutableDictionary *obj in launchDealKeywordsArray) {
            NSString *isSelectedStr = [obj valueForKey:@"isSelected"] ;
            if([isSelectedStr isEqualToString:@"1"]) {
                [selectedLaunchDealKeywordsArray addObject:obj] ;
                [selectedLaunchDealKeywordNames addObject:[obj valueForKey:kLaunchDealAPI_KeywordName]] ;
                [selectedLaunchDealKeywordIds addObject:[obj valueForKey:kLaunchDealAPI_KeywordID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        ...
        for (NSMutableDictionary *obj in targetMarketKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedTargetMarketKeywordNames addObject:[obj valueForKey:kLaunchDealAPI_Target_Market_Name]] ;
                [selectedTargetMarketKeywordIds addObject:[obj valueForKey:kLaunchDealAPI_Target_Market_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_TARGET_MARKET_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
    }
}
...
-(void)tagsControl:(UIView *)tag withTagControlIndex:(NSInteger)tagControlIndex removedFromIndex:(NSInteger)index {
    ...
    if(tagControlIndex == LAUNCHDEAL_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (int i=0; i<industryKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[industryKeywordsArray objectAtIndex:i] valueForKey:kLaunchDealAPI_Industry_Name]] ;
    ...
    } else if(tagControlIndex == LAUNCHDEAL_KEYWORDS_SELECTED) {
        NSString *selectedName = [NSString stringWithFormat:@"%@",[selectedLaunchDealKeywordNames objectAtIndex:index]] ;
        for (int i=0; i<launchDealKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[launchDealKeywordsArray objectAtIndex:i] valueForKey:kLaunchDealAPI_KeywordName]] ;
            if([keywordName isEqualToString:selectedName]){
                [[launchDealKeywordsArray objectAtIndex:i] setValue:@"0" forKey:@"isSelected"] ;
                break ;
            }
        }
        [selectedLaunchDealKeywordsArray removeObjectAtIndex:index] ;
        [selectedLaunchDealKeywordNames removeObjectAtIndex:index];
        [selectedLaunchDealKeywordIds removeObjectAtIndex:index];
        ...
        for (int i=0; i<targetMarketKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[targetMarketKeywordsArray objectAtIndex:i] valueForKey:kLaunchDealAPI_Target_Market_Name]] ;
            if([keywordName isEqualToString:selectedName]){
...
- (IBAction)Back_Click:(id)sender {
    [UtilityClass setComingFrom_LaunchDeal_AddEditScreen:YES];
    [self.navigationController popViewControllerAnimated:YES] ;
}

- (IBAction)CloseData_ClickAction:(id)sender {
    [launchDealDataView dismissPresentingPopup] ;
}
...
- (IBAction)followButton_ClickAction:(UIButton*)button {
    ...
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kLaunchDealAPI_FollowedBy] ;
        [dictParam setObject:[[[UtilityClass getLaunchDealDetails] mutableCopy] valueForKey:kLaunchDealAPI_ID] forKey:kLaunchDealAPI_LaunchDealID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isFollowed == 0) {
            [ApiCrowdBootstrap followLaunchDealWithParameters:dictParam success:^(NSDictionary *responseDict) {
                ...
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    ...
                    [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_FOLLOW_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationNone] ;
                }
            ...
            [ApiCrowdBootstrap unfollowLaunchDealWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                    if(isFollowed == 1)
...
- (IBAction)Calender_ClickAction:(id)sender {
    ...
    if ([sender tag] == LAUNCHDEAL_START_DATE_SECTION_INDEX)
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_START_DATE_SECTION_INDEX]] ;
    else
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_END_DATE_SECTION_INDEX]] ;
...
- (IBAction)Submit_ClickAction:(id)sender {
    if ((selectedSegmentControl == 100 && selectedSegment == 0)) {
        [self commitLaunchDeal];
    } else {
        if(![self validatetextFieldsWithSectionIndex:LAUNCHDEAL_TITLE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:LAUNCHDEAL_START_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:LAUNCHDEAL_END_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:LAUNCHDEAL_DESCRIPTION_SECTION_INDEX]) return ;
        ...
        else if(selectedLaunchDealKeywordsArray.count < 1) {
        ...
        else [self editLaunchDeal] ;
    }
}

- (IBAction)LaunchDealCommitmentList_ClickAction:(id)sender {
    ...
    if ([noOfcommit integerValue] > 0) {
        [self getLaunchDealCommitmentList];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    GraphicTableViewCell *cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_IMAGE_SECTION_INDEX]] ;
...
- (IBAction)DatePickerToolbarButtons_ClickAction:(id)sender {
    ...
    if([sender tag] == DONE_CLICKED) {
        if(selectedDatePickerType == LAUNCHDEAL_START_DATE_SELECTED) {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_START_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            cell.textFld.text = [dateFormatter stringFromDate:datePickerView.date];
            [[sectionsArray objectAtIndex:LAUNCHDEAL_START_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
        else {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_END_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            ...
            [[sectionsArray objectAtIndex:LAUNCHDEAL_END_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
    }
    else {
        if(selectedDatePickerType == LAUNCHDEAL_START_DATE_SELECTED) {
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    switch (selectedKeywordType) {
        case 5: // Target Keywords
        {
            ...
            break;
        case 6: // Launch Deal Keywords
        {
            for(NSDictionary *keywordDict in launchDealKeywordsArray)
            {
                NSString *keywordName = [keywordDict objectForKey:@"name"];
                NSRange range = [keywordName rangeOfString:searchText options:NSCaseInsensitiveSearch];
                if(range.location != NSNotFound)
            ...
            NSLog(@"Launch Deal Results: %@",searchResultsForKeywords);
        }
            break;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    DobTableViewCell *startDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_START_DATE_SECTION_INDEX]] ;
    
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        textField.userInteractionEnabled = true;
        if(textField.tag == LAUNCHDEAL_START_DATE_SECTION_INDEX || textField.tag == LAUNCHDEAL_END_DATE_SECTION_INDEX) {
            if(textField.tag == LAUNCHDEAL_START_DATE_SECTION_INDEX) {
                selectedDatePickerType = LAUNCHDEAL_START_DATE_SELECTED;
            }
            else {
                selectedDatePickerType = LAUNCHDEAL_END_DATE_SELECTED;
...
-(void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if(textField.tag == LAUNCHDEAL_START_DATE_SECTION_INDEX || textField.tag == LAUNCHDEAL_END_DATE_SECTION_INDEX) {
            if(textField.tag == LAUNCHDEAL_START_DATE_SECTION_INDEX) {
                DobTableViewCell *endDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:LAUNCHDEAL_END_DATE_SECTION_INDEX]] ;
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@"Launch Deal Description"] && textView.textColor == [UIColor lightGrayColor]){
            textView.text = @"" ;
...
-(BOOL)textViewShouldEndEditing:(UITextView *)textView {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@""]){
            textView.text = @"Launch Deal Description" ;
    ...
    if (indexPath.row == 0) {
        ...
        for (int i = 0; i < [sectionsArray count]; i++) {
            if (indexPath.section == i) {
                if (indexPath.section == LAUNCHDEAL_DOCUMENT_SECTION_INDEX) {
                    if (![docuementFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == LAUNCHDEAL_AUDIO_SECTION_INDEX) {
                    if (![audioFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == LAUNCHDEAL_VIDEO_SECTION_INDEX) {
...
-(void)getLaunchDealKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getLaunchDealKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [launchDealKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kLaunchDealAPI_KeywordList]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kLaunchDealAPI_KeywordList] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
                        [launchDealKeywordsArray addObject:obj] ;
                    }
                }
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode) {
...
-(void)getLaunchDealIndustryKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getLaunchDealIndustryKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [industryKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kLaunchDealAPI_Industry_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kLaunchDealAPI_Industry_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getTargetMarketKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getLaunchDealTargetMarketKeywordsListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [targetMarketKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kLaunchDealAPI_Target_Market_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kLaunchDealAPI_Target_Market_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getLaunchDealDetails {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kLaunchDealAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[launchDealData valueForKey:kLaunchDealAPI_ID]] forKey:kLaunchDealAPI_LaunchDealID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap viewLaunchDealWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"respos %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                for ( int i = 0; i < sectionsArray.count ; i++ ) {
                    NSString *key = [[sectionsArray objectAtIndex:i] valueForKey:@"key"] ;
                    if(i != LAUNCHDEAL_INDUSTRY_KEYWORDS_SECTION_INDEX && i != LAUNCHDEAL_TARGET_MARKET_KEYWORDS_SECTION_INDEX && i != LAUNCHDEAL_KEYWORDS_SECTION_INDEX) {
                ...
                // Keywords Array
                if([responseDict objectForKey:kAddLaunchDealAPI_IndustryKeywords]) {
                    selectedIndustryKeywordsArray = [[responseDict objectForKey:kAddLaunchDealAPI_IndustryKeywords] mutableCopy];
                ...
                if([responseDict objectForKey:kAddLaunchDealAPI_Keywords]) {
                    selectedLaunchDealKeywordsArray = [[responseDict objectForKey:kAddLaunchDealAPI_Keywords] mutableCopy] ;
                    selectedLaunchDealKeywordNames = [self resetTagsArrayWithData:selectedLaunchDealKeywordsArray];
                    selectedLaunchDealKeywordIds = [self resetTagIdsArrayWithData:selectedLaunchDealKeywordsArray];
                }
                if([responseDict objectForKey:kAddLaunchDealAPI_Target_Market_Keywords]) {
                    selectedTargetMarketKeywordsArray = [[responseDict objectForKey:kAddLaunchDealAPI_Target_Market_Keywords] mutableCopy];
                ...
                // Commits
                noOfcommit = [[responseDict valueForKey:kLaunchDealAPI_NoOfCommits] stringValue];
                isCommited = [[responseDict valueForKey:kLaunchDealAPI_IsCommited] intValue];
                isLiked = [[responseDict valueForKey:kLaunchDealAPI_IsLiked] intValue];
                isFollowed = [[responseDict valueForKey:kLaunchDealAPI_IsFollowed] intValue];
                
                // Document
                if([responseDict objectForKey:kLaunchDealAPI_Document])
                    docuementFile = [[responseDict objectForKey:kLaunchDealAPI_Document] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Audio
                if([responseDict objectForKey:kLaunchDealAPI_Audio])
                    audioFile = [[responseDict objectForKey:kLaunchDealAPI_Audio] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Video
                if([responseDict objectForKey:kLaunchDealAPI_Video])
                    videoFile = [[responseDict objectForKey:kLaunchDealAPI_Video] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Image
                if([responseDict objectForKey:kLaunchDealAPI_Image]) {
                    NSString *strImage = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed, [responseDict objectForKey:kLaunchDealAPI_Image]];
                    NSURL *url = [NSURL URLWithString:[strImage stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                    imgData = [NSData dataWithContentsOfURL:url];
                }
                
                // Refresh Dict
                [launchDealData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kLaunchDealAPI_Title]] forKey:kLaunchDealAPI_Title] ;
                [launchDealData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kLaunchDealAPI_CreatedBy]] forKey:kLaunchDealAPI_CreatedBy] ;
                [launchDealData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kLaunchDealAPI_StartDate]] forKey:kLaunchDealAPI_StartDate] ;
                [launchDealData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kLaunchDealAPI_EndDate]] forKey:kLaunchDealAPI_EndDate] ;
                [launchDealData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kLaunchDealAPI_Description]] forKey:kLaunchDealAPI_Description] ;
                
                [UtilityClass setLaunchDealDetails:[launchDealData mutableCopy]] ;
...
-(void)commitLaunchDeal {
    if([UtilityClass checkInternetConnection]) {
        ...
        if (isCommited == 0) {
            [ApiCrowdBootstrap commitLaunchDealWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
            ...
            [ApiCrowdBootstrap uncommitLaunchDealWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
...
-(void)getLaunchDealCommitmentList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getLaunchDealCommitmentListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode)  {
...
- (void)getRecommendedUserList: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:@"launchdeal" forKey:@"type"] ;
        ...
        [ApiCrowdBootstrap getRegisteredRoleListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kLaunchDealAPI_UserList]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]){
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kLaunchDealAPI_UserList]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kLaunchDealAPI_UserList]] ;
                    }
            ...
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
            {
                ...
                if(![searchBarUsers.text isEqualToString:@""])
                    ...
                    usersArray = [NSMutableArray arrayWithArray:[responseDict valueForKey:kLaunchDealAPI_UserList]] ;
...
-(void)editLaunchDeal {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[[sectionsArray objectAtIndex:LAUNCHDEAL_TITLE_SECTION_INDEX] valueForKey:@"value"] forKey:kLaunchDealAPI_Title] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:LAUNCHDEAL_DESCRIPTION_SECTION_INDEX] valueForKey:@"value"] forKey:kLaunchDealAPI_Description] ;
        
        [dictParam setObject:[[sectionsArray objectAtIndex:LAUNCHDEAL_START_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kLaunchDealAPI_StartDate] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:LAUNCHDEAL_END_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kLaunchDealAPI_EndDate] ;
        ...
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedLaunchDealKeywordsArray withTagsArray:launchDealKeywordsArray tagType:@"LaunchDealkeyword"] forKey:kAddLaunchDealAPI_Keywords] ;
        ...
        if(imgData)
            [dictParam setObject:imgData forKey:kLaunchDealAPI_Image] ;
        else
            [dictParam setObject:@"" forKey:kLaunchDealAPI_Image] ;
        
        [dictParam setObject:@"" forKey:kLaunchDealAPI_Document] ;
        [dictParam setObject:@"" forKey:kLaunchDealAPI_Audio] ;
        [dictParam setObject:@"" forKey:kLaunchDealAPI_Video] ;
        ...
        [ApiCrowdBootstrap editLaunchDealWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kEditLaunchDeal_SuccessMessage withResultType:@"0" withDuration:1] ;
                [UtilityClass setComingFrom_LaunchDeal_AddEditScreen:YES];
                [self.navigationController popViewControllerAnimated:YES] ;
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == popupTblView) {
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            else if (selectedKeywordType == LAUNCHDEAL_TARGET_MARKET_KEYWORDS_SELECTED)
                return targetMarketKeywordsArray.count ;
            else
                return launchDealKeywordsArray.count ;
        }
        ...
        if(section <= LAUNCHDEAL_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)
            return 1;
        else {
            if(section == LAUNCHDEAL_IMAGE_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue])
                    return 1;
                else return 0 ;
            }
            else if(section == LAUNCHDEAL_DOCUMENT_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == LAUNCHDEAL_AUDIO_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(selectedKeywordType == LAUNCHDEAL_INDUSTRY_KEYWORDS_SELECTED) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kLaunchDealAPI_Industry_Name] ;
            if ([selectedIndustryKeywordIds containsObject:[dictKeyword valueForKey:kLaunchDealAPI_Industry_ID]]) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kLaunchDealAPI_Target_Market_Name] ;
            
            if ([selectedTargetMarketKeywordIds containsObject:[dictKeyword valueForKey:kLaunchDealAPI_Target_Market_ID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        } else
        {
            if (![searchBarKeywords.text isEqualToString:@""])
                ...
                dictKeyword = [launchDealKeywordsArray objectAtIndex:indexPath.row];
            
            cell.companyNameLbl.text = [dictKeyword valueForKey:kLaunchDealAPI_KeywordName] ;
            
            if ([selectedLaunchDealKeywordIds containsObject:[dictKeyword valueForKey:kLaunchDealAPI_KeywordID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        if (![searchBarUsers.text isEqualToString:@""]) {
            ...
            cell.lblName.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kLaunchDealAPI_User_Name]] ;
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kLaunchDealAPI_User_Desc]];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kLaunchDealAPI_User_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
        ...
        if(indexPath.section == sectionsArray.count) {
        ...
        else if(indexPath.section == LAUNCHDEAL_FOLLOW_SECTION_INDEX) {
        ...
        else if(indexPath.section == LAUNCHDEAL_INDUSTRY_KEYWORDS_SECTION_INDEX || indexPath.section == LAUNCHDEAL_TARGET_MARKET_KEYWORDS_SECTION_INDEX || indexPath.section == LAUNCHDEAL_KEYWORDS_SECTION_INDEX) {
            ...
            NSMutableArray *tags ;
            if(indexPath.section == LAUNCHDEAL_INDUSTRY_KEYWORDS_SECTION_INDEX)
                tags = [selectedIndustryKeywordNames mutableCopy] ;
            else if(indexPath.section == LAUNCHDEAL_TARGET_MARKET_KEYWORDS_SECTION_INDEX)
                tags = [selectedTargetMarketKeywordNames mutableCopy] ;
            else
                tags = [selectedLaunchDealKeywordNames mutableCopy] ;
        ...
        else if(indexPath.section == LAUNCHDEAL_DESCRIPTION_SECTION_INDEX) {
        ...
        else if(indexPath.section == LAUNCHDEAL_START_DATE_SECTION_INDEX || indexPath.section == LAUNCHDEAL_END_DATE_SECTION_INDEX) {
        ...
        else if(indexPath.section == LAUNCHDEAL_IMAGE_SECTION_INDEX) {
        ...
        else if (indexPath.section == LAUNCHDEAL_DOCUMENT_SECTION_INDEX || indexPath.section == LAUNCHDEAL_AUDIO_SECTION_INDEX || indexPath.section == LAUNCHDEAL_VIDEO_SECTION_INDEX) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(indexPath.section >= LAUNCHDEAL_DOCUMENT_SECTION_INDEX && indexPath.section < sectionsArray.count) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if(indexPath.section == LAUNCHDEAL_START_DATE_SECTION_INDEX || indexPath.section == LAUNCHDEAL_END_DATE_SECTION_INDEX) {
            return 65;
        }
        else if (indexPath.section == LAUNCHDEAL_FOLLOW_SECTION_INDEX)
            return 40;
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if(section <= LAUNCHDEAL_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)return 0;
    else return 45 ;
}
