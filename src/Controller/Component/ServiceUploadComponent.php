...
class ServiceUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('ServiceInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('ServiceKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('ServiceTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('ServiceFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['service_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('ServiceCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['service_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('ServiceCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['service_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/service/"; // Upload directory
                $imgCamp='';
                $errors='';
