...
function retrieveUsers(usersStorage, callback) {
  ...
  //
  QB.users.listUsers({page: usersStorage.currentPage, per_page: '10000000'}, function(err, result) {
    if (err) {
      console.log(err);
    } else {
      console.log(result);
...
function getUserLoginById(byId) {
	...
	if (users[byId]) {
		...
		//alert(JSON.stringify(users[byId]));

    if(users[byId].full_name != null)
    {
