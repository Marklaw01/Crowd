//
//  AddEarlyAdopterViewController.m
//  CrowdBootstrap
//
...
#import "AddEarlyAdopterViewController.h"
...
@interface AddEarlyAdopterViewController ()

@end

@implementation AddEarlyAdopterViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Early Adopter Requirements";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Early Adopter Title", @"Early Adopter Description", @"Early Adopter Start Date", @"Early Adopter End Date", @"Target Market", @"Early Adopter Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kEarlyAdopterAPI_EarlyAdopter_Title, kEarlyAdopterAPI_Description, kEarlyAdopterAPI_StartDate, kEarlyAdopterAPI_EndDate, kAddEarlyAdopterAPI_Target_Market_Keywords, kAddEarlyAdopterAPI_Keywords, kAddEarlyAdopterAPI_IndustryKeywords, kEarlyAdopterAPI_EarlyAdopter_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kEarlyAdopterAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kEarlyAdopterAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kEarlyAdopterAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)addEarlyAdopter {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addEarlyAdopterWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddEarlyAdopter_SuccessMessage withResultType:@"0" withDuration:1] ;
