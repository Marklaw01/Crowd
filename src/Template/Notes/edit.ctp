<div class="container-fluid">
          ...
          <div class="row">
            <div class="col-lg-8 col-md-8 col-sm-12 ">
                ...
                if(!empty($startups)){
                            /*foreach($startups as $SingleUser):
                                if(isset($SingleUser['SID'])&&($SingleUser['SID']!='')){
                                  if($SingleUser['SID']==$sKey){ $selc= 'selected="selected"';}else{$selc='';}
                                     echo '<option value="'.$SingleUser['SID'].'" '.$selc.'>'.$SingleUser['SNAME'].'</option>';
                                }
                            endforeach;*/

                            foreach($startups as $SingleUser):
                                    if($SingleUser->id==$sKey){ $selc= 'selected="selected"';}else{$selc='';}
                                     echo '<option value="'.$SingleUser->id.'" '.$selc.'>'.$SingleUser->name.'</option>';
                            endforeach;
                }
                ?> 
              </select>  
              ...
              <div class="form-group">
                <?php
                    echo $this->Form->input('comment', ARRAY('label' => false, 'div' => false,'error' => false,'type' => 'textarea','placeholder'=>'Comment','class' => 'form-control'));
                 ?>
                 <?php 
