//
//  EditConferenceViewController.m
//  CrowdBootstrap
//
...
#import "EditConferenceViewController.h"
...
@interface EditConferenceViewController ()

@end

@implementation EditConferenceViewController
...
-(void) awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(SetViewEditingNotification:) name:kNotificationSetConferenceViewEditing
                                               object:nil];
...
-(void)SetViewEditingNotification:(NSNotification *)notification {
    if ([[notification name] isEqualToString:kNotificationSetConferenceViewEditing]) {
...
-(void)resetUISettings {
    ...
    conferenceData = [[UtilityClass getConferenceDetails] mutableCopy];
    NSLog(@"Conference Data: %@",conferenceData) ;
    ...
    self.title = [NSString stringWithFormat:@"%@",[conferenceData valueForKey:kConferenceAPI_Title]] ;
...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Created By", @"Conference Title", @"Conference Description", @"Conference Availability Start Date", @"Conference Availability End Date", @"Target Market", @"Conference Keywords", @"Interest Keywords", @"Upload Image", @"View Document",@"Play Audio",@"Play Video"] ;
    NSArray *parametersArray = @[kConferenceAPI_CreatedBy, kConferenceAPI_Title, kConferenceAPI_Description, kConferenceAPI_StartDate, kConferenceAPI_EndDate, kAddConferenceAPI_Target_Market_Keywords, kAddConferenceAPI_Keywords, kAddConferenceAPI_IndustryKeywords, kConferenceAPI_Image, kConferenceAPI_Document, kConferenceAPI_Audio, kConferenceAPI_Video] ;
    ...
    [self getConferenceKeywordsList];
    [self getConferenceIndustryKeywordsList];
...
- (void)checkKeywordsListFound:(int)keywordType {
    switch (keywordType) {
        ...
        case 6:
        {
            if (conferenceKeywordsArray.count > 0)
                [self openTagsPopup] ;
            else
                [self presentViewController:[UtilityClass displayAlertMessage:kNoConferenceKeywordsFoundMessage] animated:YES completion:nil];
        }
            break;
        default:
            break;
...
-(NSString*)convertTagsArrayToStringforArray:(NSMutableArray*)array withTagsArray:(NSArray*)tagsArray tagType:(NSString *)type {
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kConferenceAPI_Target_Market_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kConferenceAPI_Target_Market_ID])
                    selectedValue = [obj valueForKey:kConferenceAPI_Target_Market_ID] ;
                else
                    selectedValue = [obj valueForKey:kConferenceAPI_Target_Market_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag)
                        tagsStr = [dict valueForKey:kConferenceAPI_Target_Market_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kConferenceAPI_Target_Market_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
    } else if ([type isEqualToString:@"Conferencekeyword"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kConferenceAPI_KeywordID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kConferenceAPI_KeywordID])
                    selectedValue = [obj valueForKey:kConferenceAPI_KeywordID] ;
                else
                    selectedValue = [obj valueForKey:kConferenceAPI_KeywordID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kConferenceAPI_KeywordID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kConferenceAPI_KeywordID]] ;
                    isFirstTag = NO ;
                }
            }
        }
        ...
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kConferenceAPI_Industry_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kConferenceAPI_Industry_ID])
                    selectedValue = [obj valueForKey:kConferenceAPI_Industry_ID] ;
                else
                    selectedValue = [obj valueForKey:kConferenceAPI_Industry_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kConferenceAPI_Industry_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kConferenceAPI_Industry_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
...
- (IBAction)CheckUncheck_ClickAction:(id)sender {
    ...
    if([btn.accessibilityLabel isEqualToString:CHECK_IMAGE ]) { // Check
        ...
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            if(selectedKeywordType == CONFERENCE_INDUSTRY_KEYWORDS_SELECTED)
                dictKeyword = [industryKeywordsArray objectAtIndex:btn.tag];
            else if (selectedKeywordType == CONFERENCE_KEYWORDS_SELECTED)
                dictKeyword = [conferenceKeywordsArray objectAtIndex:btn.tag];
            else
    ...
    if(selectedKeywordType == CONFERENCE_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (NSMutableDictionary *obj in industryKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedIndustryKeywordNames addObject:[obj valueForKey:kConferenceAPI_Industry_Name]] ;
                [selectedIndustryKeywordIds addObject:[obj valueForKey:kConferenceAPI_Industry_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_INDUSTRY_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        
    } else if(selectedKeywordType == CONFERENCE_KEYWORDS_SELECTED) {
        [selectedConferenceKeywordsArray removeAllObjects];
        [selectedConferenceKeywordNames removeAllObjects];
        [selectedConferenceKeywordIds removeAllObjects];
        
        for (NSMutableDictionary *obj in conferenceKeywordsArray) {
            NSString *isSelectedStr = [obj valueForKey:@"isSelected"] ;
            if([isSelectedStr isEqualToString:@"1"]) {
                [selectedConferenceKeywordsArray addObject:obj] ;
                [selectedConferenceKeywordNames addObject:[obj valueForKey:kConferenceAPI_KeywordName]] ;
                [selectedConferenceKeywordIds addObject:[obj valueForKey:kConferenceAPI_KeywordID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        ...
        for (NSMutableDictionary *obj in targetMarketKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedTargetMarketKeywordNames addObject:[obj valueForKey:kConferenceAPI_Target_Market_Name]] ;
                [selectedTargetMarketKeywordIds addObject:[obj valueForKey:kConferenceAPI_Target_Market_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_TARGET_MARKET_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
    }
}
...
-(void)tagsControl:(UIView *)tag withTagControlIndex:(NSInteger)tagControlIndex removedFromIndex:(NSInteger)index {
    ...
    if(tagControlIndex == CONFERENCE_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (int i=0; i<industryKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[industryKeywordsArray objectAtIndex:i] valueForKey:kConferenceAPI_Industry_Name]] ;
    ...
    } else if(tagControlIndex == CONFERENCE_KEYWORDS_SELECTED) {
        NSString *selectedName = [NSString stringWithFormat:@"%@",[selectedConferenceKeywordNames objectAtIndex:index]] ;
        for (int i=0; i<conferenceKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[conferenceKeywordsArray objectAtIndex:i] valueForKey:kConferenceAPI_KeywordName]] ;
            if([keywordName isEqualToString:selectedName]){
                [[conferenceKeywordsArray objectAtIndex:i] setValue:@"0" forKey:@"isSelected"] ;
                break ;
            }
        }
        [selectedConferenceKeywordsArray removeObjectAtIndex:index] ;
        [selectedConferenceKeywordNames removeObjectAtIndex:index];
        [selectedConferenceKeywordIds removeObjectAtIndex:index];
        ...
        for (int i=0; i<targetMarketKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[targetMarketKeywordsArray objectAtIndex:i] valueForKey:kConferenceAPI_Target_Market_Name]] ;
            if([keywordName isEqualToString:selectedName]){
...
- (IBAction)Back_Click:(id)sender {
    [UtilityClass setComingFrom_Conference_AddEditScreen:YES];
    [self.navigationController popViewControllerAnimated:YES] ;
}

- (IBAction)CloseData_ClickAction:(id)sender {
    [conferenceDataView dismissPresentingPopup] ;
}
...
- (IBAction)followButton_ClickAction:(UIButton*)button {
    ...
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kConferenceAPI_FollowedBy] ;
        [dictParam setObject:[[[UtilityClass getConferenceDetails] mutableCopy] valueForKey:kConferenceAPI_ID] forKey:kConferenceAPI_ConferenceID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isFollowed == 0) {
            [ApiCrowdBootstrap followConferenceWithParameters:dictParam success:^(NSDictionary *responseDict) {
                ...
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    ...
                    [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_FOLLOW_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationNone] ;
                }
            ...
            [ApiCrowdBootstrap unfollowConferenceWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                    if(isFollowed == 1)
...
- (IBAction)Calender_ClickAction:(id)sender {
    ...
    if ([sender tag] == CONFERENCE_START_DATE_SECTION_INDEX)
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_START_DATE_SECTION_INDEX]] ;
    else
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_END_DATE_SECTION_INDEX]] ;
...
- (IBAction)Submit_ClickAction:(id)sender {
    if ((selectedSegmentControl == 100 && selectedSegment == 0)) {
        [self commitConference];
    } else {
        if(![self validatetextFieldsWithSectionIndex:CONFERENCE_TITLE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:CONFERENCE_START_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:CONFERENCE_END_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:CONFERENCE_DESCRIPTION_SECTION_INDEX]) return ;
        ...
        else if(selectedConferenceKeywordsArray.count < 1) {
        ...
        else [self editConference] ;
    }
}

- (IBAction)ConferenceCommitmentList_ClickAction:(id)sender {
    ...
    if ([noOfcommit integerValue] > 0) {
        [self getConferenceCommitmentList];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    GraphicTableViewCell *cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_IMAGE_SECTION_INDEX]] ;
...
- (IBAction)DatePickerToolbarButtons_ClickAction:(id)sender {
    ...
    if([sender tag] == DONE_CLICKED) {
        if(selectedDatePickerType == CONFERENCE_START_DATE_SELECTED) {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_START_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            cell.textFld.text = [dateFormatter stringFromDate:datePickerView.date];
            [[sectionsArray objectAtIndex:CONFERENCE_START_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
        else {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_END_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            ...
            [[sectionsArray objectAtIndex:CONFERENCE_END_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
    }
    else {
        if(selectedDatePickerType == CONFERENCE_START_DATE_SELECTED) {
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    switch (selectedKeywordType) {
        case 5: // Target Keywords
        {
            ...
            break;
        case 6: // Conference Keywords
        {
            for(NSDictionary *keywordDict in conferenceKeywordsArray)
            {
                NSString *keywordName = [keywordDict objectForKey:@"name"];
                NSRange range = [keywordName rangeOfString:searchText options:NSCaseInsensitiveSearch];
                if(range.location != NSNotFound)
            ...
            NSLog(@"Conference Results: %@",searchResultsForKeywords);
        }
            break;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    DobTableViewCell *startDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_START_DATE_SECTION_INDEX]] ;
    
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        textField.userInteractionEnabled = true;
        if(textField.tag == CONFERENCE_START_DATE_SECTION_INDEX || textField.tag == CONFERENCE_END_DATE_SECTION_INDEX) {
            if(textField.tag == CONFERENCE_START_DATE_SECTION_INDEX) {
                selectedDatePickerType = CONFERENCE_START_DATE_SELECTED;
            }
            else {
                selectedDatePickerType = CONFERENCE_END_DATE_SELECTED;
...
-(void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if(textField.tag == CONFERENCE_START_DATE_SECTION_INDEX || textField.tag == CONFERENCE_END_DATE_SECTION_INDEX) {
            if(textField.tag == CONFERENCE_START_DATE_SECTION_INDEX) {
                DobTableViewCell *endDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:CONFERENCE_END_DATE_SECTION_INDEX]] ;
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@"Conference Description"] && textView.textColor == [UIColor lightGrayColor]){
            textView.text = @"" ;
...
-(BOOL)textViewShouldEndEditing:(UITextView *)textView {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@""]){
            textView.text = @"Conference Description" ;
    ...
    if (indexPath.row == 0) {
        ...
        for (int i = 0; i < [sectionsArray count]; i++) {
            if (indexPath.section == i) {
                if (indexPath.section == CONFERENCE_DOCUMENT_SECTION_INDEX) {
                    if (![docuementFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == CONFERENCE_AUDIO_SECTION_INDEX) {
                    if (![audioFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == CONFERENCE_VIDEO_SECTION_INDEX) {
...
-(void)getConferenceKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getConferenceKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [conferenceKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kConferenceAPI_KeywordList]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kConferenceAPI_KeywordList] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
                        [conferenceKeywordsArray addObject:obj] ;
                    }
                }
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode) {
...
-(void)getConferenceIndustryKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getConferenceIndustryKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [industryKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kConferenceAPI_Industry_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kConferenceAPI_Industry_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getTargetMarketKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getConferenceTargetMarketKeywordsListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [targetMarketKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kConferenceAPI_Target_Market_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kConferenceAPI_Target_Market_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getConferenceDetails {
    if([UtilityClass checkInternetConnection]) {
        ...
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kConferenceAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[conferenceData valueForKey:kConferenceAPI_ID]] forKey:kConferenceAPI_ConferenceID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap viewConferenceWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"respos %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                for ( int i = 0; i < sectionsArray.count ; i++ ) {
                    NSString *key = [[sectionsArray objectAtIndex:i] valueForKey:@"key"] ;
                    if(i != CONFERENCE_INDUSTRY_KEYWORDS_SECTION_INDEX && i != CONFERENCE_TARGET_MARKET_KEYWORDS_SECTION_INDEX && i != CONFERENCE_KEYWORDS_SECTION_INDEX) {
                ...
                // Keywords Array
                if([responseDict objectForKey:kAddConferenceAPI_IndustryKeywords]) {
                    selectedIndustryKeywordsArray = [[responseDict objectForKey:kAddConferenceAPI_IndustryKeywords] mutableCopy];
                ...
                if([responseDict objectForKey:kAddConferenceAPI_Keywords]) {
                    selectedConferenceKeywordsArray = [[responseDict objectForKey:kAddConferenceAPI_Keywords] mutableCopy] ;
                    selectedConferenceKeywordNames = [self resetTagsArrayWithData:selectedConferenceKeywordsArray];
                    selectedConferenceKeywordIds = [self resetTagIdsArrayWithData:selectedConferenceKeywordsArray];
                }
                if([responseDict objectForKey:kAddConferenceAPI_Target_Market_Keywords]) {
                    selectedTargetMarketKeywordsArray = [[responseDict objectForKey:kAddConferenceAPI_Target_Market_Keywords] mutableCopy];
                ...
                // Commits
                noOfcommit = [[responseDict valueForKey:kConferenceAPI_NoOfCommits] stringValue];
                isCommited = [[responseDict valueForKey:kConferenceAPI_IsCommited] intValue];
                isLiked = [[responseDict valueForKey:kConferenceAPI_IsLiked] intValue];
                isFollowed = [[responseDict valueForKey:kConferenceAPI_IsFollowed] intValue];
                
                // Document
                if([responseDict objectForKey:kConferenceAPI_Document])
                    docuementFile = [[responseDict objectForKey:kConferenceAPI_Document] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Audio
                if([responseDict objectForKey:kConferenceAPI_Audio])
                    audioFile = [[responseDict objectForKey:kConferenceAPI_Audio] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Video
                if([responseDict objectForKey:kConferenceAPI_Video])
                    videoFile = [[responseDict objectForKey:kConferenceAPI_Video] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Image
                if([responseDict objectForKey:kConferenceAPI_Image]) {
                    NSString *strImage = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed, [responseDict objectForKey:kConferenceAPI_Image]];
                    NSURL *url = [NSURL URLWithString:[strImage stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                    imgData = [NSData dataWithContentsOfURL:url];
                }

                // Refresh Dict
                [conferenceData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConferenceAPI_Title]] forKey:kConferenceAPI_Title] ;
                [conferenceData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConferenceAPI_CreatedBy]] forKey:kConferenceAPI_CreatedBy] ;
                [conferenceData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConferenceAPI_StartDate]] forKey:kConferenceAPI_StartDate] ;
                [conferenceData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConferenceAPI_EndDate]] forKey:kConferenceAPI_EndDate] ;
                [conferenceData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConferenceAPI_Description]] forKey:kConferenceAPI_Description] ;
                
                [UtilityClass setConferenceDetails:[conferenceData mutableCopy]] ;
...
-(void)commitConference {
    if([UtilityClass checkInternetConnection]) {
        ...
        if (isCommited == 0) {
            [ApiCrowdBootstrap commitConferenceWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
            ...
            [ApiCrowdBootstrap uncommitConferenceWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
...
-(void)getConferenceCommitmentList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getConferenceCommitmentListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode)  {
...
- (void)getRecommendedUserList: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:@"Conference" forKey:@"type"] ;
        ...
        [ApiCrowdBootstrap getRegisteredRoleListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kConferenceAPI_UserList]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]){
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kConferenceAPI_UserList]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kConferenceAPI_UserList]] ;
                    }
            ...
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
            {
                ...
                if(![searchBarUsers.text isEqualToString:@""])
                    ...
                    usersArray = [NSMutableArray arrayWithArray:[responseDict valueForKey:kConferenceAPI_UserList]] ;
...
-(void)editConference {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[[sectionsArray objectAtIndex:CONFERENCE_TITLE_SECTION_INDEX] valueForKey:@"value"] forKey:kConferenceAPI_Title] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:CONFERENCE_DESCRIPTION_SECTION_INDEX] valueForKey:@"value"] forKey:kConferenceAPI_Description] ;
        
        [dictParam setObject:[[sectionsArray objectAtIndex:CONFERENCE_START_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kConferenceAPI_StartDate] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:CONFERENCE_END_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kConferenceAPI_EndDate] ;
        
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedIndustryKeywordsArray withTagsArray:industryKeywordsArray tagType:@"industry"] forKey:kAddConferenceAPI_IndustryKeywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedConferenceKeywordsArray withTagsArray:conferenceKeywordsArray tagType:@"Conferencekeyword"] forKey:kAddConferenceAPI_Keywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedTargetMarketKeywordsArray withTagsArray:targetMarketKeywordsArray tagType:@"targetmarket"] forKey:kAddConferenceAPI_Target_Market_Keywords] ;
        
        if(imgData)
            [dictParam setObject:imgData forKey:kConferenceAPI_Image] ;
        else
            [dictParam setObject:@"" forKey:kConferenceAPI_Image] ;
        
        [dictParam setObject:@"" forKey:kConferenceAPI_Document] ;
        [dictParam setObject:@"" forKey:kConferenceAPI_Audio] ;
        [dictParam setObject:@"" forKey:kConferenceAPI_Video] ;
        ...
        [ApiCrowdBootstrap editConferenceWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kEditConference_SuccessMessage withResultType:@"0" withDuration:1] ;
                [UtilityClass setComingFrom_Conference_AddEditScreen:YES];
                [self.navigationController popViewControllerAnimated:YES] ;
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == popupTblView) {
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            else if (selectedKeywordType == CONFERENCE_TARGET_MARKET_KEYWORDS_SELECTED)
                return targetMarketKeywordsArray.count ;
            else
                return conferenceKeywordsArray.count ;
        }
        ...
        if(section <= CONFERENCE_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)
            return 1;
        else {
            if(section == CONFERENCE_IMAGE_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue])
                    return 1;
                else return 0 ;
            }
            else if(section == CONFERENCE_DOCUMENT_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == CONFERENCE_AUDIO_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(selectedKeywordType == CONFERENCE_INDUSTRY_KEYWORDS_SELECTED) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kConferenceAPI_Industry_Name] ;
            if ([selectedIndustryKeywordIds containsObject:[dictKeyword valueForKey:kConferenceAPI_Industry_ID]]) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kConferenceAPI_Target_Market_Name] ;
            
            if ([selectedTargetMarketKeywordIds containsObject:[dictKeyword valueForKey:kConferenceAPI_Target_Market_ID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        } else
        {
            if (![searchBarKeywords.text isEqualToString:@""])
                ...
                dictKeyword = [conferenceKeywordsArray objectAtIndex:indexPath.row];
            
            cell.companyNameLbl.text = [dictKeyword valueForKey:kConferenceAPI_KeywordName] ;
            
            if ([selectedConferenceKeywordIds containsObject:[dictKeyword valueForKey:kConferenceAPI_KeywordID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        if (![searchBarUsers.text isEqualToString:@""]) {
            ...
            cell.lblName.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kConferenceAPI_User_Name]] ;
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kConferenceAPI_User_Desc]];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kConferenceAPI_User_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
        ...
        if(indexPath.section == sectionsArray.count) {
        ...
        else if(indexPath.section == CONFERENCE_FOLLOW_SECTION_INDEX) {
        ...
        else if(indexPath.section == CONFERENCE_INDUSTRY_KEYWORDS_SECTION_INDEX || indexPath.section == CONFERENCE_TARGET_MARKET_KEYWORDS_SECTION_INDEX || indexPath.section == CONFERENCE_KEYWORDS_SECTION_INDEX) {
            ...
            NSMutableArray *tags ;
            if(indexPath.section == CONFERENCE_INDUSTRY_KEYWORDS_SECTION_INDEX)
                tags = [selectedIndustryKeywordNames mutableCopy] ;
            else if(indexPath.section == CONFERENCE_TARGET_MARKET_KEYWORDS_SECTION_INDEX)
                tags = [selectedTargetMarketKeywordNames mutableCopy] ;
            else
                tags = [selectedConferenceKeywordNames mutableCopy] ;
        ...
        else if(indexPath.section == CONFERENCE_DESCRIPTION_SECTION_INDEX) {
        ...
        else if(indexPath.section == CONFERENCE_START_DATE_SECTION_INDEX || indexPath.section == CONFERENCE_END_DATE_SECTION_INDEX) {
        ...
        else if(indexPath.section == CONFERENCE_IMAGE_SECTION_INDEX) {
        ...
        else if (indexPath.section == CONFERENCE_DOCUMENT_SECTION_INDEX || indexPath.section == CONFERENCE_AUDIO_SECTION_INDEX || indexPath.section == CONFERENCE_VIDEO_SECTION_INDEX) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(indexPath.section >= CONFERENCE_DOCUMENT_SECTION_INDEX && indexPath.section < sectionsArray.count) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if(indexPath.section == CONFERENCE_START_DATE_SECTION_INDEX || indexPath.section == CONFERENCE_END_DATE_SECTION_INDEX) {
            return 65;
        }
        else if (indexPath.section == CONFERENCE_FOLLOW_SECTION_INDEX)
            return 40;
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if(section <= CONFERENCE_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)return 0;
    else return 45 ;
}
