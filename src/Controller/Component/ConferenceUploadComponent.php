...
class ConferenceUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('ConferenceInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('ConferenceKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('ConferenceTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('ConferenceFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['conference_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('ConferenceCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['conference_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('ConferenceCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['conference_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/conference/"; // Upload directory
                $imgCamp='';
                $errors='';
