...
//$totalDays = count($FinalObject);
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1', 'Consumed Hours');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1', 'Remaining Hours');
...
foreach($finalContractorDetails as $singleObject){
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$dateIndex,$singleObject['ConsumedHours']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$dateIndex,$singleObject['RemainingHours']);
...
$objWriter->save(WWW_ROOT.'excel_files/workorder.xlsx');
chmod(WWW_ROOT.'excel_files/test.xlsx', 0777);

            $filePath = WWW_ROOT .'excel_files'. DS . 'workorder.xlsx';
            $this->response->file($filePath ,array('download'=> true));
            return $this->response;
            $this->redirect(['action' => 'editStartupWorkorder',$startupId]);
