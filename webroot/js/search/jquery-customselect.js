      ...
      var methods   = {
          ...
          var setup = {
            ...
            value: function() {
              ...
              //value.find("span").html(option.text().length > 0 ? option.text() : $options.emptytext);
               var ccvj =$select.closest('select').attr('id');

              if(ccvj == 'standard1'){
                value.find("span").html('Select Target Market');
              }else if(ccvj == 'standard2'){
                value.find("span").html('Select Campaign Keyword');
              }else if(ccvj == 'standard11'){
                value.find("span").html("Select Fund's Managers");
              }else if(ccvj == 'standard12'){
                value.find("span").html("Select Sponsors");
              }else if(ccvj == 'standard13'){
                value.find("span").html("Select Industry");
              }else if(ccvj == 'standard14'){
                value.find("span").html("Select Portfolio Companies");
              }else if(ccvj == 'standard21'){
                value.find("span").html("Select Target Market");
              }else if(ccvj == 'standard22'){
                value.find("span").html("Select Interest Keywords");
              }else if(ccvj == 'standard31'){
                value.find("span").html("Select Keywords");
              }else{
                value.find("span").html('Select Keyword.');
            ...
            _onchange: function() { 
              $select.val($(this).val());
              methods.select($(this).val()); //alert($select.val());
              var ccc =$(this).closest('select').attr('id');
              //alert(ccc);
              var vjjj='';
              if(ccc =='standard2'){var vjjj='2'; }
              if(ccc =='standard1'){var vjjj='1'; }
              if(ccc =='standard11'){var vjjj='11'; }
              if(ccc =='standard12'){var vjjj='12'; }
              if(ccc =='standard13'){var vjjj='13'; }
              if(ccc =='standard14'){var vjjj='14'; }
              if(ccc =='standard21'){var vjjj='21'; }
              if(ccc =='standard22'){var vjjj='22'; }
              if(ccc =='standard31'){var vjjj='31'; }

              if ($('#selectedResult'+vjjj+' #sel_'+$select.val()).length)
              {
                  //alert('already');
              }else{
                  var option  = $select.find("option:selected");
                  if($select.val() != ''){
                    $('#selectedResult'+vjjj+' ul').append('<li id="sel_'+$select.val()+'"><a onClick="removeSelection'+vjjj+'('+$select.val()+')" href="javascript:void(0)">'+option.text()+'<i class="fa fa-close"></i></a></li>'); 
                    $('#hiddenKey'+vjjj+'').append('<option id="sel_'+$select.val()+'" value="'+$select.val()+'" selected="selected">'+option.text()+'</option>');
                  }  
              }
            }
          };
        ...
        select: function(value, disabled) { 
          if(!disabled) {
            ...
            var ccv =$select.closest('select').attr('id');
           
            var option  = $select.find("option:selected");
            var link    = $this.find("a");
            //link.find("span").text(option.text().length > 0 ? option.text() : $options.emptytext);
            if(ccv == 'standard'){
              link.find("span").text('Select Target Market');
            }else if(ccv == 'standard2'){
              link.find("span").text('Select Campaign Keyword');
            }else if(ccv == 'standard11'){
                link.find("span").text("Select Fund's Managers");
            }else if(ccv == 'standard12'){
                link.find("span").text("Select Sponsors");
            }else if(ccv == 'standard13'){
                link.find("span").text("Select Industry");
            }else if(ccv == 'standard14'){
                link.find("span").text("Select Portfolio Companies");
            }else if(ccv == 'standard21'){
                link.find("span").text("Select Target Market");
            }else if(ccv == 'standard22'){
                link.find("span").text("Select Interest Keywords");
            }else if(ccv == 'standard31'){
                link.find("span").text("Select Keywords");
            }else{
              link.find("span").text('Select Keyword');
            }
