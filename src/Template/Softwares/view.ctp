  ...
  <div class="row">
    <div class="col-lg-12 ">
      <ol class="breadcrumb">
        ...
        ?></li>
        <li class="active">View Software</li>
      </ol>
    </div>
  </div>
  <!-- breadcrumb ends --> 
    ...
    <div class='col-lg-4 col-md-4 col-sm-4 '>
      <div class="form-group">
        <span class="contentListingLikes">
          <h2 class="likes">
            <?php 
              $likeC = $this->Custom->getSoftwareLikeCount($fundDeatils->id);
              if(!empty($likeC)){

                  echo $this->Html->link($likeC, array('controller'=>'Softwares','action'=>'likeList',base64_encode($fundDeatils->id)), array('escape'=>false,'title'=>'View','style'=>'color: rgb(3, 66, 110);'));
            ...
            &nbsp;
            <?php 
            if($this->Custom->isSoftwareLikedbyUser($fundDeatils->id,$userId)){
            ...
            &nbsp;
            <?php 
              $disLikeC = $this->Custom->getSoftwareDislikeCount($fundDeatils->id);
              if(!empty($disLikeC)){

                  echo $this->Html->link($disLikeC, array('controller'=>'Softwares','action'=>'dislikeList',base64_encode($fundDeatils->id)), array('escape'=>false,'title'=>'View','style'=>'color: rgb(3, 66, 110);'));
            ...
            &nbsp;
            <?php 
              if($this->Custom->isSoftwareDislikedbyUser($fundDeatils->id,$userId)){
                ...
                if(!empty($skey5[0])){
                  for($ij=0; $ij<$cskey5; $ij++){
                      $slids5=$skey5[$ij];
                         ...
                         echo 'NO Software Keywords Added';
                }
          ?>
          </ul>
        </span>
                  ...
                  <div class="panel-body">
                    <div class="image-Holder">
                      <?php if(!empty($fundDeatils->image)){?>
                        <img src="<?php echo $this->request->webroot.'img/software/'.$fundDeatils->image;?>" alt="Image">
                      <?php } ?>
                    </div>
                    </div>
                </div>
                   ...
                   <ul class="listing anchor">
                    <?php 
                        if(!empty($fundDeatils->document)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/software/'.$fundDeatils->document.'" target="_blank"> Document <i class="fa fa-eye"></i></a>';
                   ...
                   <ul class="listing anchor">
                     <?php 
                        if(!empty($fundDeatils->audio)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/software/'.$fundDeatils->audio.'" target="_blank"> Audio <i class="fa fa-eye"></i></a>';
                   ...
                   <ul class="listing anchor">
                      <?php 
                        if(!empty($fundDeatils->video)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/software/'.$fundDeatils->video.'" target="_blank"> Video <i class="fa fa-eye"></i></a>';
            ...
            if($userId != $fundDeatils->user_id){
              ...
              if(!empty($is_follwed_by_user)){
                  ...
                  $titleFollow='Unfollow';
                    $followLink= $this->Url->build(["controller" => "Softwares","action" => "unfollow",base64_encode($fundDeatils->id),base64_encode($userId)]);                      
              }else {
                $class1= 'blueBtn';
                $titleFollow='Follow';
                    $followLink= $this->Url->build(["controller" => "Softwares","action" => "follow",base64_encode($fundDeatils->id),base64_encode($userId)]);   
              }
              ...
              if(!empty($is_commited_by_user)){
                  ...
                  $titleCommit='Thank you. You have registered your interest.';
                    $CommitLink= $this->Url->build(["controller" => "Softwares","action" => "uncommit",base64_encode($fundDeatils->id),base64_encode($userId)]);                      
              }else {
                $class= 'blueBtn';
                $titleCommit='Click here to register your interest.';
                    $CommitLink= $this->Url->build(["controller" => "Softwares","action" => "commit",base64_encode($fundDeatils->id),base64_encode($userId)]);   
              }
