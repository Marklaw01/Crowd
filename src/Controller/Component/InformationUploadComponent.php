...
class InformationUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('InformationInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('InformationKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('InformationTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('InformationFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['information_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('InformationCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['information_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('InformationCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['information_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/information/"; // Upload directory
                $imgCamp='';
                $errors='';
