...
class GroupUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('GroupInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('GroupKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('GroupTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('GroupFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['group_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('GroupCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['group_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('GroupCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['group_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/group/"; // Upload directory
                $imgCamp='';
                $errors='';
