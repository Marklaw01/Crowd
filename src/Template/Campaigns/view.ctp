          ...
          <div class="row">
            <div class="col-lg-12 ">
              <ol class="breadcrumb">
                ...
                ?></li>
                <li><?php
                    echo $this->Html->link('Campaigns', array('controller'=>'Campaigns','action'=>'recommended'), array('escape'=>false));
                ?></li>
          ...
          <div class='row'>
            <div class='col-lg-12 col-md-12 col-sm-12 '>
             <div class="page_heading">
               <h1><?= h($campaign->campaigns_name) ?></h1> 
              </div>
            </div>
             ...
             <?php } ?> 
              ...
              <div class="col-lg-12 col-md-12 col-sm-12 ">
               <div class="form-group pull-right">
                <?php 
                    if($userId != $campaign->user_id){
                      // Commit and Uncommit button
                      if(empty($campaignDnationC)){
                            $titleComiit='Commit';
                            $comittLink= $this->Url->build(["controller" => "CampaignDonations","action" => "add",base64_encode($campaign->id)]);
                            // echo $this->Html->link(__('Commit'), ['controller' => 'CampaignDonations','action' => 'add',base64_encode($campaign->id)]) ;
                      }else{
                            $titleComiit='Uncommit';
                            $comittLink= $this->Url->build(["controller" => "CampaignDonations","action" => "uncommit",base64_encode($dId),base64_encode($campaign->id)]);
                              //echo $this->Html->link(__('Uncommit'), ['controller' => 'CampaignDonations','action' => 'uncommit',base64_encode($dId),base64_encode($campaign->id)]) ;
                      }

                      // Follow and unfollow button
                      if(!empty($campaignFollow)){
                          $titleFollow='Unfollow';
                            $followLink= $this->Url->build(["controller" => "Campaigns","action" => "unfollowCampaign",base64_encode($campaign->id)]);                      
                      }else {
                        $titleFollow='Follow';
                            $followLink= $this->Url->build(["controller" => "Campaigns","action" => "followCampaign",base64_encode($campaign->id)]);   
                      }

                ?>
                 <button class="customBtn blueBtn" type="button" onclick="location.href='<?php echo $comittLink;?>'"><?php echo $titleComiit;?></button>

                 <button class="customBtn greenBtn" type="button" onclick="location.href='<?php echo $followLink;?>'"><?php echo $titleFollow;?></button>
                <?php } ?> 
              </div>
              </div>
          </div>
