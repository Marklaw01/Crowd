...
class MeetupUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('MeetupInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('MeetupKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('MeetupTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('MeetupFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['meetup_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('MeetupCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['meetup_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('MeetupCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['meetup_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/meetup/"; // Upload directory
                $imgCamp='';
                $errors='';
