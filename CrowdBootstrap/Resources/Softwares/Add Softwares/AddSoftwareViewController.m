...
#import "AddSoftwareViewController.h"
...
@interface AddSoftwareViewController ()

@end

@implementation AddSoftwareViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Add Software";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Software Title", @"Software Description", @"Software Availability Start Date", @"Software Availability End Date", @"Target Market", @"Software Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kSoftwareAPI_Software_Title, kSoftwareAPI_Description, kSoftwareAPI_StartDate, kSoftwareAPI_EndDate, kAddSoftwareAPI_Target_Market_Keywords, kAddSoftwareAPI_Keywords, kAddSoftwareAPI_IndustryKeywords, kSoftwareAPI_Software_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kSoftwareAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kSoftwareAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kSoftwareAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createSoftware {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addSoftwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddSoftware_SuccessMessage withResultType:@"0" withDuration:1] ;
