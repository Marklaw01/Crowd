...
-(void) awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(SetViewEditingNotification:) name:kNotificationSetInformationViewEditing
                                               object:nil];
...
-(void)SetViewEditingNotification:(NSNotification *)notification {
    if ([[notification name] isEqualToString:kNotificationSetInformationViewEditing]) {
...
-(void)resetUISettings {
    ...
    informationData = [[UtilityClass getInformationDetails] mutableCopy];
    NSLog(@"Information Data: %@",informationData) ;
...
//    }
    ...
    self.title = [NSString stringWithFormat:@"%@",[informationData valueForKey:kInformationAPI_Title]] ;
    [self getInformationDetails] ;
...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Created By", @"Information Title", @"Information Description", @"Information Availability Start Date", @"Information Availability End Date", @"Target Market", @"Information Keywords", @"Interest Keywords", @"Upload Image", @"View Document",@"Play Audio",@"Play Video"] ;
    NSArray *parametersArray = @[kInformationAPI_CreatedBy, kInformationAPI_Title, kInformationAPI_Description, kInformationAPI_StartDate, kInformationAPI_EndDate, kAddInformationAPI_Target_Market_Keywords, kAddInformationAPI_Keywords, kAddInformationAPI_IndustryKeywords, kInformationAPI_Image, kInformationAPI_Document, kInformationAPI_Audio, kInformationAPI_Video] ;
    ...
    [self getInformationKeywordsList];
    [self getInformationIndustryKeywordsList];
...
- (void)checkKeywordsListFound:(int)keywordType {
    switch (keywordType) {
        ...
        case 6:
        {
            if (informationKeywordsArray.count > 0)
                [self openTagsPopup] ;
            else
                [self presentViewController:[UtilityClass displayAlertMessage:kNoInformationKeywordsFoundMessage] animated:YES completion:nil];
        }
            break;
        default:
            break;
...
-(NSString*)convertTagsArrayToStringforArray:(NSMutableArray*)array withTagsArray:(NSArray*)tagsArray tagType:(NSString *)type {
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kInformationAPI_Target_Market_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kInformationAPI_Target_Market_ID])
                    selectedValue = [obj valueForKey:kInformationAPI_Target_Market_ID] ;
                else
                    selectedValue = [obj valueForKey:kInformationAPI_Target_Market_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag)
                        tagsStr = [dict valueForKey:kInformationAPI_Target_Market_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kInformationAPI_Target_Market_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
    } else if ([type isEqualToString:@"informationkeyword"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kInformationAPI_KeywordID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kInformationAPI_KeywordID])
                    selectedValue = [obj valueForKey:kInformationAPI_KeywordID] ;
                else
                    selectedValue = [obj valueForKey:kInformationAPI_KeywordID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kInformationAPI_KeywordID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kInformationAPI_KeywordID]] ;
                    isFirstTag = NO ;
                }
            }
        }
        ...
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kInformationAPI_Industry_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kInformationAPI_Industry_ID])
                    selectedValue = [obj valueForKey:kInformationAPI_Industry_ID] ;
                else
                    selectedValue = [obj valueForKey:kInformationAPI_Industry_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kInformationAPI_Industry_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kInformationAPI_Industry_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
...
- (IBAction)CheckUncheck_ClickAction:(id)sender {
    ...
    if([btn.accessibilityLabel isEqualToString:CHECK_IMAGE ]) { // Check
        ...
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            if(selectedKeywordType == INFORMATION_INDUSTRY_KEYWORDS_SELECTED)
                dictKeyword = [industryKeywordsArray objectAtIndex:btn.tag];
            else if (selectedKeywordType == INFORMATION_KEYWORDS_SELECTED)
                dictKeyword = [informationKeywordsArray objectAtIndex:btn.tag];
            else
    ...
    if(selectedKeywordType == INFORMATION_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (NSMutableDictionary *obj in industryKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedIndustryKeywordNames addObject:[obj valueForKey:kInformationAPI_Industry_Name]] ;
                [selectedIndustryKeywordIds addObject:[obj valueForKey:kInformationAPI_Industry_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:INFORMATION_INDUSTRY_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        
    } else if(selectedKeywordType == INFORMATION_KEYWORDS_SELECTED) {
        [selectedInformationKeywordsArray removeAllObjects];
        [selectedInformationKeywordNames removeAllObjects];
        [selectedInformationKeywordIds removeAllObjects];
        
        for (NSMutableDictionary *obj in informationKeywordsArray) {
            NSString *isSelectedStr = [obj valueForKey:@"isSelected"] ;
            if([isSelectedStr isEqualToString:@"1"]) {
                [selectedInformationKeywordsArray addObject:obj] ;
                [selectedInformationKeywordNames addObject:[obj valueForKey:kInformationAPI_KeywordName]] ;
                [selectedInformationKeywordIds addObject:[obj valueForKey:kInformationAPI_KeywordID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:INFORMATION_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        ...
        for (NSMutableDictionary *obj in targetMarketKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedTargetMarketKeywordNames addObject:[obj valueForKey:kInformationAPI_Target_Market_Name]] ;
                [selectedTargetMarketKeywordIds addObject:[obj valueForKey:kInformationAPI_Target_Market_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:INFORMATION_TARGET_MARKET_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
    }
}
...
-(void)tagsControl:(UIView *)tag withTagControlIndex:(NSInteger)tagControlIndex removedFromIndex:(NSInteger)index {
    ...
    if(tagControlIndex == INFORMATION_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (int i=0; i<industryKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[industryKeywordsArray objectAtIndex:i] valueForKey:kInformationAPI_Industry_Name]] ;
    ...
    } else if(tagControlIndex == INFORMATION_KEYWORDS_SELECTED) {
        NSString *selectedName = [NSString stringWithFormat:@"%@",[selectedInformationKeywordNames objectAtIndex:index]] ;
        for (int i=0; i<informationKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[informationKeywordsArray objectAtIndex:i] valueForKey:kInformationAPI_KeywordName]] ;
            if([keywordName isEqualToString:selectedName]){
                [[informationKeywordsArray objectAtIndex:i] setValue:@"0" forKey:@"isSelected"] ;
                break ;
            }
        }
        [selectedInformationKeywordsArray removeObjectAtIndex:index] ;
        [selectedInformationKeywordNames removeObjectAtIndex:index];
        [selectedInformationKeywordIds removeObjectAtIndex:index];
        ...
        for (int i=0; i<targetMarketKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[targetMarketKeywordsArray objectAtIndex:i] valueForKey:kInformationAPI_Target_Market_Name]] ;
            if([keywordName isEqualToString:selectedName]){
...
- (IBAction)Back_Click:(id)sender {
    [UtilityClass setComingFrom_Information_AddEditScreen:YES];
    [self.navigationController popViewControllerAnimated:YES] ;
}

- (IBAction)CloseData_ClickAction:(id)sender {
    [informationDataView dismissPresentingPopup] ;
}
...
- (IBAction)followButton_ClickAction:(UIButton*)button {
    ...
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kInformationAPI_FollowedBy] ;
        [dictParam setObject:[[[UtilityClass getInformationDetails] mutableCopy] valueForKey:kInformationAPI_ID] forKey:kInformationAPI_InformationID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isFollowed == 0) {
            [ApiCrowdBootstrap followInformationWithParameters:dictParam success:^(NSDictionary *responseDict) {
                ...
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    ...
                    [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:INFORMATION_FOLLOW_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationNone] ;
                }
            ...
            [ApiCrowdBootstrap unfollowInformationWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                    if(isFollowed == 1)
...
- (IBAction)Calender_ClickAction:(id)sender {
    ...
    if ([sender tag] == INFORMATION_START_DATE_SECTION_INDEX)
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:INFORMATION_START_DATE_SECTION_INDEX]] ;
    else
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:INFORMATION_END_DATE_SECTION_INDEX]] ;
...
- (IBAction)Submit_ClickAction:(id)sender {
    if ((selectedSegmentControl == 100 && selectedSegment == 0)) {
        ...
        if(![self validatetextFieldsWithSectionIndex:INFORMATION_TITLE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:INFORMATION_START_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:INFORMATION_END_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:INFORMATION_DESCRIPTION_SECTION_INDEX]) return ;
        ...
        else if(selectedInformationKeywordsArray.count < 1) {
        ...
        else [self editInformation] ;
    }
}

- (IBAction)informationCommitmentList_ClickAction:(id)sender {
    ...
    if ([noOfcommit integerValue] > 0) {
        [self getInformationCommitmentList];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    GraphicTableViewCell *cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:INFORMATION_IMAGE_SECTION_INDEX]] ;
...
- (IBAction)DatePickerToolbarButtons_ClickAction:(id)sender {
    ...
    if([sender tag] == DONE_CLICKED) {
        if(selectedDatePickerType == INFORMATION_START_DATE_SELECTED) {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:INFORMATION_START_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            cell.textFld.text = [dateFormatter stringFromDate:datePickerView.date];
            [[sectionsArray objectAtIndex:INFORMATION_START_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
        else {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:INFORMATION_END_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            ...
            [[sectionsArray objectAtIndex:INFORMATION_END_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
    }
    else {
        if(selectedDatePickerType == INFORMATION_START_DATE_SELECTED) {
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    switch (selectedKeywordType) {
        case 5: // Target Keywords
        {
            ...
            break;
        case 6: // Information Keywords
        {
            for(NSDictionary *keywordDict in informationKeywordsArray)
            {
                NSString *keywordName = [keywordDict objectForKey:@"name"];
                NSRange range = [keywordName rangeOfString:searchText options:NSCaseInsensitiveSearch];
                if(range.location != NSNotFound)
            ...
            NSLog(@"Information Results: %@",searchResultsForKeywords);
        }
            break;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    DobTableViewCell *startDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:INFORMATION_START_DATE_SECTION_INDEX]] ;
    
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        textField.userInteractionEnabled = true;
        if(textField.tag == INFORMATION_START_DATE_SECTION_INDEX || textField.tag == INFORMATION_END_DATE_SECTION_INDEX) {
            if(textField.tag == INFORMATION_START_DATE_SECTION_INDEX) {
                selectedDatePickerType = INFORMATION_START_DATE_SELECTED;
            }
            else {
                selectedDatePickerType = INFORMATION_END_DATE_SELECTED;
...
-(void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if(textField.tag == INFORMATION_START_DATE_SECTION_INDEX || textField.tag == INFORMATION_END_DATE_SECTION_INDEX) {
            if(textField.tag == INFORMATION_START_DATE_SECTION_INDEX) {
                DobTableViewCell *endDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:INFORMATION_END_DATE_SECTION_INDEX]] ;
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@"Information Description"] && textView.textColor == [UIColor lightGrayColor]){
            textView.text = @"" ;
...
-(BOOL)textViewShouldEndEditing:(UITextView *)textView {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@""]){
            textView.text = @"Information Description" ;
    ...
    if (indexPath.row == 0) {
        ...
        for (int i = 0; i < [sectionsArray count]; i++) {
            if (indexPath.section == i) {
                if (indexPath.section == INFORMATION_DOCUMENT_SECTION_INDEX) {
                    if (![docuementFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == INFORMATION_AUDIO_SECTION_INDEX) {
                    if (![audioFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == INFORMATION_VIDEO_SECTION_INDEX) {
...
-(void)getInformationKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getInformationKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [informationKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kInformationAPI_KeywordList]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kInformationAPI_KeywordList] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
                        [informationKeywordsArray addObject:obj] ;
                    }
                }
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode) {
...
-(void)getInformationIndustryKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getInformationIndustryKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [industryKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kInformationAPI_Industry_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kInformationAPI_Industry_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getTargetMarketKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getInformationTargetMarketKeywordsListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [targetMarketKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kInformationAPI_Target_Market_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kInformationAPI_Target_Market_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getInformationDetails {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kInformationAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[informationData valueForKey:kInformationAPI_ID]] forKey:kInformationAPI_InformationID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap viewInformationWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"respos %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                for ( int i = 0; i < sectionsArray.count ; i++ ) {
                    NSString *key = [[sectionsArray objectAtIndex:i] valueForKey:@"key"] ;
                    if(i != INFORMATION_INDUSTRY_KEYWORDS_SECTION_INDEX && i != INFORMATION_TARGET_MARKET_KEYWORDS_SECTION_INDEX && i != INFORMATION_KEYWORDS_SECTION_INDEX) {
                ...
                // Keywords Array
                if([responseDict objectForKey:kAddInformationAPI_IndustryKeywords]) {
                    selectedIndustryKeywordsArray = [[responseDict objectForKey:kAddInformationAPI_IndustryKeywords] mutableCopy];
                ...
                if([responseDict objectForKey:kAddInformationAPI_Keywords]) {
                    selectedInformationKeywordsArray = [[responseDict objectForKey:kAddInformationAPI_Keywords] mutableCopy] ;
                    selectedInformationKeywordNames = [self resetTagsArrayWithData:selectedInformationKeywordsArray];
                    selectedInformationKeywordIds = [self resetTagIdsArrayWithData:selectedInformationKeywordsArray];
                }
                if([responseDict objectForKey:kAddInformationAPI_Target_Market_Keywords]) {
                    selectedTargetMarketKeywordsArray = [[responseDict objectForKey:kAddInformationAPI_Target_Market_Keywords] mutableCopy];
                ...
                // Commits
                noOfcommit = [[responseDict valueForKey:kInformationAPI_NoOfCommits] stringValue];
                isCommited = [[responseDict valueForKey:kInformationAPI_IsCommited] intValue];
                isLiked = [[responseDict valueForKey:kInformationAPI_IsLiked] intValue];
                isFollowed = [[responseDict valueForKey:kInformationAPI_IsFollowed] intValue];

                // Document
                if([responseDict objectForKey:kInformationAPI_Document])
                    docuementFile = [[responseDict objectForKey:kInformationAPI_Document] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Audio
                if([responseDict objectForKey:kInformationAPI_Audio])
                    audioFile = [[responseDict objectForKey:kInformationAPI_Audio] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Video
                if([responseDict objectForKey:kInformationAPI_Video])
                    videoFile = [[responseDict objectForKey:kInformationAPI_Video] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Image
                if([responseDict objectForKey:kInformationAPI_Image]) {
                    NSString *strImage = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed, [responseDict objectForKey:kInformationAPI_Image]];
                    NSURL *url = [NSURL URLWithString:[strImage stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                    imgData = [NSData dataWithContentsOfURL:url];
                }
                
                // Refresh Dict
                [informationData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kInformationAPI_Title]] forKey:kInformationAPI_Title] ;
                [informationData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kInformationAPI_CreatedBy]] forKey:kInformationAPI_CreatedBy] ;
                [informationData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kInformationAPI_StartDate]] forKey:kInformationAPI_StartDate] ;
                [informationData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kInformationAPI_EndDate]] forKey:kInformationAPI_EndDate] ;
                [informationData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kInformationAPI_Description]] forKey:kInformationAPI_Description] ;
                
                [UtilityClass setInformationDetails:[informationData mutableCopy]] ;
...
-(void)commitInformation {
    if([UtilityClass checkInternetConnection]) {
        ...
        if (isCommited == 0) {
            [ApiCrowdBootstrap commitInformationWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
            ...
            [ApiCrowdBootstrap uncommitInformationWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
...
-(void)getInformationCommitmentList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getInformationCommitmentListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode)  {
...
- (void)getRecommendedUserList: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:@"information" forKey:@"type"] ;
        ...
        [ApiCrowdBootstrap getRegisteredRoleListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kInformationAPI_UserList]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]){
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kInformationAPI_UserList]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kInformationAPI_UserList]] ;
                    }
            ...
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
            {
                ...
                if(![searchBarUsers.text isEqualToString:@""])
                    ...
                    usersArray = [NSMutableArray arrayWithArray:[responseDict valueForKey:kInformationAPI_UserList]] ;
...
-(void)editInformation {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[[sectionsArray objectAtIndex:INFORMATION_TITLE_SECTION_INDEX] valueForKey:@"value"] forKey:kInformationAPI_Title] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:INFORMATION_DESCRIPTION_SECTION_INDEX] valueForKey:@"value"] forKey:kInformationAPI_Description] ;
        
        [dictParam setObject:[[sectionsArray objectAtIndex:INFORMATION_START_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kInformationAPI_StartDate] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:INFORMATION_END_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kInformationAPI_EndDate] ;
        
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedIndustryKeywordsArray withTagsArray:industryKeywordsArray tagType:@"industry"] forKey:kAddInformationAPI_IndustryKeywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedInformationKeywordsArray withTagsArray:informationKeywordsArray tagType:@"informationkeyword"] forKey:kAddInformationAPI_Keywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedTargetMarketKeywordsArray withTagsArray:targetMarketKeywordsArray tagType:@"targetmarket"] forKey:kAddInformationAPI_Target_Market_Keywords] ;
        
        if(imgData)
            [dictParam setObject:imgData forKey:kInformationAPI_Image] ;
        else
            [dictParam setObject:@"" forKey:kInformationAPI_Image] ;
        
        [dictParam setObject:@"" forKey:kInformationAPI_Document] ;
        [dictParam setObject:@"" forKey:kInformationAPI_Audio] ;
        [dictParam setObject:@"" forKey:kInformationAPI_Video] ;
        ...
        [ApiCrowdBootstrap editInformationWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kEditInformation_SuccessMessage withResultType:@"0" withDuration:1] ;
                [UtilityClass setComingFrom_Information_AddEditScreen:YES];
                [self.navigationController popViewControllerAnimated:YES] ;
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == popupTblView) {
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            else if (selectedKeywordType == INFORMATION_TARGET_MARKET_KEYWORDS_SELECTED)
                return targetMarketKeywordsArray.count ;
            else
                return informationKeywordsArray.count ;
        }
        ...
        if(section <= INFORMATION_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)
            return 1;
        else {
            if(section == INFORMATION_IMAGE_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue])
                    return 1;
                else return 0 ;
            }
            else if(section == INFORMATION_DOCUMENT_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == INFORMATION_AUDIO_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(selectedKeywordType == INFORMATION_INDUSTRY_KEYWORDS_SELECTED) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kInformationAPI_Industry_Name] ;
            if ([selectedIndustryKeywordIds containsObject:[dictKeyword valueForKey:kInformationAPI_Industry_ID]]) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kInformationAPI_Target_Market_Name] ;
            
            if ([selectedTargetMarketKeywordIds containsObject:[dictKeyword valueForKey:kInformationAPI_Target_Market_ID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        } else
        {
            if (![searchBarKeywords.text isEqualToString:@""])
                ...
                dictKeyword = [informationKeywordsArray objectAtIndex:indexPath.row];
            
            cell.companyNameLbl.text = [dictKeyword valueForKey:kInformationAPI_KeywordName] ;
            
            if ([selectedInformationKeywordIds containsObject:[dictKeyword valueForKey:kInformationAPI_KeywordID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        if (![searchBarUsers.text isEqualToString:@""]) {
            ...
            cell.lblName.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kInformationAPI_User_Name]] ;
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kInformationAPI_User_Desc]];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kInformationAPI_User_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
        ...
        if(indexPath.section == sectionsArray.count) {
        ...
        else if(indexPath.section == INFORMATION_FOLLOW_SECTION_INDEX) {
        ...
        else if(indexPath.section == INFORMATION_INDUSTRY_KEYWORDS_SECTION_INDEX || indexPath.section == INFORMATION_TARGET_MARKET_KEYWORDS_SECTION_INDEX || indexPath.section == INFORMATION_KEYWORDS_SECTION_INDEX) {
            ...
            NSMutableArray *tags ;
            if(indexPath.section == INFORMATION_INDUSTRY_KEYWORDS_SECTION_INDEX)
                tags = [selectedIndustryKeywordNames mutableCopy] ;
            else if(indexPath.section == INFORMATION_TARGET_MARKET_KEYWORDS_SECTION_INDEX)
                tags = [selectedTargetMarketKeywordNames mutableCopy] ;
            else
                tags = [selectedInformationKeywordNames mutableCopy] ;
        ...
        else if(indexPath.section == INFORMATION_DESCRIPTION_SECTION_INDEX) {
        ...
        else if(indexPath.section == INFORMATION_START_DATE_SECTION_INDEX || indexPath.section == INFORMATION_END_DATE_SECTION_INDEX) {
        ...
        else if(indexPath.section == INFORMATION_IMAGE_SECTION_INDEX) {
        ...
        else if (indexPath.section == INFORMATION_DOCUMENT_SECTION_INDEX || indexPath.section == INFORMATION_AUDIO_SECTION_INDEX || indexPath.section == INFORMATION_VIDEO_SECTION_INDEX) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(indexPath.section >= INFORMATION_DOCUMENT_SECTION_INDEX && indexPath.section < sectionsArray.count) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if(indexPath.section == INFORMATION_START_DATE_SECTION_INDEX || indexPath.section == INFORMATION_END_DATE_SECTION_INDEX) {
            return 65;
        }
        else if (indexPath.section == INFORMATION_FOLLOW_SECTION_INDEX)
            return 40;
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if(section <= INFORMATION_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)return 0;
    else return 45 ;
}
