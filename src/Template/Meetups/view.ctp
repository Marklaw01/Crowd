<div class="container-fluid">
  <div class="row">
    <div class="col-lg-12 ">
      <ol class="breadcrumb">
        ...
        <li class="active">View Meet Up</li>
      </ol>
    </div>
  </div>
  <!-- breadcrumb ends --> 
    ...
    <div class='col-lg-4 col-md-4 col-sm-4 '>
      <div class="form-group">
        <span class="contentListingLikes">
          <h2 class="likes">
            <?php 
              $likeC = $this->Custom->getMeetupLikeCount($fundDeatils->id);
              if(!empty($likeC)){

                  echo $this->Html->link($likeC, array('controller'=>'Meetups','action'=>'likeList',base64_encode($fundDeatils->id)), array('escape'=>false,'title'=>'View','style'=>'color: rgb(3, 66, 110);'));
            ...
            &nbsp;
            <?php 
            if($this->Custom->isMeetupLikedbyUser($fundDeatils->id,$userId)){
            ...
            &nbsp;
            <?php 
              $disLikeC = $this->Custom->getMeetupDislikeCount($fundDeatils->id);
              if(!empty($disLikeC)){

                  echo $this->Html->link($disLikeC, array('controller'=>'Meetups','action'=>'dislikeList',base64_encode($fundDeatils->id)), array('escape'=>false,'title'=>'View','style'=>'color: rgb(3, 66, 110);'));
            ...
            &nbsp;
            <?php 
              if($this->Custom->isMeetupDislikedbyUser($fundDeatils->id,$userId)){
  ...
  <div class="row">
    <div class="col-lg-6 col-md-6 col-sm-12 ">
      ...
      <div class="form-group">
          <label>Forum</label>
          <?php $forum = $forum['title']!=""?$forum["title"]:'No forum selected'; ?>
          <span class="form-control"><?= h($forum) ?></span>
          
      </div>

      <div class="form-group">
          <label>Meet Up Access</label>
          <?php $access_level = array('1' => 'Group' , '2' => 'Connections', '3' =>'public' );
          foreach ($access_level as $key => $value) { 
            if($key == $fundDeatils->access_level ){
              $access_level = $value; 
              break;
            }else{
              $access_level = "No option selected";
            }
          } 
          ?>
          <span class="form-control"><?= h($access_level) ?></span>
          
      </div>

      <div class="form-group">
          <label>Meet Up Notification</label>
          <?php $notification = array('1' => 'Group' , '2' => 'Connections');
                foreach ($notification as $key => $value) {
                  
                  if($key == $fundDeatils->send_notifications ){
                    $sendNotification = $value;
                    break;
                  }else{
                     $sendNotification = "No option selected"; 
                  }
                }
           ?>
          <span class="form-control"><?= h($sendNotification) ?></span>
                ...
                if(!empty($skey5[0])){
                  for($ij=0; $ij<$cskey5; $ij++){
                      $slids5=$skey5[$ij];
                         ...
                         echo 'No Meet Up Keywords Added';
                }
          ?>
          </ul>
        </span>
                  ...
                  <div class="panel-body">
                    <div class="image-Holder">
                      <?php if(!empty($fundDeatils->image)){?>
                        <img src="<?php echo $this->request->webroot.'img/meetup/'.$fundDeatils->image;?>" alt="Image">
                      <?php } ?>
                    </div>
                    </div>
                </div>
                   ...
                   <ul class="listing anchor">
                    <?php 
                        if(!empty($fundDeatils->document)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/meetup/'.$fundDeatils->document.'" target="_blank"> Document <i class="fa fa-eye"></i></a>';
                   ...
                   <ul class="listing anchor">
                     <?php 
                        if(!empty($fundDeatils->audio)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/meetup/'.$fundDeatils->audio.'" target="_blank"> Audio <i class="fa fa-eye"></i></a>';
                   ...
                   <ul class="listing anchor">
                      <?php 
                        if(!empty($fundDeatils->video)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/meetup/'.$fundDeatils->video.'" target="_blank"> Video <i class="fa fa-eye"></i></a>';
              ...
              if(!empty($is_follwed_by_user)){
                $class1= 'greenBtn';
                  $titleFollow='Unfollow';
                    $followLink= $this->Url->build(["controller" => "Meetups","action" => "unfollow",base64_encode($fundDeatils->id),base64_encode($userId)]);                      
              }else {
                $class1= 'blueBtn';
                $titleFollow='Follow';
                    $followLink= $this->Url->build(["controller" => "Meetups","action" => "follow",base64_encode($fundDeatils->id),base64_encode($userId)]);   
              }
              ...
              if(!empty($is_commited_by_user)){
                $class= 'greenBtn';
                  $titleCommit='Thank you. You have registered your interest.';
                    $CommitLink= $this->Url->build(["controller" => "Meetups","action" => "uncommit",base64_encode($fundDeatils->id),base64_encode($userId)]);                      
              }else {
                $class= 'blueBtn';
                $titleCommit='Click here to register your interest.';
                    $CommitLink= $this->Url->build(["controller" => "Meetups","action" => "commit",base64_encode($fundDeatils->id),base64_encode($userId)]);   
              }
