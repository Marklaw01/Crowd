...
#import "EditSoftwareViewController.h"
...
@interface EditSoftwareViewController ()

@end

@implementation EditSoftwareViewController
...
-(void) awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(SetViewEditingNotification:) name:kNotificationSetSoftwareViewEditing
                                               object:nil];
...
-(void)SetViewEditingNotification:(NSNotification *)notification {
    if ([[notification name] isEqualToString:kNotificationSetSoftwareViewEditing]) {
...
-(void)resetUISettings {
    ...
    softwareData = [[UtilityClass getSoftwareDetails] mutableCopy];
    NSLog(@"Software Data: %@",softwareData) ;
    ...
    self.title = [NSString stringWithFormat:@"%@",[softwareData valueForKey:kSoftwareAPI_Software_Title]] ;
    [self getSoftwareDetails] ;
...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Created By", @"Software Title", @"Software Description", @"Software Availability Start Date", @"Software Availability End Date", @"Target Market", @"Software Keywords", @"Interest Keywords", @"Upload Image", @"View Document",@"Play Audio",@"Play Video"] ;
    NSArray *parametersArray = @[kSoftwareAPI_CreatedBy, kSoftwareAPI_Software_Title, kSoftwareAPI_Description, kSoftwareAPI_StartDate, kSoftwareAPI_EndDate, kAddSoftwareAPI_Target_Market_Keywords, kAddSoftwareAPI_Keywords, kAddSoftwareAPI_IndustryKeywords, kSoftwareAPI_Software_Image, kSoftwareAPI_Software_Document, kSoftwareAPI_Software_Audio, kSoftwareAPI_Software_Video] ;
    ...
    [self getSoftwareKeywordsList];
    [self getSoftwareIndustryKeywordsList];
...
- (void)checkKeywordsListFound:(int)keywordType {
    switch (keywordType) {
        ...
        case 6:
        {
            if (softwareKeywordsArray.count > 0)
                [self openTagsPopup] ;
            else
                [self presentViewController:[UtilityClass displayAlertMessage:kNoSoftwareKeywordsFoundMessage] animated:YES completion:nil];
        }
            break;
        default:
            break;
...
-(NSString*)convertTagsArrayToStringforArray:(NSMutableArray*)array withTagsArray:(NSArray*)tagsArray tagType:(NSString *)type {
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kSoftwareAPI_Target_Market_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kSoftwareAPI_Target_Market_ID])
                    selectedValue = [obj valueForKey:kSoftwareAPI_Target_Market_ID] ;
                else
                    selectedValue = [obj valueForKey:kSoftwareAPI_Target_Market_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag)
                        tagsStr = [dict valueForKey:kSoftwareAPI_Target_Market_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kSoftwareAPI_Target_Market_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
    } else if ([type isEqualToString:@"softwarekeyword"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kSoftwareAPI_KeywordID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kSoftwareAPI_KeywordID])
                    selectedValue = [obj valueForKey:kSoftwareAPI_KeywordID] ;
                else
                    selectedValue = [obj valueForKey:kSoftwareAPI_KeywordID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kSoftwareAPI_KeywordID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kSoftwareAPI_KeywordID]] ;
                    isFirstTag = NO ;
                }
            }
        }
        ...
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kSoftwareAPI_Industry_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kSoftwareAPI_Industry_ID])
                    selectedValue = [obj valueForKey:kSoftwareAPI_Industry_ID] ;
                else
                    selectedValue = [obj valueForKey:kSoftwareAPI_Industry_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kSoftwareAPI_Industry_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kSoftwareAPI_Industry_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
...
- (IBAction)CheckUncheck_ClickAction:(id)sender {
    ...
    if([btn.accessibilityLabel isEqualToString:CHECK_IMAGE ]) { // Check
        ...
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            if(selectedKeywordType == SOFTWARE_INDUSTRY_KEYWORDS_SELECTED)
                dictKeyword = [industryKeywordsArray objectAtIndex:btn.tag];
            else if (selectedKeywordType == SOFTWARE_KEYWORDS_SELECTED)
                dictKeyword = [softwareKeywordsArray objectAtIndex:btn.tag];
            else
    ...
    if(selectedKeywordType == SOFTWARE_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (NSMutableDictionary *obj in industryKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedIndustryKeywordNames addObject:[obj valueForKey:kSoftwareAPI_Industry_Name]] ;
                [selectedIndustryKeywordIds addObject:[obj valueForKey:kSoftwareAPI_Industry_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_INDUSTRY_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        
    } else if(selectedKeywordType == SOFTWARE_KEYWORDS_SELECTED) {
        [selectedSoftwareKeywordsArray removeAllObjects];
        [selectedSoftwareKeywordNames removeAllObjects];
        [selectedSoftwareKeywordIds removeAllObjects];
        
        for (NSMutableDictionary *obj in softwareKeywordsArray) {
            NSString *isSelectedStr = [obj valueForKey:@"isSelected"] ;
            if([isSelectedStr isEqualToString:@"1"]) {
                [selectedSoftwareKeywordsArray addObject:obj] ;
                [selectedSoftwareKeywordNames addObject:[obj valueForKey:kSoftwareAPI_KeywordName]] ;
                [selectedSoftwareKeywordIds addObject:[obj valueForKey:kSoftwareAPI_KeywordID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        ...
        for (NSMutableDictionary *obj in targetMarketKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedTargetMarketKeywordNames addObject:[obj valueForKey:kSoftwareAPI_Target_Market_Name]] ;
                [selectedTargetMarketKeywordIds addObject:[obj valueForKey:kSoftwareAPI_Target_Market_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_TARGET_MARKET_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
    }
}
...
-(void)tagsControl:(UIView *)tag withTagControlIndex:(NSInteger)tagControlIndex removedFromIndex:(NSInteger)index {
    ...
    if(tagControlIndex == SOFTWARE_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (int i=0; i<industryKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[industryKeywordsArray objectAtIndex:i] valueForKey:kSoftwareAPI_Industry_Name]] ;
    ...
    } else if(tagControlIndex == SOFTWARE_KEYWORDS_SELECTED) {
        NSString *selectedName = [NSString stringWithFormat:@"%@",[selectedSoftwareKeywordNames objectAtIndex:index]] ;
        for (int i=0; i<softwareKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[softwareKeywordsArray objectAtIndex:i] valueForKey:kSoftwareAPI_KeywordName]] ;
            if([keywordName isEqualToString:selectedName]){
                [[softwareKeywordsArray objectAtIndex:i] setValue:@"0" forKey:@"isSelected"] ;
                break ;
            }
        }
        [selectedSoftwareKeywordsArray removeObjectAtIndex:index] ;
        [selectedSoftwareKeywordNames removeObjectAtIndex:index];
        [selectedSoftwareKeywordIds removeObjectAtIndex:index];
        ...
        for (int i=0; i<targetMarketKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[targetMarketKeywordsArray objectAtIndex:i] valueForKey:kSoftwareAPI_Target_Market_Name]] ;
            if([keywordName isEqualToString:selectedName]){
...
- (IBAction)Back_Click:(id)sender {
    [UtilityClass setComingFrom_Software_AddEditScreen:YES];
    [self.navigationController popViewControllerAnimated:YES] ;
}

- (IBAction)CloseData_ClickAction:(id)sender {
    [softwareDataView dismissPresentingPopup] ;
}
...
- (IBAction)followButton_ClickAction:(UIButton*)button {
    ...
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kSoftwareAPI_Software_FollowedBy] ;
        [dictParam setObject:[[[UtilityClass getSoftwareDetails] mutableCopy] valueForKey:kSoftwareAPI_ID] forKey:kSoftwareAPI_SoftwareID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isFollowed == 0) {
            [ApiCrowdBootstrap followSoftwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
                ...
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    ...
                    [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_FOLLOW_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationNone] ;
                }
            ...
            [ApiCrowdBootstrap unfollowSoftwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                    if(isFollowed == 1)
...
- (IBAction)Calender_ClickAction:(id)sender {
    ...
    if ([sender tag] == SOFTWARE_START_DATE_SECTION_INDEX)
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_START_DATE_SECTION_INDEX]] ;
    else
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_END_DATE_SECTION_INDEX]] ;
...
- (IBAction)Submit_ClickAction:(id)sender {
    if ((selectedSegmentControl == 100 && selectedSegment == 0)) {
        ...
        if(![self validatetextFieldsWithSectionIndex:SOFTWARE_TITLE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:SOFTWARE_START_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:SOFTWARE_END_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:SOFTWARE_DESCRIPTION_SECTION_INDEX]) return ;
        ...
        else if(selectedSoftwareKeywordsArray.count < 1) {
        ...
        else [self editSoftware] ;
    }
}

- (IBAction)softwareCommitmentList_ClickAction:(id)sender {
...
//    imgVwPeople.image = [UIImage imageNamed:@"like_green"];
    ...
    if ([noOfcommit integerValue] > 0) {
        [self getSoftwareCommitmentList];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    GraphicTableViewCell *cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_IMAGE_SECTION_INDEX]] ;
...
- (IBAction)DatePickerToolbarButtons_ClickAction:(id)sender {
    ...
    if([sender tag] == DONE_CLICKED) {
        if(selectedDatePickerType == SOFTWARE_START_DATE_SELECTED) {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_START_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            cell.textFld.text = [dateFormatter stringFromDate:datePickerView.date];
            [[sectionsArray objectAtIndex:SOFTWARE_START_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
        else {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_END_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            ...
            [[sectionsArray objectAtIndex:SOFTWARE_END_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
    }
    else {
        if(selectedDatePickerType == SOFTWARE_START_DATE_SELECTED) {
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    switch (selectedKeywordType) {
        case 5: // Target Keywords
        {
            ...
            break;
        case 6: // Software Keywords
        {
            for(NSDictionary *keywordDict in softwareKeywordsArray)
            {
                NSString *keywordName = [keywordDict objectForKey:@"name"];
                NSRange range = [keywordName rangeOfString:searchText options:NSCaseInsensitiveSearch];
                if(range.location != NSNotFound)
            ...
            NSLog(@"Software Results: %@",searchResultsForKeywords);
        }
            break;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    DobTableViewCell *startDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_START_DATE_SECTION_INDEX]] ;
    
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        textField.userInteractionEnabled = true;
        if(textField.tag == SOFTWARE_START_DATE_SECTION_INDEX || textField.tag == SOFTWARE_END_DATE_SECTION_INDEX) {
            if(textField.tag == SOFTWARE_START_DATE_SECTION_INDEX) {
                selectedDatePickerType = SOFTWARE_START_DATE_SELECTED;
            }
            else {
                selectedDatePickerType = SOFTWARE_END_DATE_SELECTED;
...
-(void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if(textField.tag == SOFTWARE_START_DATE_SECTION_INDEX || textField.tag == SOFTWARE_END_DATE_SECTION_INDEX) {
            if(textField.tag == SOFTWARE_START_DATE_SECTION_INDEX) {
                DobTableViewCell *endDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:SOFTWARE_END_DATE_SECTION_INDEX]] ;
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@"Software Description"] && textView.textColor == [UIColor lightGrayColor]){
            textView.text = @"" ;
...
-(BOOL)textViewShouldEndEditing:(UITextView *)textView {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@""]){
            textView.text = @"Software Description" ;
    ...
    if (indexPath.row == 0) {
        ...
        for (int i = 0; i < [sectionsArray count]; i++) {
            if (indexPath.section == i) {
                if (indexPath.section == SOFTWARE_DOCUMENT_SECTION_INDEX) {
                    if (![docuementFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == SOFTWARE_AUDIO_SECTION_INDEX) {
                    if (![audioFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == SOFTWARE_VIDEO_SECTION_INDEX) {
...
-(void)getSoftwareKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getSoftwareKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [softwareKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kSoftwareAPI_KeywordList]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kSoftwareAPI_KeywordList] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
                        [softwareKeywordsArray addObject:obj] ;
                    }
                }
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode) {
...
-(void)getSoftwareIndustryKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getSoftwareIndustryKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [industryKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kSoftwareAPI_Industry_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kSoftwareAPI_Industry_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getTargetMarketKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getSoftwareTargetMarketKeywordsListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [targetMarketKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kSoftwareAPI_Target_Market_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kSoftwareAPI_Target_Market_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getSoftwareDetails {
    if([UtilityClass checkInternetConnection]) {
        ...
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kSoftwareAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[softwareData valueForKey:kSoftwareAPI_ID]] forKey:kSoftwareAPI_SoftwareID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap viewSoftwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"respos %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                for ( int i = 0; i < sectionsArray.count ; i++ ) {
                    NSString *key = [[sectionsArray objectAtIndex:i] valueForKey:@"key"] ;
                    if(i != SOFTWARE_INDUSTRY_KEYWORDS_SECTION_INDEX && i != SOFTWARE_TARGET_MARKET_KEYWORDS_SECTION_INDEX && i != SOFTWARE_KEYWORDS_SECTION_INDEX) {
                ...
                // Keywords Array
                if([responseDict objectForKey:kAddSoftwareAPI_IndustryKeywords]) {
                    selectedIndustryKeywordsArray = [[responseDict objectForKey:kAddSoftwareAPI_IndustryKeywords] mutableCopy];
                ...
                if([responseDict objectForKey:kAddSoftwareAPI_Keywords]) {
                    selectedSoftwareKeywordsArray = [[responseDict objectForKey:kAddSoftwareAPI_Keywords] mutableCopy] ;
                    selectedSoftwareKeywordNames = [self resetTagsArrayWithData:selectedSoftwareKeywordsArray];
                    selectedSoftwareKeywordIds = [self resetTagIdsArrayWithData:selectedSoftwareKeywordsArray];
                }
                if([responseDict objectForKey:kAddSoftwareAPI_Target_Market_Keywords]) {
                    selectedTargetMarketKeywordsArray = [[responseDict objectForKey:kAddSoftwareAPI_Target_Market_Keywords] mutableCopy];
                ...
                // Commits
                noOfcommit = [[responseDict valueForKey:kSoftwareAPI_NoOfCommits] stringValue];
                isCommited = [[responseDict valueForKey:kSoftwareAPI_IsCommited] intValue];
                isLiked = [[responseDict valueForKey:kSoftwareAPI_IsLiked] intValue];
                isFollowed = [[responseDict valueForKey:kSoftwareAPI_IsFollowed] intValue];

                // Document
                if([responseDict objectForKey:kSoftwareAPI_Software_Document])
                    docuementFile = [[responseDict objectForKey:kSoftwareAPI_Software_Document] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Audio
                if([responseDict objectForKey:kSoftwareAPI_Software_Audio])
                    audioFile = [[responseDict objectForKey:kSoftwareAPI_Software_Audio] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Video
                if([responseDict objectForKey:kSoftwareAPI_Software_Video])
                    videoFile = [[responseDict objectForKey:kSoftwareAPI_Software_Video] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Image
                if([responseDict objectForKey:kSoftwareAPI_Software_Image]) {
                    NSString *strImage = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed, [responseDict objectForKey:kSoftwareAPI_Software_Image]];
                    NSURL *url = [NSURL URLWithString:[strImage stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                    imgData = [NSData dataWithContentsOfURL:url];
                }
                
                // Refresh Dict
                [softwareData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kSoftwareAPI_Software_Title]] forKey:kSoftwareAPI_Software_Title] ;
                [softwareData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kSoftwareAPI_CreatedBy]] forKey:kSoftwareAPI_CreatedBy] ;
                [softwareData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kSoftwareAPI_StartDate]] forKey:kSoftwareAPI_StartDate] ;
                [softwareData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kSoftwareAPI_EndDate]] forKey:kSoftwareAPI_EndDate] ;
                [softwareData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kSoftwareAPI_Description]] forKey:kSoftwareAPI_Description] ;
                
                [UtilityClass setSoftwareDetails:[softwareData mutableCopy]] ;
...
-(void)commitSoftware {
    if([UtilityClass checkInternetConnection]) {
        ...
        if (isCommited == 0) {
            [ApiCrowdBootstrap commitSoftwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
            ...
            [ApiCrowdBootstrap uncommitSoftwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
...
-(void)getSoftwareCommitmentList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getSoftwareCommitmentListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode)  {
...
- (void)getRecommendedUserList: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:@"software" forKey:@"type"] ;
        ...
        [ApiCrowdBootstrap getRegisteredRoleListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kSoftwareAPI_UserList]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]){
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kSoftwareAPI_UserList]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kSoftwareAPI_UserList]] ;
                    }
            ...
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
            {
                ...
                if(![searchBarUsers.text isEqualToString:@""])
                    ...
                    usersArray = [NSMutableArray arrayWithArray:[responseDict valueForKey:kSoftwareAPI_UserList]] ;
...
-(void)editSoftware {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[[sectionsArray objectAtIndex:SOFTWARE_TITLE_SECTION_INDEX] valueForKey:@"value"] forKey:kSoftwareAPI_Software_Title] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:SOFTWARE_DESCRIPTION_SECTION_INDEX] valueForKey:@"value"] forKey:kSoftwareAPI_Description] ;
        
        [dictParam setObject:[[sectionsArray objectAtIndex:SOFTWARE_START_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kSoftwareAPI_StartDate] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:SOFTWARE_END_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kSoftwareAPI_EndDate] ;
        
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedIndustryKeywordsArray withTagsArray:industryKeywordsArray tagType:@"industry"] forKey:kAddSoftwareAPI_IndustryKeywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedSoftwareKeywordsArray withTagsArray:softwareKeywordsArray tagType:@"softwarekeyword"] forKey:kAddSoftwareAPI_Keywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedTargetMarketKeywordsArray withTagsArray:targetMarketKeywordsArray tagType:@"targetmarket"] forKey:kAddSoftwareAPI_Target_Market_Keywords] ;
        
        if(imgData)
            [dictParam setObject:imgData forKey:kSoftwareAPI_Software_Image] ;
        else
            [dictParam setObject:@"" forKey:kSoftwareAPI_Software_Image] ;
        
        [dictParam setObject:@"" forKey:kSoftwareAPI_Software_Document] ;
        [dictParam setObject:@"" forKey:kSoftwareAPI_Software_Audio] ;
        [dictParam setObject:@"" forKey:kSoftwareAPI_Software_Video] ;
        ...
        [ApiCrowdBootstrap editSoftwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kEditSoftware_SuccessMessage withResultType:@"0" withDuration:1] ;
                [UtilityClass setComingFrom_Software_AddEditScreen:YES];
                [self.navigationController popViewControllerAnimated:YES] ;
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == popupTblView) {
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            else if (selectedKeywordType == SOFTWARE_TARGET_MARKET_KEYWORDS_SELECTED)
                return targetMarketKeywordsArray.count ;
            else
                return softwareKeywordsArray.count ;
        }
        ...
        if(section <= SOFTWARE_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)
            return 1;
        else {
            if(section == SOFTWARE_IMAGE_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue])
                    return 1;
                else return 0 ;
            }
            else if(section == SOFTWARE_DOCUMENT_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == SOFTWARE_AUDIO_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(selectedKeywordType == SOFTWARE_INDUSTRY_KEYWORDS_SELECTED) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kSoftwareAPI_Industry_Name] ;
            if ([selectedIndustryKeywordIds containsObject:[dictKeyword valueForKey:kSoftwareAPI_Industry_ID]]) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kSoftwareAPI_Target_Market_Name] ;
            
            if ([selectedTargetMarketKeywordIds containsObject:[dictKeyword valueForKey:kSoftwareAPI_Target_Market_ID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        } else
        {
            if (![searchBarKeywords.text isEqualToString:@""])
                ...
                dictKeyword = [softwareKeywordsArray objectAtIndex:indexPath.row];
            
            cell.companyNameLbl.text = [dictKeyword valueForKey:kSoftwareAPI_KeywordName] ;
            
            if ([selectedSoftwareKeywordIds containsObject:[dictKeyword valueForKey:kSoftwareAPI_KeywordID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        if (![searchBarUsers.text isEqualToString:@""]) {
            ...
            cell.lblName.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kSoftwareAPI_User_Name]] ;
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kSoftwareAPI_User_Desc]];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kSoftwareAPI_User_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
        ...
        if(indexPath.section == sectionsArray.count) {
        ...
        else if(indexPath.section == SOFTWARE_FOLLOW_SECTION_INDEX) {
        ...
        else if(indexPath.section == SOFTWARE_INDUSTRY_KEYWORDS_SECTION_INDEX || indexPath.section == SOFTWARE_TARGET_MARKET_KEYWORDS_SECTION_INDEX || indexPath.section == SOFTWARE_KEYWORDS_SECTION_INDEX) {
            ...
            NSMutableArray *tags ;
            if(indexPath.section == SOFTWARE_INDUSTRY_KEYWORDS_SECTION_INDEX)
                tags = [selectedIndustryKeywordNames mutableCopy] ;
            else if(indexPath.section == SOFTWARE_TARGET_MARKET_KEYWORDS_SECTION_INDEX)
                tags = [selectedTargetMarketKeywordNames mutableCopy] ;
            else
                tags = [selectedSoftwareKeywordNames mutableCopy] ;
        ...
        else if(indexPath.section == SOFTWARE_DESCRIPTION_SECTION_INDEX) {
        ...
        else if(indexPath.section == SOFTWARE_START_DATE_SECTION_INDEX || indexPath.section == SOFTWARE_END_DATE_SECTION_INDEX) {
        ...
        else if(indexPath.section == SOFTWARE_IMAGE_SECTION_INDEX) {
        ...
        else if (indexPath.section == SOFTWARE_DOCUMENT_SECTION_INDEX || indexPath.section == SOFTWARE_AUDIO_SECTION_INDEX || indexPath.section == SOFTWARE_VIDEO_SECTION_INDEX) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(indexPath.section >= SOFTWARE_DOCUMENT_SECTION_INDEX && indexPath.section < sectionsArray.count) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if(indexPath.section == SOFTWARE_START_DATE_SECTION_INDEX || indexPath.section == SOFTWARE_END_DATE_SECTION_INDEX) {
            return 65;
        }
        else if (indexPath.section == SOFTWARE_FOLLOW_SECTION_INDEX)
            return 40;
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if(section <= SOFTWARE_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)return 0;
    else return 45 ;
}
