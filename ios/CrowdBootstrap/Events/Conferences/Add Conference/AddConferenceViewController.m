...
@interface AddConferenceViewController ()
...
-(void)resetUISettings {
    ...
    self.title = @"Add Conference";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Conference Title", @"Conference Description", @"Conference Availability Start Date", @"Conference Availability End Date", @"Target Market", @"Conference Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kConferenceAPI_Title, kConferenceAPI_Description, kConferenceAPI_StartDate, kConferenceAPI_EndDate, kAddConferenceAPI_Target_Market_Keywords, kAddConferenceAPI_Keywords, kAddConferenceAPI_IndustryKeywords, kConferenceAPI_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kConferenceAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kConferenceAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kConferenceAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createConference {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addConferenceWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddConference_SuccessMessage withResultType:@"0" withDuration:1] ;
