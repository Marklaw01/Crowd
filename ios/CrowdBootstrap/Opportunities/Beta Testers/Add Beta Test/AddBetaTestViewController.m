...
#import "AddBetaTestViewController.h"
...
@interface AddBetaTestViewController ()

@end

@implementation AddBetaTestViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Beta Test Requirements";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Beta Test Title", @"Beta Test Description", @"Beta Test Start Date", @"Beta Test End Date", @"Target Market", @"Beta Test Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kBetaTestAPI_BetaTest_Title, kBetaTestAPI_Description, kBetaTestAPI_StartDate, kBetaTestAPI_EndDate, kAddBetaTestAPI_Target_Market_Keywords, kAddBetaTestAPI_Keywords, kAddBetaTestAPI_IndustryKeywords, kBetaTestAPI_BetaTest_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kBetaTestAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kBetaTestAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kBetaTestAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createBetaTest {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addBetaTestWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddBetaTest_SuccessMessage withResultType:@"0" withDuration:1] ;
