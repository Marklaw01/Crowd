...
//alert (JSON.stringify(resDialogs.items)); 
      ...
      updateDialogsUsersStorage(jQuery.unique(occupantsIds), function(){
        ...
        resDialogs.items.forEach(function(item, i, arr) { 
          ...
          var ocupId= '"'+item.occupants_ids+'"'; //alert(ocupId);
          var arr = ocupId.split(',');
          var aa= arr[0].replace('"',''); 
          var bb= arr[1].replace('"','');
          if(aa != currId){
            chatUser = aa;
          }else{
            chatUser = bb;
          ...
          showOrUpdateDialogInUI(item,chatUser, false);
        });
...
function showOrUpdateDialogInUI(itemRes,chatUser, updateHtml) {
  var dialogId = itemRes._id;
  var user_id = itemRes.occupants_ids[1]; 
  ...
  if (updateHtml === true) {
  	var updatedDialogHtml = buildDialogHtml(dialogId, dialogUnreadMessagesCount, dialogIcon, dialogName, dialogLastMessage,user_id,chatUser);
  	$('#dialogs-list').prepend(updatedDialogHtml);
  	$('.list-group-item.active .badge').text(0).hide(0);
	} else {
    var dialogHtml = buildDialogHtml(dialogId, dialogUnreadMessagesCount, dialogIcon, dialogName, dialogLastMessage,user_id,chatUser);
    $('#dialogs-list').append(dialogHtml);
	}

  $( "#dialogs-list li" ).each(function() {
      if($(this).html() == ''){
        $( this ).remove();
      }
  });
...
function getDialogIcon (dialogType) {
  var groupPhoto = '<i class="icon"><span class="fa fa-group"></span></i>';
  var privatPhoto  = '<i class="icon"><span class="fa fa-user"></span></i>'; 
...
function triggerDialog(dialogId){
  ...
  // deselect
  var kids = $('#dialogs-list li').children();
    ...
    if  ($('.list-group.pre-scrollable.for-scroll').scrollTop() == $('#users_list').height() - $('.list-group.pre-scrollable.for-scroll').height()){
      ...
      retrieveUsersForDialogCreation(function(users) {
        $.each(users, function(index, item){
          showUsers(this.user.login, this.user.id, this.user.full_name);
        });
      });
    }
  });
...
//
function showUsers(userLogin, userId, userName ) {
  var userHtml = buildUserHtml(userLogin, userId, userName, false);
  $('#users_list').append(userHtml);
}

function showGroupChatPoup(){
...
function showNewDialogPopup() {
  ...
  retrieveUsersForDialogCreation(function(users) {
    ...
    $.each(users, function(index, item){
      if(logeUserId != this.user.id){
        showUsers(this.user.login, this.user.id, this.user.full_name);
        showContacts(this.user.login, this.user.id, this.user.full_name);
      }
    });
  });
...
// Custom by VJ to get user on page load


// show modal window with users

/*function setupUsersScrollHandlerContacts(){
  $('.list-group.pre-scrollable.for-scroll').scroll(function() {
    if  ($('.list-group.pre-scrollable.for-scroll').scrollTop() == $('#users_list').height() - $('.list-group.pre-scrollable.for-scroll').height()){

      // get and show users
      retrieveUsersForDialogCreationContacts(function(users) {
        $.each(users, function(index, item){
          showContacts(this.user.login, this.user.id, this.user.full_name);
        });
      });
...
function showContacts(userLogin, userId, userName) {
  var userContactHtml = buildContactHtml(userLogin, userId, userName, false);
  $('#contacts_list').append(userContactHtml);
}
...
/*function getContactsList(userId) {
  ...
  setupUsersScrollHandlerContacts();
}*/
...
function clickToAdd(forFocus) {
  if ($('#users_list li #'+forFocus).hasClass("active")) {
    $('#users_list li #'+forFocus).removeClass("active");
  } else {
    $('#users_list li #'+forFocus).addClass("active");
  }
}

function clickToAddGroup(forFocus) {
  if ($('#wrapper ul li #'+forFocus).hasClass("active")) {
    $('#wrapper ul li #'+forFocus).removeClass("active");
  } else {
    $('#wrapper ul li #'+forFocus).addClass("active");
  }
}

// create new dialog for group
function createNewDialog() {
   
  var usersIds = [];
  var usersNames = [];
  var customName = $('#custom_name_group').val();
  ...
  //alert('dfsd');
      $('#tabbing li').removeClass('active');
      $('#tabbing #li-recent').addClass( "active" );
      $('.tab-content #recent').addClass( "active" );
...
function createContactDialog(useId) 
{
  var usersIds = useId;
  $('#dialogs-list').att
  //$("#dialogs-list a[ data !="+usersIds+"]").addClass("vijay");

$links = $("#dialogs-list a[data*="+usersIds+"]");
$length = $links.length; 
  if($length > 0)
  {
    var idVal = $("#dialogs-list a[ data *="+usersIds+"]").attr("id");
    //alert(idVal);
    triggerDialog(idVal);
    $('#tabbing li').removeClass('active');
    $('#tabbing #li-recent a').trigger( "click" );

    $('#recent ul li a').removeClass('active');
    $('#recent ul li #'+idVal).addClass('active');
...
function showDialogInfoPopup() {
  ...
  var checkMsg = $('#recent #dialogs-list li').length;
    if(checkMsg != 0){  
...
function setupDialogInfoPopup(occupantsIds, name) {
  ...
  $('#all_occupants').text('');
  $('#all_occupants').append('<b>Users: </b>'+logins.join(', '));

  // show type
  //
  // private
  if (currentDialog.type == 3) {
    $('.dialog-type-info').text('').append('<b>Chat Type: </b>Private Chat');
    $('.new-info').hide();
    $('.push').hide();
    $('#push_usersList').hide();
  ...
  // group
    ...
    $('.dialog-type-info').text('').append('<b>Chat Type: </b>Group Chat');
    $('.new-info').show();
    $('.push').show();
    $('#push_usersList').show();
    ...
    retrieveUsersForDialogUpdate(function(users){
      ...
      $.each(users, function(index, item){
        var userHtml = buildUserHtml(this.user.login, this.user.id, this.user.full_name, true);
  ...
  $('#push_usersList').scroll(function() {
    if  ($('#push_usersList').scrollTop() == $('#add_new_occupant').height() - $('#push_usersList').height()){
      ...
      retrieveUsersForDialogUpdate(function(users){
        ...
        $.each(users, function(index, item){
          var userHtml = buildUserHtml(this.user.login, this.user.id, this.user.full_name, false);
          $('#add_new_occupant').append(userHtml);
        });
      });
...
function onDialogDelete() {
  if (confirm("Are you sure you want delete chat?")) { 
    QB.chat.dialog.delete(currentDialog._id, function(err, res) { 
       
        console.log("Dialog removed"); 
        $('#dialogs-list li #'+currentDialog._id).parent('li').remove();
        ...
        if(Object.keys(dialogs).length > 0){
          ...
          $('#messages-list #'+currentDialog._id).remove();
        }
     ...
     /* if (err) {
        ...
        console.log("Dialog removed"); 
        $('#dialogs-list li #'+currentDialog._id).remove();
