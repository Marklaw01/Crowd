...
class SoftwareUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('SoftwareInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('SoftwareKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('SoftwareTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('SoftwareFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['software_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('SoftwareCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['software_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('SoftwareCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['software_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/software/"; // Upload directory
                $imgCamp='';
                $errors='';
		...
		public function base64ToJPG($base64_string =nul) 
	    {	
	    	$path = WWW_ROOT . "img/software/";
	    	$imgNum = rand(10,1000000);
	    	$imgName = $imgNum.'-img.png';
	    	$output_file = $path.$imgName;

	    	$ifp = fopen($output_file, "w+") or die(print_r(error_get_last()));
