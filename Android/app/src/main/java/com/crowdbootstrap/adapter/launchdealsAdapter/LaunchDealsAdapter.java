package com.crowdbootstrap.adapter.launchdealsAdapter;
...
import com.crowdbootstrap.fragments.launchdealsmodule.LaunchDealsLikeDislikeFragment;
 ...
 */
public class LaunchDealsAdapter extends BaseAdapter implements View.OnClickListener {
    ...
    public LaunchDealsAdapter(Context context, ArrayList<FundsObject> list, String fragmentName) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        try {
            ...
            holder.tv_delete.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    showDialog(position, "Do you want to delete this deal?", Constants.LAUNCHDEAL_DELETE_URL);
                }
            });
            ...
            holder.tv_archive.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    showDialog(position, "Do you want to archive this deal?", Constants.LAUNCHDEAL_ARCHIEVE_URL);
                }
            });
            ...
            holder.tv_deactivate.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    showDialog(position, "Do you want to deactivate this deal?", Constants.LAUNCHDEAL_DEACTIVATE_URL);
                }
            });
                    ...
                    public void onClick(DialogInterface dialog, int arg1) {
                        ...
                        if (networkConnectivity.isInternetConnectionAvaliable()) {
                            ...
                            try {
                                ...
                                obj.put("launchdeal_id", list.get(position).getId());
        ...
        switch (v.getId()) {
            case R.id.like:
                ...
                if (list.get(tagLikePosition).getIs_liked_by_user() == 1) {
                    Toast.makeText(context, "You already liked this deal", Toast.LENGTH_LONG).show();
                } else {
                    try {
                        JSONObject likeObj = new JSONObject();
                        likeObj.put("like_by", PrefManager.getInstance(context).getString(Constants.USER_ID));
                        likeObj.put("launchdeal_id", list.get(tagLikePosition).getId());
                        fundLikeDislike(tagLikePosition, Constants.LAUNCHDEAL_LIKE_URL, Constants.HTTP_POST_REQUEST, likeObj);
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                }
            ...
            case R.id.dislike:
                ...
                if (list.get(tagDislikeIdPosition).getIs_disliked_by_user() == 1) {
                    Toast.makeText(context, "You already disliked this deal", Toast.LENGTH_LONG).show();
                } else {
                    try {
                        JSONObject dislikeObj = new JSONObject();
                        dislikeObj.put("dislike_by", PrefManager.getInstance(context).getString(Constants.USER_ID));
                        dislikeObj.put("launchdeal_id", list.get(tagDislikeIdPosition).getId());
                        fundLikeDislike(tagDislikeIdPosition, Constants.LAUNCHDEAL_DISLIKE_URL, Constants.HTTP_POST_REQUEST, dislikeObj);
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                }
            ...
            case R.id.tv_Like:
                ...
                if (list.get(tagLikeId).getFund_likes() != 0) {
                    ...
                    Fragment likeFragment = new LaunchDealsLikeDislikeFragment();
            ...
            case R.id.tv_dislike:
                ...
                if (list.get(tagDislikeId).getFund_dislike() != 0) {
                    ...
                    Fragment dislikeFragment = new LaunchDealsLikeDislikeFragment();
