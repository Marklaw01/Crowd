  ...
  <div class="row">
    <div class="col-lg-12 ">
      <ol class="breadcrumb">
        ...
        ?></li>
        <li> <?php
            echo $this->Html->link('Focus Groups', array('controller'=>'FocusGroups','action'=>'index'), array('escape'=>false));
        ?></li>
        <li class="active">View Focus Groups</li>
      </ol>
    </div>
  </div>
  <!-- breadcrumb ends --> 
    ...
    <div class='col-lg-4 col-md-4 col-sm-4 '>
      <div class="form-group">
        <span class="contentListingLikes">
          <h2 class="likes">
            <?php 
              $likeC = $this->Custom->getFocusLikeCount($fundDeatils->id);
              if(!empty($likeC)){

                  echo $this->Html->link($likeC, array('controller'=>'FocusGroups','action'=>'likeList',base64_encode($fundDeatils->id)), array('escape'=>false,'title'=>'View','style'=>'color: rgb(3, 66, 110);'));
            ...
            &nbsp;
            <?php 
            if($this->Custom->isFocusLikedbyUser($fundDeatils->id,$userId)){
            ...
            &nbsp;
            <?php 
              $disLikeC = $this->Custom->getFocusDislikeCount($fundDeatils->id);
              if(!empty($disLikeC)){

                  echo $this->Html->link($disLikeC, array('controller'=>'FocusGroups','action'=>'dislikeList',base64_encode($fundDeatils->id)), array('escape'=>false,'title'=>'View','style'=>'color: rgb(3, 66, 110);'));
            ...
            &nbsp;
            <?php 
              if($this->Custom->isFocusDislikedbyUser($fundDeatils->id,$userId)){
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 ">
      ...
      <div class="form-group">
        ...
        <span id="selectedResult" class="form-control textArea">
          <ul>
          <?php
            $skey5= explode(',', $fundDeatils->focus_group_keywords_id);
              $cskey5=count($skey5);
                if(!empty($skey5[0])){
                  for($ij=0; $ij<$cskey5; $ij++){
                      $slids5=$skey5[$ij];
                         ...
                         echo 'No Focus Group Keywords Added';
                }
          ?>
          </ul>
        </span>
          ...
          <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
              <div class="form-group">
                ...
                <span id="selectedResult" class="form-control textArea">
                  <ul>
                  <?php
                    $skey3= explode(',', $fundDeatils->focus_group_interest_keywords_id);
                      $cskey3=count($skey3);
                        if(!empty($skey3[0])){
                  ...
                  <div class="panel-body">
                    <div class="image-Holder">
                      <?php if(!empty($fundDeatils->image)){?>
                        <img src="<?php echo $this->request->webroot.'img/focus_group/'.$fundDeatils->image;?>" alt="Fund Image">
                      <?php } ?>
                    </div>
                    </div>
                </div>
                   ...
                   <ul class="listing anchor">
                    <?php 
                        if(!empty($fundDeatils->document)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/focus_group/'.$fundDeatils->document.'" target="_blank"> Document <i class="fa fa-eye"></i></a>';
                   ...
                   <ul class="listing anchor">
                     <?php 
                        if(!empty($fundDeatils->audio)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/focus_group/'.$fundDeatils->audio.'" target="_blank"> Audio <i class="fa fa-eye"></i></a>';
                   ...
                   <ul class="listing anchor">
                      <?php 
                        if(!empty($fundDeatils->video)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/focus_group/'.$fundDeatils->video.'" target="_blank"> Video <i class="fa fa-eye"></i></a>';
              ...
              if(!empty($is_follwed_by_user)){
                $class1= 'greenBtn';
                  $titleFollow='Unfollow';
                    $followLink= $this->Url->build(["controller" => "FocusGroups","action" => "unfollow",base64_encode($fundDeatils->id),base64_encode($userId)]);                      
              }else {
                $class1= 'blueBtn';
                $titleFollow='Follow';
                    $followLink= $this->Url->build(["controller" => "FocusGroups","action" => "follow",base64_encode($fundDeatils->id),base64_encode($userId)]);   
              }
              ...
              if(!empty($is_commited_by_user)){
                $class= 'greenBtn';
                  $titleCommit='Thank you. You have registered your interest.';
                    $CommitLink= $this->Url->build(["controller" => "FocusGroups","action" => "uncommit",base64_encode($fundDeatils->id),base64_encode($userId)]);                      
              }else {
                $class= 'blueBtn';
                $titleCommit='Click here to register your interest.';
                    $CommitLink= $this->Url->build(["controller" => "FocusGroups","action" => "commit",base64_encode($fundDeatils->id),base64_encode($userId)]);   
              }
