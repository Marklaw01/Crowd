...
#import "AddCareerViewController.h"
...
@interface AddCareerViewController ()

@end

@implementation AddCareerViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Add Career Help Tool";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Career Help Title", @"Career Help Description", @"Career Help Availability Start Date", @"Career Help Availability End Date", @"Target Market", @"Career Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kCareerAPI_Title, kCareerAPI_Description, kCareerAPI_StartDate, kCareerAPI_EndDate, kAddCareerAPI_Target_Market_Keywords, kAddCareerAPI_Keywords, kAddCareerAPI_IndustryKeywords, kCareerAPI_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kCareerAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kCareerAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kCareerAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if([textView.text isEqualToString:@"Career Help Description"] && textView.textColor == [UIColor lightGrayColor]){
        textView.text = @"" ;
...
-(BOOL)textViewShouldEndEditing:(UITextView *)textView {
    ...
    if([textView.text isEqualToString:@""]){
        textView.text = @"Career Help Description" ;
...
-(void)createCareer {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addCareerWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddCareer_SuccessMessage withResultType:@"0" withDuration:1] ;
