...
#import "AddHardwareViewController.h"
...
@interface AddHardwareViewController ()
...
-(void)resetUISettings {
    ...
    self.title = @"Add Hardware";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Hardware Title", @"Hardware Description", @"Hardware Availability Start Date", @"Hardware Availability End Date", @"Target Market", @"Hardware Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kHardwareAPI_Hardware_Title, kHardwareAPI_Description, kHardwareAPI_StartDate, kHardwareAPI_EndDate, kAddHardwareAPI_Target_Market_Keywords, kAddHardwareAPI_Keywords, kAddHardwareAPI_IndustryKeywords, kHardwareAPI_Hardware_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kHardwareAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kHardwareAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kHardwareAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createHardware {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addHardwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddHardware_SuccessMessage withResultType:@"0" withDuration:1] ;
