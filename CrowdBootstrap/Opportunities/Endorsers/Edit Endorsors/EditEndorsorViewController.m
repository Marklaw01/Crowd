...
#import "EditEndorsorViewController.h"
...
@interface EditEndorsorViewController ()

@end

@implementation EditEndorsorViewController
...
-(void) awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(SetViewEditingNotification:) name:kNotificationSetEndorsorViewEditing
                                               object:nil];
...
-(void)SetViewEditingNotification:(NSNotification *)notification {
    if ([[notification name] isEqualToString:kNotificationSetEndorsorViewEditing]) {
...
-(void)resetUISettings {
    ...
    endorsorData = [[UtilityClass getEndorsorDetails] mutableCopy];
    NSLog(@"Endorsor Data: %@",endorsorData) ;
    ...
    self.title = [NSString stringWithFormat:@"%@",[endorsorData valueForKey:kEndorsorAPI_Endorsor_Title]] ;
    [self getEndorsorDetails] ;
...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Created By", @"Endorsement Title", @"Endorsement Description", @"Endorsement Start Date", @"Endorsement End Date", @"Target Market", @"Endorsement Keywords", @"Interest Keywords", @"Upload Image", @"View Document",@"Play Audio",@"Play Video"] ;
    NSArray *parametersArray = @[kEndorsorAPI_CreatedBy, kEndorsorAPI_Endorsor_Title, kEndorsorAPI_Description, kEndorsorAPI_StartDate, kEndorsorAPI_EndDate, kAddEndorsorAPI_Target_Market_Keywords, kAddEndorsorAPI_Keywords, kAddEndorsorAPI_IndustryKeywords, kEndorsorAPI_Endorsor_Image, kEndorsorAPI_Endorsor_Document, kEndorsorAPI_Endorsor_Audio, kEndorsorAPI_Endorsor_Video] ;
    ...
    [dateFormatter setDateFormat:@"MMM dd, yyyy"];
        ...
        [self getEndorsorKeywordsList];
        [self getEndorsorIndustryKeywordsList];
...
- (void)checkKeywordsListFound:(int)keywordType {
    switch (keywordType) {
        ...
        case 6:
        {
            if (endorsorKeywordsArray.count > 0)
                [self openTagsPopup] ;
            else
                [self presentViewController:[UtilityClass displayAlertMessage:kNoEndorsorKeywordsFoundMessage] animated:YES completion:nil];
        }
            break;
        default:
            break;
...
-(NSString*)convertTagsArrayToStringforArray:(NSMutableArray*)array withTagsArray:(NSArray*)tagsArray tagType:(NSString *)type {
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kEndorsorAPI_Target_Market_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kEndorsorAPI_Target_Market_ID])
                    selectedValue = [obj valueForKey:kEndorsorAPI_Target_Market_ID] ;
                else
                    selectedValue = [obj valueForKey:kEndorsorAPI_Target_Market_ID] ;

                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag)
                        tagsStr = [dict valueForKey:kEndorsorAPI_Target_Market_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kEndorsorAPI_Target_Market_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
    } else if ([type isEqualToString:@"endorsorkeyword"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kEndorsorAPI_KeywordID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kEndorsorAPI_KeywordID])
                    selectedValue = [obj valueForKey:kEndorsorAPI_KeywordID] ;
                else
                    selectedValue = [obj valueForKey:kEndorsorAPI_KeywordID] ;

                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kEndorsorAPI_KeywordID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kEndorsorAPI_KeywordID]] ;
                    isFirstTag = NO ;
                }
            }
        }
        ...
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kEndorsorAPI_Industry_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kEndorsorAPI_Industry_ID])
                    selectedValue = [obj valueForKey:kEndorsorAPI_Industry_ID] ;
                else
                    selectedValue = [obj valueForKey:kEndorsorAPI_Industry_ID] ;

                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kEndorsorAPI_Industry_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kEndorsorAPI_Industry_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
...
- (IBAction)CheckUncheck_ClickAction:(id)sender {
    ...
    if([btn.accessibilityLabel isEqualToString:CHECK_IMAGE ]) { // Check
        ...
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            if(selectedKeywordType == ENDORSOR_INDUSTRY_KEYWORDS_SELECTED)
                dictKeyword = [industryKeywordsArray objectAtIndex:btn.tag];
            else if (selectedKeywordType == ENDORSOR_KEYWORDS_SELECTED)
                dictKeyword = [endorsorKeywordsArray objectAtIndex:btn.tag];
            else
    ...
    if(selectedKeywordType == ENDORSOR_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (NSMutableDictionary *obj in industryKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedIndustryKeywordNames addObject:[obj valueForKey:kEndorsorAPI_Industry_Name]] ;
                [selectedIndustryKeywordIds addObject:[obj valueForKey:kEndorsorAPI_Industry_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_INDUSTRY_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        
    } else if(selectedKeywordType == ENDORSOR_KEYWORDS_SELECTED) {
        [selectedEndorsorKeywordsArray removeAllObjects];
        [selectedEndorsorKeywordNames removeAllObjects];
        [selectedEndorsorKeywordIds removeAllObjects];
        
        for (NSMutableDictionary *obj in endorsorKeywordsArray) {
            NSString *isSelectedStr = [obj valueForKey:@"isSelected"] ;
            if([isSelectedStr isEqualToString:@"1"]) {
                [selectedEndorsorKeywordsArray addObject:obj] ;
                [selectedEndorsorKeywordNames addObject:[obj valueForKey:kEndorsorAPI_KeywordName]] ;
                [selectedEndorsorKeywordIds addObject:[obj valueForKey:kEndorsorAPI_KeywordID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        ...
        for (NSMutableDictionary *obj in targetMarketKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedTargetMarketKeywordNames addObject:[obj valueForKey:kEndorsorAPI_Target_Market_Name]] ;
                [selectedTargetMarketKeywordIds addObject:[obj valueForKey:kEndorsorAPI_Target_Market_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_TARGET_MARKET_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
    }
}
...
-(void)tagsControl:(UIView *)tag withTagControlIndex:(NSInteger)tagControlIndex removedFromIndex:(NSInteger)index {
    ...
    if(tagControlIndex == ENDORSOR_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (int i=0; i<industryKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[industryKeywordsArray objectAtIndex:i] valueForKey:kEndorsorAPI_Industry_Name]] ;
    ...
    } else if(tagControlIndex == ENDORSOR_KEYWORDS_SELECTED) {
        NSString *selectedName = [NSString stringWithFormat:@"%@",[selectedEndorsorKeywordNames objectAtIndex:index]] ;
        for (int i=0; i<endorsorKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[endorsorKeywordsArray objectAtIndex:i] valueForKey:kEndorsorAPI_KeywordName]] ;
            if([keywordName isEqualToString:selectedName]){
                [[endorsorKeywordsArray objectAtIndex:i] setValue:@"0" forKey:@"isSelected"] ;
                break ;
            }
        }
        [selectedEndorsorKeywordsArray removeObjectAtIndex:index] ;
        [selectedEndorsorKeywordNames removeObjectAtIndex:index];
        [selectedEndorsorKeywordIds removeObjectAtIndex:index];
        ...
        for (int i=0; i<targetMarketKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[targetMarketKeywordsArray objectAtIndex:i] valueForKey:kEndorsorAPI_Target_Market_Name]] ;
            if([keywordName isEqualToString:selectedName]){
...
- (IBAction)Back_Click:(id)sender {
    [UtilityClass setComingFrom_Endorsor_AddEditScreen:YES];
    [self.navigationController popViewControllerAnimated:YES] ;
}

- (IBAction)CloseData_ClickAction:(id)sender {
    [endorsorDataView dismissPresentingPopup] ;
}
...
- (IBAction)followButton_ClickAction:(UIButton*)button {
    ...
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kEndorsorAPI_Endorsor_FollowedBy] ;
        [dictParam setObject:[[[UtilityClass getEndorsorDetails] mutableCopy] valueForKey:kEndorsorAPI_ID] forKey:kEndorsorAPI_EndorsorID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isFollowed == 0) {
            [ApiCrowdBootstrap followEndorsorWithParameters:dictParam success:^(NSDictionary *responseDict) {
                ...
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    ...
                    [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_FOLLOW_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationNone] ;
                }
            ...
            [ApiCrowdBootstrap unfollowEndorsorWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                    if(isFollowed == 1)
...
- (IBAction)Calender_ClickAction:(id)sender {
    ...
    if ([sender tag] == ENDORSOR_START_DATE_SECTION_INDEX)
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_START_DATE_SECTION_INDEX]] ;
    else
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_END_DATE_SECTION_INDEX]] ;
...
- (IBAction)Submit_ClickAction:(id)sender {
    if ((selectedSegmentControl == 100 && selectedSegment == 0)) {
        ...
        if(![self validatetextFieldsWithSectionIndex:ENDORSOR_TITLE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:ENDORSOR_START_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:ENDORSOR_END_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:ENDORSOR_DESCRIPTION_SECTION_INDEX]) return ;
        ...
        else if(selectedEndorsorKeywordsArray.count < 1) {
        ...
        else [self editEndorsor] ;
    }
}

- (IBAction)endorsorCommitmentList_ClickAction:(id)sender {
...
//    imgVwPeople.image = [UIImage imageNamed:@"like_green"];
    ...
    if ([noOfcommit integerValue] > 0) {
        [self getEndorsorCommitmentList];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    GraphicTableViewCell *cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_IMAGE_SECTION_INDEX]] ;
...
- (IBAction)DatePickerToolbarButtons_ClickAction:(id)sender {
    ...
    if([sender tag] == DONE_CLICKED) {
        if(selectedDatePickerType == ENDORSOR_START_DATE_SELECTED) {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_START_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            cell.textFld.text = [dateFormatter stringFromDate:datePickerView.date];
            [[sectionsArray objectAtIndex:ENDORSOR_START_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
        else {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_END_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            ...
            [[sectionsArray objectAtIndex:ENDORSOR_END_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
    }
    else {
        if(selectedDatePickerType == ENDORSOR_START_DATE_SELECTED) {
    ...
    if (searchBar.tag == 10) {
        ...
        switch (selectedKeywordType) {
            case 5: // Target Keywords
            {
                ...
                break;
            case 6: // Endorser Keywords
            {
                for(NSDictionary *keywordDict in endorsorKeywordsArray)
                {
                    NSString *keywordName = [keywordDict objectForKey:@"name"];
                    NSRange range = [keywordName rangeOfString:searchText options:NSCaseInsensitiveSearch];
                    if(range.location != NSNotFound)
                ...
                NSLog(@"Endorser Results: %@",searchResultsForKeywords);
            }
                break;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    DobTableViewCell *startDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_START_DATE_SECTION_INDEX]] ;
    
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        textField.userInteractionEnabled = true;
        if(textField.tag == ENDORSOR_START_DATE_SECTION_INDEX || textField.tag == ENDORSOR_END_DATE_SECTION_INDEX) {
            if(textField.tag == ENDORSOR_START_DATE_SECTION_INDEX) {
                selectedDatePickerType = ENDORSOR_START_DATE_SELECTED;
            }
            else {
                selectedDatePickerType = ENDORSOR_END_DATE_SELECTED;
...
-(void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if(textField.tag == ENDORSOR_START_DATE_SECTION_INDEX || textField.tag == ENDORSOR_END_DATE_SECTION_INDEX) {
            if(textField.tag == ENDORSOR_START_DATE_SECTION_INDEX) {
                DobTableViewCell *endDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:ENDORSOR_END_DATE_SECTION_INDEX]] ;
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@"Endorsement Description"] && textView.textColor == [UIColor lightGrayColor]){
            textView.text = @"" ;
...
-(BOOL)textViewShouldEndEditing:(UITextView *)textView {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@""]){
            textView.text = @"Endorsement Description" ;
...
-(void)getEndorsorKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getEndorsorKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [endorsorKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kEndorsorAPI_KeywordList]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kEndorsorAPI_KeywordList] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
                        [endorsorKeywordsArray addObject:obj] ;
                    }
                }
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode) {
...
-(void)getEndorsorIndustryKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getEndorsorIndustryKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [industryKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kEndorsorAPI_Industry_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kEndorsorAPI_Industry_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getTargetMarketKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getEndorsorTargetMarketKeywordsListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [targetMarketKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kEndorsorAPI_Target_Market_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kEndorsorAPI_Target_Market_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getEndorsorDetails {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kEndorsorAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[endorsorData valueForKey:kEndorsorAPI_ID]] forKey:kEndorsorAPI_EndorsorID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap viewEndorsorWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"respos %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                for ( int i = 0; i < sectionsArray.count ; i++ ) {
                    NSString *key = [[sectionsArray objectAtIndex:i] valueForKey:@"key"] ;
                    if(i != ENDORSOR_INDUSTRY_KEYWORDS_SECTION_INDEX && i != ENDORSOR_TARGET_MARKET_KEYWORDS_SECTION_INDEX && i != ENDORSOR_KEYWORDS_SECTION_INDEX) {
                ...
                // Keywords Array
                if([responseDict objectForKey:kAddEndorsorAPI_IndustryKeywords]) {
                    selectedIndustryKeywordsArray = [[responseDict objectForKey:kAddEndorsorAPI_IndustryKeywords] mutableCopy];
                ...
                if([responseDict objectForKey:kAddEndorsorAPI_Keywords]) {
                    selectedEndorsorKeywordsArray = [[responseDict objectForKey:kAddEndorsorAPI_Keywords] mutableCopy] ;
                    selectedEndorsorKeywordNames = [self resetTagsArrayWithData:selectedEndorsorKeywordsArray];
                    selectedEndorsorKeywordIds = [self resetTagIdsArrayWithData:selectedEndorsorKeywordsArray];
                }
                if([responseDict objectForKey:kAddEndorsorAPI_Target_Market_Keywords]) {
                    selectedTargetMarketKeywordsArray = [[responseDict objectForKey:kAddEndorsorAPI_Target_Market_Keywords] mutableCopy];
                ...
                // Commits
                noOfcommit = [[responseDict valueForKey:kEndorsorAPI_NoOfCommits] stringValue];
                isCommited = [[responseDict valueForKey:kEndorsorAPI_IsCommited] intValue];
                isLiked = [[responseDict valueForKey:kEndorsorAPI_IsLiked] intValue];
                isFollowed = [[responseDict valueForKey:kEndorsorAPI_IsFollowed] intValue];

                // Document
                if([responseDict objectForKey:kEndorsorAPI_Endorsor_Document])
                    docuementFile = [[responseDict objectForKey:kEndorsorAPI_Endorsor_Document] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Audio
                if([responseDict objectForKey:kEndorsorAPI_Endorsor_Audio])
                    audioFile = [[responseDict objectForKey:kEndorsorAPI_Endorsor_Audio] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Video
                if([responseDict objectForKey:kEndorsorAPI_Endorsor_Video])
                    videoFile = [[responseDict objectForKey:kEndorsorAPI_Endorsor_Video] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Image
                if([responseDict objectForKey:kEndorsorAPI_Endorsor_Image]) {
                    NSString *strImage = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed, [responseDict objectForKey:kEndorsorAPI_Endorsor_Image]];
                    NSURL *url = [NSURL URLWithString:[strImage stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                    imgData = [NSData dataWithContentsOfURL:url];
                }
                
                // Refresh Dict
                [endorsorData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kEndorsorAPI_Endorsor_Title]] forKey:kEndorsorAPI_Endorsor_Title] ;
                [endorsorData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kEndorsorAPI_CreatedBy]] forKey:kEndorsorAPI_CreatedBy] ;
                [endorsorData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kEndorsorAPI_StartDate]] forKey:kEndorsorAPI_StartDate] ;
                [endorsorData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kEndorsorAPI_EndDate]] forKey:kEndorsorAPI_EndDate] ;
                [endorsorData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kEndorsorAPI_Description]] forKey:kEndorsorAPI_Description] ;
                
                [UtilityClass setEndorsorDetails:[endorsorData mutableCopy]] ;
...
-(void)commitEndorsor {
    if([UtilityClass checkInternetConnection]) {
        ...
        if (isCommited == 0) {
            [ApiCrowdBootstrap commitEndorsorWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
            ...
            [ApiCrowdBootstrap uncommitEndorsorWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
...
-(void)getEndorsorCommitmentList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getEndorsorCommitmentListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode)  {
...
- (void)getRecommendedUserList: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:@"endorsor" forKey:@"type"] ;
        ...
        [ApiCrowdBootstrap getRegisteredRoleListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kEndorsorAPI_UserList]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]){
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kEndorsorAPI_UserList]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kEndorsorAPI_UserList]] ;
                    }
            ...
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
            {
                ...
                if(![searchBarUsers.text isEqualToString:@""])
                    ...
                    usersArray = [NSMutableArray arrayWithArray:[responseDict valueForKey:kEndorsorAPI_UserList]] ;
...
-(void)editEndorsor {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[[sectionsArray objectAtIndex:ENDORSOR_TITLE_SECTION_INDEX] valueForKey:@"value"] forKey:kEndorsorAPI_Endorsor_Title] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:ENDORSOR_DESCRIPTION_SECTION_INDEX] valueForKey:@"value"] forKey:kEndorsorAPI_Description] ;
        
        [dictParam setObject:[[sectionsArray objectAtIndex:ENDORSOR_START_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kEndorsorAPI_StartDate] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:ENDORSOR_END_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kEndorsorAPI_EndDate] ;
        
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedIndustryKeywordsArray withTagsArray:industryKeywordsArray tagType:@"industry"] forKey:kAddEndorsorAPI_IndustryKeywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedEndorsorKeywordsArray withTagsArray:endorsorKeywordsArray tagType:@"endorsorkeyword"] forKey:kAddEndorsorAPI_Keywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedTargetMarketKeywordsArray withTagsArray:targetMarketKeywordsArray tagType:@"targetmarket"] forKey:kAddEndorsorAPI_Target_Market_Keywords] ;
        
        if(imgData)
            [dictParam setObject:imgData forKey:kEndorsorAPI_Endorsor_Image] ;
        else
            [dictParam setObject:@"" forKey:kEndorsorAPI_Endorsor_Image] ;
        
        [dictParam setObject:@"" forKey:kEndorsorAPI_Endorsor_Document] ;
        [dictParam setObject:@"" forKey:kEndorsorAPI_Endorsor_Audio] ;
        [dictParam setObject:@"" forKey:kEndorsorAPI_Endorsor_Video] ;
        ...
        [ApiCrowdBootstrap editEndorsorWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kEditEndorsor_SuccessMessage withResultType:@"0" withDuration:1] ;
                [UtilityClass setComingFrom_Endorsor_AddEditScreen:YES];
                [self.navigationController popViewControllerAnimated:YES] ;
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == popupTblView) {
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            else if (selectedKeywordType == ENDORSOR_TARGET_MARKET_KEYWORDS_SELECTED)
                return targetMarketKeywordsArray.count ;
            else
                return endorsorKeywordsArray.count ;
        }
        ...
        if(section <= ENDORSOR_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)
            return 1;
        else {
            if(section == ENDORSOR_IMAGE_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue])
                    return 1;
                else return 0 ;
            }
            else if(section == ENDORSOR_DOCUMENT_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == ENDORSOR_AUDIO_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(selectedKeywordType == ENDORSOR_INDUSTRY_KEYWORDS_SELECTED) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kEndorsorAPI_Industry_Name] ;
            if ([selectedIndustryKeywordIds containsObject:[dictKeyword valueForKey:kEndorsorAPI_Industry_ID]]) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kEndorsorAPI_Target_Market_Name] ;
            
            if ([selectedTargetMarketKeywordIds containsObject:[dictKeyword valueForKey:kEndorsorAPI_Target_Market_ID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        } else
        {
            if (![searchBarKeywords.text isEqualToString:@""])
                ...
                dictKeyword = [endorsorKeywordsArray objectAtIndex:indexPath.row];
            
            cell.companyNameLbl.text = [dictKeyword valueForKey:kEndorsorAPI_KeywordName] ;
            
            if ([selectedEndorsorKeywordIds containsObject:[dictKeyword valueForKey:kEndorsorAPI_KeywordID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        if (![searchBarUsers.text isEqualToString:@""]) {
            ...
            cell.lblName.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kEndorsorAPI_User_Name]] ;
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kEndorsorAPI_User_Desc]];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kEndorsorAPI_User_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
        ...
        if(indexPath.section == sectionsArray.count) {
        ...
        else if(indexPath.section == ENDORSOR_FOLLOW_SECTION_INDEX) {
        ...
        else if(indexPath.section == ENDORSOR_INDUSTRY_KEYWORDS_SECTION_INDEX || indexPath.section == ENDORSOR_TARGET_MARKET_KEYWORDS_SECTION_INDEX || indexPath.section == ENDORSOR_KEYWORDS_SECTION_INDEX) {
            ...
            NSMutableArray *tags ;
            if(indexPath.section == ENDORSOR_INDUSTRY_KEYWORDS_SECTION_INDEX)
                tags = [selectedIndustryKeywordNames mutableCopy] ;
            else if(indexPath.section == ENDORSOR_TARGET_MARKET_KEYWORDS_SECTION_INDEX)
                tags = [selectedTargetMarketKeywordNames mutableCopy] ;
            else
                tags = [selectedEndorsorKeywordNames mutableCopy] ;
        ...
        else if(indexPath.section == ENDORSOR_DESCRIPTION_SECTION_INDEX) {
        ...
        else if(indexPath.section == ENDORSOR_START_DATE_SECTION_INDEX || indexPath.section == ENDORSOR_END_DATE_SECTION_INDEX) {
        ...
        else if(indexPath.section == ENDORSOR_IMAGE_SECTION_INDEX) {
        ...
        else if (indexPath.section == ENDORSOR_DOCUMENT_SECTION_INDEX || indexPath.section == ENDORSOR_AUDIO_SECTION_INDEX || indexPath.section == ENDORSOR_VIDEO_SECTION_INDEX) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(indexPath.section >= ENDORSOR_DOCUMENT_SECTION_INDEX && indexPath.section < sectionsArray.count) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if(indexPath.section == ENDORSOR_START_DATE_SECTION_INDEX || indexPath.section == ENDORSOR_END_DATE_SECTION_INDEX) {
            return 65;
        }
        else if (indexPath.section == ENDORSOR_FOLLOW_SECTION_INDEX)
            return 40;
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if(section <= ENDORSOR_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)return 0;
    else return 45 ;
}
