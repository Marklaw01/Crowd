...
-(void)addChatNotifications{
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(connectDisconnectChatNotification:)
                                                 name:kNotificationPush_QB_ConnectChat
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(connectDisconnectChatNotification:)
                                                 name:kNotificationPush_QB_DisconnectChat
                                               object:nil];
}

#pragma mark - Notifications
- (void)connectDisconnectChatNotification:(NSNotification *) notification
{
    if ([[notification name] isEqualToString:kNotificationPush_QB_ConnectChat]){
        if(ServicesManager.instance != nil)[ServicesManager.instance.chatService connectWithCompletionBlock:nil];
    }
    else if ([[notification name] isEqualToString:kNotificationPush_QB_DisconnectChat]){
        if(ServicesManager.instance != nil)[ServicesManager.instance.chatService disconnectWithCompletionBlock:nil];
    }
}
...
- (IBAction)GroupInfo_ClickAction:(id)sender {
    [self.view endEditing:YES] ;
   // if(usersArray)[usersArray removeAllObjects] ;
    [popupTblView reloadData] ;
    //[self getUsersInfo] ;
    if(self.dialog.type == QBChatDialogTypePrivate)groupNameLbl.text = @"Chat" ;
    else groupNameLbl.text = self.title ;
    [UtilityClass displayPopupWithContentView:popupView view:self.view] ;
}

- (IBAction)CloseInfoView_Click:(id)sender {
//    [popupView dismissPresentingPopup] ;
    [popupView removeFromSuperview];
}

#pragma mark - Chat Info Methods
-(void)getUsersInfo {
    usersArray = [[NSMutableArray alloc] init] ;
    [[[ServicesManager instance].usersService getUsersWithIDs:self.dialog.occupantIDs] continueWithSuccessBlock:^id _Nullable(BFTask<NSArray<QBUUser *> *> * _Nonnull task) {
        usersArray = [NSMutableArray arrayWithArray:task.result] ;
...
-(void)viewDidDisappear:(BOOL)animated{
     //[[NSNotificationCenter defaultCenter] removeObserver:self name:kNotificationPush_QB_ConnectChat object:nil];
    // [[NSNotificationCenter defaultCenter] removeObserver:self name:kNotificationPush_QB_DisconnectChat object:nil];
}

- (void)viewDidLoad {
    [super viewDidLoad];
    //[self addChatNotifications] ;
   ...
   // [self getUsersInfo] ;
    ...
    [self.inputToolbar.contentView.rightBarButtonItem setTitleColor:[UtilityClass blueColor] forState:UIControlStateNormal] ;
    ...
    self.collectionView.backgroundColor = [UtilityClass backgroundColor];
    self.inputToolbar.contentView.backgroundColor = [UtilityClass backgroundColor];
    ...
    self.title = [UtilityClass GetSelectedChatUserName];
...
- (void)refreshMessagesShowingProgress:(BOOL)showingProgress {
    ...
    if (showingProgress) {
        [UtilityClass showHudWithTitle:NSLocalizedString(@"SA_STR_LOADING_MESSAGES", nil)] ;
    ...
    [[ServicesManager instance].chatService messagesWithChatDialogID:self.dialog.ID completion:^(QBResponse *response, NSArray *messages) {
        if (response.success) {
            ...
            [UtilityClass showHudWithTitle:NSLocalizedString(@"SA_STR_ERROR", nil)] ;
...
- (void)readMessages:(NSArray *)messages {
    ...
    if ([ServicesManager instance].isAuthorized) {
        ...
        NSLog(@"readMessages >> %@",self.dialog) ;
...
- (NSAttributedString *)attributedStringForItem:(QBChatMessage *)messageItem {
    ...
    if (messageItem.isNotificationMessage) {
        textColor =  [UIColor blackColor];
    }
    else {
       textColor = [messageItem senderID] == self.senderID ? [UIColor whiteColor] : [UIColor whiteColor];
    }
...
- (NSAttributedString *)topLabelAttributedStringForItem:(QBChatMessage *)messageItem {
    ...
    NSLog(@"messageItem: %@",messageItem) ;
    
    UIFont *font = [UIFont fontWithName:@"HelveticaNeue-Medium" size:15.0f];
    ...
    if (self.dialog.type != QBChatDialogTypePrivate) {
		...
		if (messageSender) {
           ...
           // NSLog(@"usersArray: %@ messageItem: %@",usersArray,messageItem) ;
            topLabelText = [NSString stringWithFormat:@"@%lu",(unsigned long)messageItem.senderID];
            if(usersArray != nil && messageItem != nil){
                for (QBUUser *qbUser in usersArray) {
                    NSLog(@"senderID: %@ qbUser:%lu",messageItem,(unsigned long)qbUser.ID) ;
                    if(qbUser.ID == messageItem.senderID) {
                        NSLog(@"fullName: %@",qbUser.fullName) ;
                        topLabelText = [NSString stringWithFormat:@"%@",qbUser.fullName];
                        break ;
                    }
                }
            }
    ...
    NSDictionary *attributes = @{ NSForegroundColorAttributeName:[UtilityClass blueColor],
...
- (void)collectionView:(QMChatCollectionView *)collectionView configureCell:(UICollectionViewCell *)cell forIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cell isKindOfClass:[QMChatOutgoingCell class]] || [cell isKindOfClass:[QMChatAttachmentOutgoingCell class]]) {
        [chatCell containerView].bgColor = [UtilityClass greenColor];
        ...
        //[chatCell containerView].bgColor = [UIColor grayColor];
        //[chatCell containerView].bgColor = [UIColor colorWithRed:231.0f / 255.0f green:231.0f / 255.0f blue:231.0f / 255.0f alpha:1.0f];
        [chatCell containerView].bgColor = [UIColor colorWithRed:153.0f / 255.0f green:153.0f / 255.0f blue:153.0f / 255.0f alpha:1.0f];
    }
	...
	[[ServicesManager instance].chatService.chatAttachmentService imageForAttachmentMessage:message completion:^(NSError *error, UIImage *image) {
		...
		if (error != nil) {
            [UtilityClass showHudWithTitle:error.localizedDescription] ;
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    QBUUser *qbUser = (QBUUser*)[usersArray objectAtIndex:indexPath.row] ;
