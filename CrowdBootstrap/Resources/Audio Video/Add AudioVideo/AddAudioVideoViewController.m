...
#import "AddAudioVideoViewController.h"
...
@interface AddAudioVideoViewController ()

@end

@implementation AddAudioVideoViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Add Audio/Video";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Audio/Video Title", @"Audio/Video Description", @"Audio/Video Availability Start Date", @"Audio/Video Availability End Date", @"Target Market", @"Audio/Video Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kAudioVideoAPI_Title, kAudioVideoAPI_Description, kAudioVideoAPI_StartDate, kAudioVideoAPI_EndDate, kAddAudioVideoAPI_Target_Market_Keywords, kAddAudioVideoAPI_Keywords, kAddAudioVideoAPI_IndustryKeywords, kAudioVideoAPI_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kAudioVideoAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kAudioVideoAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kAudioVideoAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createAudioVideo {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addAudioVideoWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddAudioVideo_SuccessMessage withResultType:@"0" withDuration:1] ;
