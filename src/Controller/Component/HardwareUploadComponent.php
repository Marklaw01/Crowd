...
class HardwareUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('HardwareInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('HardwareKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('HardwareTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('HardwareFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['hardware_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('HardwareCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['hardware_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('HardwareCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['hardware_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/hardware/"; // Upload directory
                $imgCamp='';
                $errors='';
