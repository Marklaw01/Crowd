...
#import "AddWebinarViewController.h"
...
@interface AddWebinarViewController ()

@end

@implementation AddWebinarViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Add Webinar";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Webinar Title", @"Webinar Description", @"Webinar Availability Start Date", @"Webinar Availability End Date", @"Target Market", @"Webinar Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kWebinarAPI_Title, kWebinarAPI_Description, kWebinarAPI_StartDate, kWebinarAPI_EndDate, kAddWebinarAPI_Target_Market_Keywords, kAddWebinarAPI_Keywords, kAddWebinarAPI_IndustryKeywords, kWebinarAPI_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kWebinarAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kWebinarAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kWebinarAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createWebinar {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addWebinarWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddWebinar_SuccessMessage withResultType:@"0" withDuration:1] ;
