...
import com.crowdbootstrapapp.fragments.WorkOrderStartupEntrepreneurDetails;
 ...
 */
public abstract class WorkOrderEntrepreneurTableAdapter extends BaseTableAdapter {
     ...
     */
    public WorkOrderEntrepreneurTableAdapter(Context context) {
    ...
    public View getView(int row, int column, View converView, ViewGroup parent) {
        ...
        WorkOrderStartupEntrepreneurDetails.ViewHolder viewHolder;
        if (converView == null) {
            viewHolder = new WorkOrderStartupEntrepreneurDetails.ViewHolder();
            ...
            viewHolder = (WorkOrderStartupEntrepreneurDetails.ViewHolder) converView.getTag();
        }

        try {
            if (layout == R.layout.item_table1_header) {
                viewHolder.Text.setText(WorkOrderStartupEntrepreneurDetails.arrDeliverablesLabel.get(column).toString());
            } else if (layout == R.layout.item_table1) {
                if (row < 7) {

                    viewHolder.Text2.setText(WorkOrderStartupEntrepreneurDetails.arrDeliverablesworkUnitsLabel.get(((row) * getColumnCount()) + (column)).getWorkUnit());

                } else if (row == 7) {
                    viewHolder.Text2.setText(String.valueOf(WorkOrderStartupEntrepreneurDetails.sumOfColumnText[column]));
