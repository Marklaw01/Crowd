...
class BetaUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('BetaInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('BetaTestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('BetaTestTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('BetaTestFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['beta_test_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('BetaCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['beta_test_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('BetaCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['beta_test_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/beta_test/"; // Upload directory
                $imgCamp='';
                $errors='';
