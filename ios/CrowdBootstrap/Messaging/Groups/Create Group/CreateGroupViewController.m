...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Group Title", @"Group Description", @"Group Availability Start Date", @"Group Availability End Date", @"Target Market", @"Group Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kGroupAPI_Title, kGroupAPI_Description, kGroupAPI_StartDate, kGroupAPI_EndDate, kAddGroupAPI_Target_Market_Keywords, kAddGroupAPI_Keywords, kAddGroupAPI_IndustryKeywords, kGroupAPI_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kGroupAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kGroupAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kGroupAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createGroup {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addGroupWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddGroup_SuccessMessage withResultType:@"0" withDuration:1] ;
