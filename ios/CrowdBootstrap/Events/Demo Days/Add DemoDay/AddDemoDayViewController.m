...
#import "AddDemoDayViewController.h"
...
@interface AddDemoDayViewController ()

@end

@implementation AddDemoDayViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Add Demo Day";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Demo Day Title", @"Demo Day Description", @"Demo Day Start Date", @"Demo Day End Date", @"Target Market", @"Demo Day Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kDemoDayAPI_Title, kDemoDayAPI_Description, kDemoDayAPI_StartDate, kDemoDayAPI_EndDate, kAddDemoDayAPI_Target_Market_Keywords, kAddDemoDayAPI_Keywords, kAddDemoDayAPI_IndustryKeywords, kDemoDayAPI_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kDemoDayAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kDemoDayAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kDemoDayAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createDemoDay {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addDemoDayWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddDemoDay_SuccessMessage withResultType:@"0" withDuration:1] ;
