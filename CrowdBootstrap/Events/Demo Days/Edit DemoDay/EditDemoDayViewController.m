...
#import "EditDemoDayViewController.h"
...
@interface EditDemoDayViewController ()

@end

@implementation EditDemoDayViewController
...
-(void) awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(SetViewEditingNotification:) name:kNotificationSetDemoDayViewEditing
                                               object:nil];
...
-(void)SetViewEditingNotification:(NSNotification *)notification {
    if ([[notification name] isEqualToString:kNotificationSetDemoDayViewEditing]) {
...
-(void)resetUISettings {
    ...
    demoDayData = [[UtilityClass getDemoDayDetails] mutableCopy];
    NSLog(@"Demo Day Data: %@",demoDayData) ;
    ...
    self.title = [NSString stringWithFormat:@"%@",[demoDayData valueForKey:kDemoDayAPI_Title]] ;
    [self getDemoDayDetails] ;
...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Created By", @"Demo Day Title", @"Demo Day Description", @"Demo Day Start Date", @"Demo Day End Date", @"Target Market", @"Demo Day Keywords", @"Interest Keywords", @"Upload Image", @"View Document",@"Play Audio",@"Play Video"] ;
    NSArray *parametersArray = @[kDemoDayAPI_CreatedBy, kDemoDayAPI_Title, kDemoDayAPI_Description, kDemoDayAPI_StartDate, kDemoDayAPI_EndDate, kAddDemoDayAPI_Target_Market_Keywords, kAddDemoDayAPI_Keywords, kAddDemoDayAPI_IndustryKeywords, kDemoDayAPI_Image, kDemoDayAPI_Document, kDemoDayAPI_Audio, kDemoDayAPI_Video] ;
    ...
    [self getDemoDayKeywordsList];
    [self getDemoDayIndustryKeywordsList];
...
- (void)checkKeywordsListFound:(int)keywordType {
    switch (keywordType) {
        ...
        case 6:
        {
            if (demoDayKeywordsArray.count > 0)
                [self openTagsPopup] ;
            else
                [self presentViewController:[UtilityClass displayAlertMessage:kNoDemoDayKeywordsFoundMessage] animated:YES completion:nil];
        }
            break;
        default:
            break;
...
-(NSString*)convertTagsArrayToStringforArray:(NSMutableArray*)array withTagsArray:(NSArray*)tagsArray tagType:(NSString *)type {
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kDemoDayAPI_Target_Market_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kDemoDayAPI_Target_Market_ID])
                    selectedValue = [obj valueForKey:kDemoDayAPI_Target_Market_ID] ;
                else
                    selectedValue = [obj valueForKey:kDemoDayAPI_Target_Market_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag)
                        tagsStr = [dict valueForKey:kDemoDayAPI_Target_Market_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kDemoDayAPI_Target_Market_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
    } else if ([type isEqualToString:@"DemoDaykeyword"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kDemoDayAPI_KeywordID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kDemoDayAPI_KeywordID])
                    selectedValue = [obj valueForKey:kDemoDayAPI_KeywordID] ;
                else
                    selectedValue = [obj valueForKey:kDemoDayAPI_KeywordID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kDemoDayAPI_KeywordID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kDemoDayAPI_KeywordID]] ;
                    isFirstTag = NO ;
                }
            }
        }
        ...
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kDemoDayAPI_Industry_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kDemoDayAPI_Industry_ID])
                    selectedValue = [obj valueForKey:kDemoDayAPI_Industry_ID] ;
                else
                    selectedValue = [obj valueForKey:kDemoDayAPI_Industry_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kDemoDayAPI_Industry_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kDemoDayAPI_Industry_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
...
- (IBAction)CheckUncheck_ClickAction:(id)sender {
    ...
    if([btn.accessibilityLabel isEqualToString:CHECK_IMAGE ]) { // Check
        ...
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            if(selectedKeywordType == DEMODAY_INDUSTRY_KEYWORDS_SELECTED)
                dictKeyword = [industryKeywordsArray objectAtIndex:btn.tag];
            else if (selectedKeywordType == DEMODAY_KEYWORDS_SELECTED)
                dictKeyword = [demoDayKeywordsArray objectAtIndex:btn.tag];
            else
    ...
    if(selectedKeywordType == DEMODAY_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (NSMutableDictionary *obj in industryKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedIndustryKeywordNames addObject:[obj valueForKey:kDemoDayAPI_Industry_Name]] ;
                [selectedIndustryKeywordIds addObject:[obj valueForKey:kDemoDayAPI_Industry_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:DEMODAY_INDUSTRY_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        
    } else if(selectedKeywordType == DEMODAY_KEYWORDS_SELECTED) {
        [selectedDemoDayKeywordsArray removeAllObjects];
        [selectedDemoDayKeywordNames removeAllObjects];
        [selectedDemoDayKeywordIds removeAllObjects];
        
        for (NSMutableDictionary *obj in demoDayKeywordsArray) {
            NSString *isSelectedStr = [obj valueForKey:@"isSelected"] ;
            if([isSelectedStr isEqualToString:@"1"]) {
                [selectedDemoDayKeywordsArray addObject:obj] ;
                [selectedDemoDayKeywordNames addObject:[obj valueForKey:kDemoDayAPI_KeywordName]] ;
                [selectedDemoDayKeywordIds addObject:[obj valueForKey:kDemoDayAPI_KeywordID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:DEMODAY_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        ...
        for (NSMutableDictionary *obj in targetMarketKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedTargetMarketKeywordNames addObject:[obj valueForKey:kDemoDayAPI_Target_Market_Name]] ;
                [selectedTargetMarketKeywordIds addObject:[obj valueForKey:kDemoDayAPI_Target_Market_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:DEMODAY_TARGET_MARKET_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
    }
}
...
-(void)tagsControl:(UIView *)tag withTagControlIndex:(NSInteger)tagControlIndex removedFromIndex:(NSInteger)index {
    ...
    if(tagControlIndex == DEMODAY_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (int i=0; i<industryKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[industryKeywordsArray objectAtIndex:i] valueForKey:kDemoDayAPI_Industry_Name]] ;
    ...
    } else if(tagControlIndex == DEMODAY_KEYWORDS_SELECTED) {
        NSString *selectedName = [NSString stringWithFormat:@"%@",[selectedDemoDayKeywordNames objectAtIndex:index]] ;
        for (int i=0; i<demoDayKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[demoDayKeywordsArray objectAtIndex:i] valueForKey:kDemoDayAPI_KeywordName]] ;
            if([keywordName isEqualToString:selectedName]){
                [[demoDayKeywordsArray objectAtIndex:i] setValue:@"0" forKey:@"isSelected"] ;
                break ;
            }
        }
        [selectedDemoDayKeywordsArray removeObjectAtIndex:index] ;
        [selectedDemoDayKeywordNames removeObjectAtIndex:index];
        [selectedDemoDayKeywordIds removeObjectAtIndex:index];
        ...
        for (int i=0; i<targetMarketKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[targetMarketKeywordsArray objectAtIndex:i] valueForKey:kDemoDayAPI_Target_Market_Name]] ;
            if([keywordName isEqualToString:selectedName]){
...
- (IBAction)Back_Click:(id)sender {
    [UtilityClass setComingFrom_DemoDay_AddEditScreen:YES];
    [self.navigationController popViewControllerAnimated:YES] ;
}

- (IBAction)CloseData_ClickAction:(id)sender {
    [demoDayDataView dismissPresentingPopup] ;
}
...
- (IBAction)followButton_ClickAction:(UIButton*)button {
    ...
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kDemoDayAPI_FollowedBy] ;
        [dictParam setObject:[[[UtilityClass getDemoDayDetails] mutableCopy] valueForKey:kDemoDayAPI_ID] forKey:kDemoDayAPI_DemoDayID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isFollowed == 0) {
            [ApiCrowdBootstrap followDemoDayWithParameters:dictParam success:^(NSDictionary *responseDict) {
                ...
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    ...
                    [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:DEMODAY_FOLLOW_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationNone] ;
                }
            ...
            [ApiCrowdBootstrap unfollowDemoDayWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                    if(isFollowed == 1)
...
- (IBAction)Calender_ClickAction:(id)sender {
    ...
    if ([sender tag] == DEMODAY_START_DATE_SECTION_INDEX)
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:DEMODAY_START_DATE_SECTION_INDEX]] ;
    else
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:DEMODAY_END_DATE_SECTION_INDEX]] ;
...
- (IBAction)Submit_ClickAction:(id)sender {
    if ((selectedSegmentControl == 100 && selectedSegment == 0)) {
        ...
        if(![self validatetextFieldsWithSectionIndex:DEMODAY_TITLE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:DEMODAY_START_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:DEMODAY_END_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:DEMODAY_DESCRIPTION_SECTION_INDEX]) return ;
        ...
        else if(selectedDemoDayKeywordsArray.count < 1) {
        ...
        else [self editDemoDay] ;
    }
}

- (IBAction)demoDayCommitmentList_ClickAction:(id)sender {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    GraphicTableViewCell *cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:DEMODAY_IMAGE_SECTION_INDEX]] ;
...
- (IBAction)DatePickerToolbarButtons_ClickAction:(id)sender {
    ...
    if([sender tag] == DONE_CLICKED) {
        if(selectedDatePickerType == DEMODAY_START_DATE_SELECTED) {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:DEMODAY_START_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            cell.textFld.text = [dateFormatter stringFromDate:datePickerView.date];
            [[sectionsArray objectAtIndex:DEMODAY_START_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
        else {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:DEMODAY_END_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            ...
            [[sectionsArray objectAtIndex:DEMODAY_END_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
    }
    else {
        if(selectedDatePickerType == DEMODAY_START_DATE_SELECTED) {
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    switch (selectedKeywordType) {
        case 5: // Target Keywords
        {
            ...
            break;
        case 6: // Demo Day Keywords
        {
            for(NSDictionary *keywordDict in demoDayKeywordsArray)
            {
                NSString *keywordName = [keywordDict objectForKey:@"name"];
                NSRange range = [keywordName rangeOfString:searchText options:NSCaseInsensitiveSearch];
                if(range.location != NSNotFound)
            ...
            NSLog(@"Demo Day Results: %@",searchResultsForKeywords);
        }
            break;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    DobTableViewCell *startDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:DEMODAY_START_DATE_SECTION_INDEX]] ;
    
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        textField.userInteractionEnabled = true;
        if(textField.tag == DEMODAY_START_DATE_SECTION_INDEX || textField.tag == DEMODAY_END_DATE_SECTION_INDEX) {
            if(textField.tag == DEMODAY_START_DATE_SECTION_INDEX) {
                selectedDatePickerType = DEMODAY_START_DATE_SELECTED;
            }
            else {
                selectedDatePickerType = DEMODAY_END_DATE_SELECTED;
...
-(void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if(textField.tag == DEMODAY_START_DATE_SECTION_INDEX || textField.tag == DEMODAY_END_DATE_SECTION_INDEX) {
            if(textField.tag == DEMODAY_START_DATE_SECTION_INDEX) {
                DobTableViewCell *endDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:DEMODAY_END_DATE_SECTION_INDEX]] ;
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@"Demo Day Description"] && textView.textColor == [UIColor lightGrayColor]){
            textView.text = @"" ;
...
-(BOOL)textViewShouldEndEditing:(UITextView *)textView {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@""]){
            textView.text = @"Demo Day Description" ;
    ...
    if (indexPath.row == 0) {
        ...
        for (int i = 0; i < [sectionsArray count]; i++) {
            if (indexPath.section == i) {
                if (indexPath.section == DEMODAY_DOCUMENT_SECTION_INDEX) {
                    if (![docuementFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == DEMODAY_AUDIO_SECTION_INDEX) {
                    if (![audioFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == DEMODAY_VIDEO_SECTION_INDEX) {
...
-(void)getDemoDayKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getDemoDayKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [demoDayKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kDemoDayAPI_KeywordList]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kDemoDayAPI_KeywordList] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
                        [demoDayKeywordsArray addObject:obj] ;
                    }
                }
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode) {
...
-(void)getDemoDayIndustryKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getDemoDayIndustryKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [industryKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kDemoDayAPI_Industry_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kDemoDayAPI_Industry_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getTargetMarketKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getDemoDayTargetMarketKeywordsListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [targetMarketKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kDemoDayAPI_Target_Market_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kDemoDayAPI_Target_Market_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getDemoDayDetails {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kDemoDayAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[demoDayData valueForKey:kDemoDayAPI_ID]] forKey:kDemoDayAPI_DemoDayID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap viewDemoDayWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"respos %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                for ( int i = 0; i < sectionsArray.count ; i++ ) {
                    NSString *key = [[sectionsArray objectAtIndex:i] valueForKey:@"key"] ;
                    if(i != DEMODAY_INDUSTRY_KEYWORDS_SECTION_INDEX && i != DEMODAY_TARGET_MARKET_KEYWORDS_SECTION_INDEX && i != DEMODAY_KEYWORDS_SECTION_INDEX) {
                ...
                // Keywords Array
                if([responseDict objectForKey:kAddDemoDayAPI_IndustryKeywords]) {
                    selectedIndustryKeywordsArray = [[responseDict objectForKey:kAddDemoDayAPI_IndustryKeywords] mutableCopy];
                ...
                if([responseDict objectForKey:kAddDemoDayAPI_Keywords]) {
                    selectedDemoDayKeywordsArray = [[responseDict objectForKey:kAddDemoDayAPI_Keywords] mutableCopy] ;
                    selectedDemoDayKeywordNames = [self resetTagsArrayWithData:selectedDemoDayKeywordsArray];
                    selectedDemoDayKeywordIds = [self resetTagIdsArrayWithData:selectedDemoDayKeywordsArray];
                }
                if([responseDict objectForKey:kAddDemoDayAPI_Target_Market_Keywords]) {
                    selectedTargetMarketKeywordsArray = [[responseDict objectForKey:kAddDemoDayAPI_Target_Market_Keywords] mutableCopy];
                ...
                // Commits
                noOfcommit = [[responseDict valueForKey:kDemoDayAPI_NoOfCommits] stringValue];
                isCommited = [[responseDict valueForKey:kDemoDayAPI_IsCommited] intValue];
                isLiked = [[responseDict valueForKey:kDemoDayAPI_IsLiked] intValue];
                isFollowed = [[responseDict valueForKey:kDemoDayAPI_IsFollowed] intValue];
                
                // Document
                if([responseDict objectForKey:kDemoDayAPI_Document])
                    docuementFile = [[responseDict objectForKey:kDemoDayAPI_Document] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Audio
                if([responseDict objectForKey:kDemoDayAPI_Audio])
                    audioFile = [[responseDict objectForKey:kDemoDayAPI_Audio] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Video
                if([responseDict objectForKey:kDemoDayAPI_Video])
                    videoFile = [[responseDict objectForKey:kDemoDayAPI_Video] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Image
                if([responseDict objectForKey:kDemoDayAPI_Image]) {
                    NSString *strImage = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed, [responseDict objectForKey:kDemoDayAPI_Image]];
                    NSURL *url = [NSURL URLWithString:[strImage stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                    imgData = [NSData dataWithContentsOfURL:url];
                }
                
                // Refresh Dict
                [demoDayData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kDemoDayAPI_Title]] forKey:kDemoDayAPI_Title] ;
                [demoDayData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kDemoDayAPI_CreatedBy]] forKey:kDemoDayAPI_CreatedBy] ;
                [demoDayData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kDemoDayAPI_StartDate]] forKey:kDemoDayAPI_StartDate] ;
                [demoDayData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kDemoDayAPI_EndDate]] forKey:kDemoDayAPI_EndDate] ;
                [demoDayData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kDemoDayAPI_Description]] forKey:kDemoDayAPI_Description] ;
                
                [UtilityClass setDemoDayDetails:[demoDayData mutableCopy]] ;
...
-(void)commitDemoDay {
    if([UtilityClass checkInternetConnection]) {
        ...
        if (isCommited == 0) {
            [ApiCrowdBootstrap commitDemoDayWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
            ...
            [ApiCrowdBootstrap uncommitDemoDayWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
...
-(void)getDemoDayCommitmentList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getDemoDayCommitmentListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode)  {
...
- (void)getRecommendedUserList: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:@"DemoDay" forKey:@"type"] ;
        ...
        [ApiCrowdBootstrap getRegisteredRoleListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kDemoDayAPI_UserList]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]){
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kDemoDayAPI_UserList]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kDemoDayAPI_UserList]] ;
                    }
            ...
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
            {
                ...
                if(![searchBarUsers.text isEqualToString:@""])
                    ...
                    usersArray = [NSMutableArray arrayWithArray:[responseDict valueForKey:kDemoDayAPI_UserList]] ;
...
-(void)editDemoDay {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[[sectionsArray objectAtIndex:DEMODAY_TITLE_SECTION_INDEX] valueForKey:@"value"] forKey:kDemoDayAPI_Title] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:DEMODAY_DESCRIPTION_SECTION_INDEX] valueForKey:@"value"] forKey:kDemoDayAPI_Description] ;
        
        [dictParam setObject:[[sectionsArray objectAtIndex:DEMODAY_START_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kDemoDayAPI_StartDate] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:DEMODAY_END_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kDemoDayAPI_EndDate] ;
        
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedIndustryKeywordsArray withTagsArray:industryKeywordsArray tagType:@"industry"] forKey:kAddDemoDayAPI_IndustryKeywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedDemoDayKeywordsArray withTagsArray:demoDayKeywordsArray tagType:@"DemoDaykeyword"] forKey:kAddDemoDayAPI_Keywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedTargetMarketKeywordsArray withTagsArray:targetMarketKeywordsArray tagType:@"targetmarket"] forKey:kAddDemoDayAPI_Target_Market_Keywords] ;
        
        if(imgData)
            [dictParam setObject:imgData forKey:kDemoDayAPI_Image] ;
        else
            [dictParam setObject:@"" forKey:kDemoDayAPI_Image] ;
        
        [dictParam setObject:@"" forKey:kDemoDayAPI_Document] ;
        [dictParam setObject:@"" forKey:kDemoDayAPI_Audio] ;
        [dictParam setObject:@"" forKey:kDemoDayAPI_Video] ;
        ...
        [ApiCrowdBootstrap editDemoDayWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kEditDemoDay_SuccessMessage withResultType:@"0" withDuration:1] ;
                [UtilityClass setComingFrom_DemoDay_AddEditScreen:YES];
                [self.navigationController popViewControllerAnimated:YES] ;
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == popupTblView) {
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            else if (selectedKeywordType == DEMODAY_TARGET_MARKET_KEYWORDS_SELECTED)
                return targetMarketKeywordsArray.count ;
            else
                return demoDayKeywordsArray.count ;
        }
        ...
        if(section <= DEMODAY_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)
            return 1;
        else {
            if(section == DEMODAY_IMAGE_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue])
                    return 1;
                else return 0 ;
            }
            else if(section == DEMODAY_DOCUMENT_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == DEMODAY_AUDIO_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(selectedKeywordType == DEMODAY_INDUSTRY_KEYWORDS_SELECTED) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kDemoDayAPI_Industry_Name] ;
            if ([selectedIndustryKeywordIds containsObject:dictKeyword]) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kDemoDayAPI_Target_Market_Name] ;
            
            if ([selectedTargetMarketKeywordIds containsObject:[dictKeyword valueForKey:kDemoDayAPI_Target_Market_ID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        } else
        {
            if (![searchBarKeywords.text isEqualToString:@""])
                ...
                dictKeyword = [demoDayKeywordsArray objectAtIndex:indexPath.row];
            
            cell.companyNameLbl.text = [dictKeyword valueForKey:kDemoDayAPI_KeywordName] ;
            
            if ([selectedDemoDayKeywordIds containsObject:[dictKeyword valueForKey:kDemoDayAPI_KeywordID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        if (![searchBarUsers.text isEqualToString:@""]) {
            ...
            cell.lblName.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kDemoDayAPI_User_Name]] ;
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kDemoDayAPI_User_Desc]];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kDemoDayAPI_User_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
        ...
        if(indexPath.section == sectionsArray.count) {
        ...
        else if(indexPath.section == DEMODAY_FOLLOW_SECTION_INDEX) {
        ...
        else if(indexPath.section == DEMODAY_INDUSTRY_KEYWORDS_SECTION_INDEX || indexPath.section == DEMODAY_TARGET_MARKET_KEYWORDS_SECTION_INDEX || indexPath.section == DEMODAY_KEYWORDS_SECTION_INDEX) {
            ...
            NSMutableArray *tags ;
            if(indexPath.section == DEMODAY_INDUSTRY_KEYWORDS_SECTION_INDEX)
                tags = [selectedIndustryKeywordNames mutableCopy] ;
            else if(indexPath.section == DEMODAY_TARGET_MARKET_KEYWORDS_SECTION_INDEX)
                tags = [selectedTargetMarketKeywordNames mutableCopy] ;
            else
                tags = [selectedDemoDayKeywordNames mutableCopy] ;
        ...
        else if(indexPath.section == DEMODAY_DESCRIPTION_SECTION_INDEX) {
        ...
        else if(indexPath.section == DEMODAY_START_DATE_SECTION_INDEX || indexPath.section == DEMODAY_END_DATE_SECTION_INDEX) {
        ...
        else if(indexPath.section == DEMODAY_IMAGE_SECTION_INDEX) {
        ...
        else if (indexPath.section == DEMODAY_DOCUMENT_SECTION_INDEX || indexPath.section == DEMODAY_AUDIO_SECTION_INDEX || indexPath.section == DEMODAY_VIDEO_SECTION_INDEX) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(indexPath.section >= DEMODAY_DOCUMENT_SECTION_INDEX && indexPath.section < sectionsArray.count) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if(indexPath.section == DEMODAY_START_DATE_SECTION_INDEX || indexPath.section == DEMODAY_END_DATE_SECTION_INDEX) {
            return 65;
        }
        else if (indexPath.section == DEMODAY_FOLLOW_SECTION_INDEX)
            return 40;
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if(section <= DEMODAY_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)return 0;
    else return 45 ;
}
