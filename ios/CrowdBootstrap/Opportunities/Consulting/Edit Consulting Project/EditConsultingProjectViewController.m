//
//  EditConsultingProjectViewController.m
//  CrowdBootstrap
//
...
#import "EditConsultingProjectViewController.h"
...
@interface EditConsultingProjectViewController ()

@end

@implementation EditConsultingProjectViewController
...
-(void) awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(SetViewEditingNotification:) name:kNotificationSetConsultingViewEditing
                                               object:nil];
...
-(void)SetViewEditingNotification:(NSNotification *)notification {
    if ([[notification name] isEqualToString:kNotificationSetConsultingViewEditing]) {
...
-(void)resetUISettings {
    ...
    NSLog(@"Consulting Project Data: %@",consultingProjectData) ;
...
//    }
    ...
    self.title = [NSString stringWithFormat:@"%@",[consultingProjectData valueForKey:kConsultingAPI_Title]] ;
    [self getConsultingDetails] ;
...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Created By", @"Consulting Project Title", @"Project Overview", @"Description/Conditions", @"Target User Keywords", @"Interest Keywords", @"Target Date to Distribute Project Overview", @"Deadline for Intent to Bid", @"Target Date to Distribute Project Requirements", @"Deadline for Commitment to Bid", @"Deadline for Questions", @"Target Date for Answers", @"Deadline for Proposals", @"Target Date for Bidder Presentations", @"Target Date to Award the Project", @"Target Project Start Date", @"Target Project End Date", @"Upload Image", @"View Document",@"Play Audio",@"Play Video", @"Questions", @"Final Bid"] ;
    NSArray *parametersArray = @[kConsultingAPI_CreatedBy,
                                 kConsultingAPI_Title,
                                 ...
                                 kConsultingAPI_Document,
                                 kConsultingAPI_Audio,
                                 kConsultingAPI_Video,
                                 kConsultingAPI_Question,
                                 kConsultingAPI_FinalBid] ;
    ...
    if ([type isEqualToString:@"targetuser"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                ...
                if ([obj valueForKey:kConsultingAPI_Target_User_ID])
                    selectedValue = [obj valueForKey:kConsultingAPI_Target_User_ID] ;
                else
                    selectedValue = [obj valueForKey:kConsultingAPI_Target_User_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kConsultingAPI_Interest_ID])
                    selectedValue = [obj valueForKey:kConsultingAPI_Interest_ID] ;
                else
                    selectedValue = [obj valueForKey:kConsultingAPI_Interest_ID] ;
...
- (IBAction)CloseData_ClickAction:(id)sender {
    [consultingProjectDataView dismissPresentingPopup] ;
}
...
- (IBAction)followButton_ClickAction:(UIButton*)button {
    ...
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kConsultingAPI_FollowedBy] ;
        [dictParam setObject:[[[UtilityClass getConsultingDetails] mutableCopy] valueForKey:kConsultingAPI_ID] forKey:kConsultingAPI_ConsultingID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isFollowed == 0) {
            [ApiCrowdBootstrap followConsultingWithParameters:dictParam success:^(NSDictionary *responseDict) {
                ...
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    ...
                    [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:CONSULTING_FOLLOW_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationNone] ;
                }
            ...
            [ApiCrowdBootstrap unfollowConsultingWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                    if(isFollowed == 1)
...
- (IBAction)Submit_ClickAction:(id)sender {
    if ((selectedSegmentControl == 100 && selectedSegment == 0)) {
        ...
        // if validation is successful, hit api to edit consulting project
        else [self editConsulting] ;
    }
}

- (IBAction)ConsultingCommitmentList_ClickAction:(id)sender {
    ...
    if ([noOfcommit integerValue] > 0) {
        [self getConsultingCommitmentList];
    }
}
...
- (IBAction)searchRecommendedUser_ClickAction:(id)sender {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        lblSearchContractor.hidden = true;
    }
    
    [self getRecommendedUserList:@""];
}

- (IBAction)invite_ClickAction:(id)sender {
    UIButton *button = (UIButton *)sender;
    rowIndex = (int)[button tag];
    pageNo = 1;
    ...
    searchBarUsers.hidden = false;
    lblPeople.hidden = true;
    lblSearchContractor.hidden = false;
    
    [self searchContractors:@""];
}

- (IBAction)viewProfile_ClickAction:(id)sender {
    if ([userType isEqualToString:@"Contractor"]) {
        // Hit API - sendConsultingInvitation
        if(![searchBarUsers.text isEqualToString:@""]) {
            [self sendConsultingInvitation:[[searchResultsForRegisteredUsers objectAtIndex:[sender tag]] valueForKey:kConsultingAPI_User_ID]];

        } else {
            [self sendConsultingInvitation:[[usersArray objectAtIndex:[sender tag]] valueForKey:kConsultingAPI_User_ID]];
        }
    } else {
...
/*
 ...
 DobTableViewCell *startDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:CONSULTING_START_DATE_SECTION_INDEX]] ;
 DobTableViewCell *endDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:CONSULTING_END_DATE_SECTION_INDEX]] ;
 ...
 [[sectionsArray objectAtIndex:CONSULTING_START_DATE_SECTION_INDEX] setValue:startDatecell.textFld.text forKey:@"value"] ;
 } else {
 ...
 [[sectionsArray objectAtIndex:CONSULTING_END_DATE_SECTION_INDEX] setValue:endDatecell.textFld.text forKey:@"value"] ;
 } else {
 NSComparisonResult result;
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    if (searchBar.tag == 10) {
        ...
        if ([userType isEqualToString:@"Contractor"])
            [self searchContractors:searchText];
        else
...
-(void)getConsultingDetails {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kConsultingAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[consultingProjectData valueForKey:kConsultingAPI_ID]] forKey:kConsultingAPI_ConsultingID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap viewConsultingWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"respos %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                for ( int i = 0; i < sectionsArray.count ; i++ ) {
                    NSString *key = [[sectionsArray objectAtIndex:i] valueForKey:@"key"] ;
                    if(i != CONSULTING_INTEREST_KEYWORDS_SECTION_INDEX && i != CONSULTING_TARGET_USER_KEYWORDS_SECTION_INDEX) {
                ...
                // Keywords Array
                if([responseDict objectForKey:kAddConsultingAPI_InterestKeywords]) {
                    selectedInterestKeywordsArray = [[responseDict objectForKey:kAddConsultingAPI_InterestKeywords] mutableCopy];
                    selectedInterestKeywordNames = [self resetTagsArrayWithData:selectedInterestKeywordsArray];
                    selectedInterestKeywordIds = [self resetTagIdsArrayWithData:selectedInterestKeywordsArray];
                }
                if([responseDict objectForKey:kAddConsultingAPI_Target_User_Keywords]) {
                    selectedTargetUserKeywordsArray = [[responseDict objectForKey:kAddConsultingAPI_Target_User_Keywords] mutableCopy];
                    selectedTargetUserKeywordNames = [self resetTagsArrayWithData:selectedTargetUserKeywordsArray];
                    selectedTargetUserKeywordIds = [self resetTagIdsArrayWithData:selectedTargetUserKeywordsArray];
                }
                
                // Commits
                noOfcommit = [[responseDict valueForKey:kConsultingAPI_NoOfCommits] stringValue];
                isCommited = [[responseDict valueForKey:kConsultingAPI_IsCommited] intValue];
                isLiked = [[responseDict valueForKey:kConsultingAPI_IsLiked] intValue];
                isFollowed = [[responseDict valueForKey:kConsultingAPI_IsFollowed] intValue];
                
                // Document
                if([responseDict objectForKey:kConsultingAPI_Document])
                    docuementFile = [[responseDict objectForKey:kConsultingAPI_Document] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Audio
                if([responseDict objectForKey:kConsultingAPI_Audio])
                    audioFile = [[responseDict objectForKey:kConsultingAPI_Audio] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Video
                if([responseDict objectForKey:kConsultingAPI_Video])
                    videoFile = [[responseDict objectForKey:kConsultingAPI_Video] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Questions
                if([responseDict objectForKey:kConsultingAPI_Question])
                    questionFile = [[responseDict objectForKey:kConsultingAPI_Question] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Final Bid
                if([responseDict objectForKey:kConsultingAPI_FinalBid])
                    finalBidFile = [[responseDict objectForKey:kConsultingAPI_FinalBid] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Image
                if([responseDict objectForKey:kConsultingAPI_Image]) {
                    NSString *strImage = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed, [responseDict objectForKey:kConsultingAPI_Image]];
                    NSURL *url = [NSURL URLWithString:[strImage stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                    imgData = [NSData dataWithContentsOfURL:url];
                }
                
                // Refresh Dict
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Title]] forKey:kConsultingAPI_Title] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_CreatedBy]] forKey:kConsultingAPI_CreatedBy] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Overview]] forKey:kConsultingAPI_Overview] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Description]] forKey:kConsultingAPI_Description] ;

                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Project_Overview_Date]] forKey:kConsultingAPI_Project_Overview_Date] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Bid_Intent_Deadline_Date]] forKey:kConsultingAPI_Bid_Intent_Deadline_Date] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Req_Dist_Date]] forKey:kConsultingAPI_Req_Dist_Date] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Bid_Comm_Deadline_Date]] forKey:kConsultingAPI_Bid_Comm_Deadline_Date] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Ques_Deadline_Date]] forKey:kConsultingAPI_Ques_Deadline_Date] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Answer_Target_Date]] forKey:kConsultingAPI_Answer_Target_Date] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Proposal_Submit_Date]] forKey:kConsultingAPI_Proposal_Submit_Date] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Bidder_Pres_Date]] forKey:kConsultingAPI_Bidder_Pres_Date] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Project_Award_Date]] forKey:kConsultingAPI_Project_Award_Date] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Project_Start_Date]] forKey:kConsultingAPI_Project_Start_Date] ;
                [consultingProjectData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kConsultingAPI_Project_Complete_Date]] forKey:kConsultingAPI_Project_Complete_Date] ;

                [UtilityClass setConsultingDetails:[consultingProjectData mutableCopy]] ;
...
-(void)commitConsulting {
    if([UtilityClass checkInternetConnection]) {
        ...
        if (isCommited == 0) {
            [ApiCrowdBootstrap commitConsultingWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
            ...
            [ApiCrowdBootstrap uncommitConsultingWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
...
-(void)getConsultingCommitmentList {
    if([UtilityClass checkInternetConnection]) {
...
- (void)getRecommendedUserList: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:@"consulting" forKey:@"type"] ;
        ...
        [ApiCrowdBootstrap getRegisteredRoleListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kConsultingAPI_UserList]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]){
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kConsultingAPI_UserList]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kConsultingAPI_UserList]] ;
                    }
...
- (void)searchContractors: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kConsultingAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%d",pageNo] forKey:kConsultingAPI_PageNo] ;
        [dictParam setObject:searchedString forKey:kConsultingAPI_SearchText] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[consultingProjectData valueForKey:kConsultingAPI_ID]] forKey:kConsultingAPI_ConsultingID] ;

        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap searchConsultingContractorsWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kRecommendedContAPI_Contractors]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]) {
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kRecommendedContAPI_Contractors]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kRecommendedContAPI_Contractors]] ;
                    }
                    lblNoUserAvailable.hidden = true;
                    pageNo++ ;
            ...
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
            {
                if(![searchBarUsers.text isEqualToString:@""])
                    ...
                    usersArray = [NSMutableArray arrayWithArray:[responseDict valueForKey:kRecommendedContAPI_Contractors]] ;
...
-(void)editConsulting {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap editConsultingWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kEditConsultingProject_SuccessMessage withResultType:@"0" withDuration:1] ;
                [UtilityClass setComingFrom_Consulting_AddEditScreen:YES];
                [self.navigationController popViewControllerAnimated:YES] ;
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
...
- (void)sendConsultingInvitation: (NSString *)contractorID {
    if([UtilityClass checkInternetConnection]) {
        ...
        NSMutableDictionary *dictParam = [[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kConsultingAPI_InvitationSentBy] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[consultingProjectData valueForKey:kConsultingAPI_ID]] forKey:kConsultingAPI_ConsultingID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",contractorID] forKey:kConsultingAPI_ContractorID] ;
        
        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap sendConsultingInvitationWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            [self presentViewController:[UtilityClass displayAlertMessage:[responseDict valueForKey:@"message"]] animated:YES completion:nil];
            pageNo = 1;
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == popupTblView) {
        ...
        if(section <= CONSULTING_PROJECT_COMPLETE_TARGET_DATE_SECTION_INDEX || section == sectionsArray.count)
            ...
            if(section == CONSULTING_IMAGE_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else return 0 ;
            }
            else if(section == CONSULTING_DOCUMENT_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == CONSULTING_AUDIO_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == CONSULTING_VIDEO_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == CONSULTING_QUESTIONS_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(tableView == popupTblView) {
        ...
        if (![searchBarUsers.text isEqualToString:@""]) {
            ...
            cell.lblName.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kConsultingAPI_User_Name]] ;
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kConsultingAPI_User_Desc]];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kConsultingAPI_User_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
            ...
            if ([userType isEqualToString:@"Contractor"]) {
                //is_invitation_sent
                if ([[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:@"is_invitation_sent"] intValue] == 0) {
                    [cell.btnViewProfile setTitle:@"Invite" forState:UIControlStateNormal];
                    cell.btnViewProfile.userInteractionEnabled = true;
                }
                else {
                    [cell.btnViewProfile setTitle:@"Invited" forState:UIControlStateNormal];
                    cell.btnViewProfile.userInteractionEnabled = false;
                }
            }
            else
                [cell.btnViewProfile setTitle:@"View Profile" forState:UIControlStateNormal];
            ...
            if ([userType isEqualToString:@"Contractor"]) {
                //is_invitation_sent
                if ([[[usersArray objectAtIndex:indexPath.row] valueForKey:@"is_invitation_sent"] intValue] == 0) {
                    [cell.btnViewProfile setTitle:@"Invite" forState:UIControlStateNormal];
                    cell.btnViewProfile.userInteractionEnabled = true;
                }
                else {
...
//                }
                ...
                cell.commitBtn.userInteractionEnabled = false;
                cell.commitBtn.alpha = 0.7;
                cell.inviteBtn.hidden = false;
                cell.progressLbl.hidden = true;
                cell.progressView.hidden = true;
                cell.fileTypeLbl.hidden = true;
            } else {
                
                cell.commitBtn.userInteractionEnabled = true;
                cell.commitBtn.alpha = 1.0;

                [cell.commitBtn setTitle:@"Submit" forState:UIControlStateNormal];
                cell.inviteBtn.hidden = true;
                cell.progressLbl.hidden = false;
                cell.progressView.hidden = false;
                cell.fileTypeLbl.hidden = false;
            }
            return cell ;
        }
        else if(indexPath.section == CONSULTING_FOLLOW_SECTION_INDEX) {
        ...
        else if (indexPath.section == CONSULTING_DOCUMENT_SECTION_INDEX || indexPath.section == CONSULTING_AUDIO_SECTION_INDEX || indexPath.section == CONSULTING_VIDEO_SECTION_INDEX || indexPath.section == CONSULTING_QUESTIONS_SECTION_INDEX || indexPath.section == CONSULTING_FINALBID_SECTION_INDEX) {
            ...
            if(indexPath.section == CONSULTING_DOCUMENT_SECTION_INDEX)
                cell.lbl.text = [NSString stringWithFormat:@"Docuement %ld",indexPath.row+1] ;
            else if(indexPath.section == CONSULTING_AUDIO_SECTION_INDEX)
                cell.lbl.text = [NSString stringWithFormat:@"Audio %ld",indexPath.row+1] ;
            else if(indexPath.section == CONSULTING_VIDEO_SECTION_INDEX)
                cell.lbl.text = [NSString stringWithFormat:@"Video %ld",indexPath.row+1] ;
            else if(indexPath.section == CONSULTING_QUESTIONS_SECTION_INDEX)
                cell.lbl.text = [NSString stringWithFormat:@"Question %ld",indexPath.row+1] ;
            else
                cell.lbl.text = [NSString stringWithFormat:@"Final Bid %ld",indexPath.row+1] ;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        [viewPopUp setHidden:true];
        [_tblView setUserInteractionEnabled:true];
        ...
        NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
        if(![searchBarUsers.text isEqualToString:@""])
            [dict setValue:[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kConsultingAPI_User_ID] forKey:kRecommendedContAPI_ContractorID] ;
        else
            [dict setValue:[[usersArray objectAtIndex:indexPath.row] valueForKey:kConsultingAPI_User_ID] forKey:kRecommendedContAPI_ContractorID] ;
        ...
        if(indexPath.section >= CONSULTING_DOCUMENT_SECTION_INDEX && indexPath.section < sectionsArray.count) {
            ...
            if(indexPath.section == CONSULTING_DOCUMENT_SECTION_INDEX) {
                ...
                filePath = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed,questionFile] ;
                dataTitle.text = @"Questions" ;
            }
            else {
                filePath = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed,finalBidFile] ;
                dataTitle.text = @"Final Bid" ;
            }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if (indexPath.section == CONSULTING_FOLLOW_SECTION_INDEX)
            return 40;
        else if(indexPath.section == CONSULTING_DESCRIPTION_SECTION_INDEX || indexPath.section == CONSULTING_PROJECT_OVERVIEW_SECTION_INDEX)
            return 121 ;
        else if(indexPath.section == CONSULTING_IMAGE_SECTION_INDEX)
