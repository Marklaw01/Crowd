...
class EarlyUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('EarlyAdopterInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('EarlyAdopterKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('EarlyAdopterTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('EarlyAdopterFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['early_adopter_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('EarlyCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['early_adopter_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('EarlyCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['early_adopter_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/early_adopter/"; // Upload directory
                $imgCamp='';
                $errors='';
