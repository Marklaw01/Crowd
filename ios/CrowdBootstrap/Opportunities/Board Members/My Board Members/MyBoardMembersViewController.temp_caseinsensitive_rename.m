...
-(void)resetUISettings {
    ...
    // Hit Service to get My BetaTest List
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == tblViewPopUp) {
        ...
        if(indexPath.row == betaTestsArray.count) {
            ...
            if (betaTestSearchController.active && ![betaTestSearchController.searchBar.text isEqualToString:@""]) {
                dicData = [searchResults objectAtIndex:indexPath.row];
            } else {
                dicData = [betaTestsArray objectAtIndex:indexPath.row];
            }
            cell.lblTitle.text = [NSString stringWithFormat:@"%@",[dicData valueForKey:kBetaTestAPI_BetaTest_Title]] ;
            cell.lblPostedOn.text = [NSString stringWithFormat:@"%@",[dicData valueForKey:kBetaTestAPI_StartDate]];
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[dicData valueForKey:kBetaTestAPI_Description]];
            
            [cell.btnLikeCount setTitle:[NSString stringWithFormat:@"%@",[dicData valueForKey:kBetaTestAPI_BetaTest_Likes]] forState:UIControlStateNormal];
            [cell.btnDislikeCount setTitle:[NSString stringWithFormat:@"%@",[dicData valueForKey:kBetaTestAPI_BetaTest_Dislikes]] forState:UIControlStateNormal];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[dicData valueForKey:kBetaTestAPI_BetaTest_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
