...
class LaunchUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('LaunchDealInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('LaunchDealKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('LaunchDealTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('LaunchDealFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['launchdeal_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('LaunchDealCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['launchdeal_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('LaunchDealCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['launchdeal_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/launchdeal/"; // Upload directory
                $imgCamp='';
                $errors='';
