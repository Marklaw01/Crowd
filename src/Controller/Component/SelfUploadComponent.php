...
class SelfUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('SelfInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('SelfKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('SelfTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('SelfFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['self_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('SelfCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['self_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('SelfCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['self_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/self/"; // Upload directory
                $imgCamp='';
                $errors='';
