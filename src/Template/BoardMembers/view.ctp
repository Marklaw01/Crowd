  ...
  <div class="row">
    <div class="col-lg-12 ">
      <ol class="breadcrumb">
        ...
        ?></li>
        <li class="active">View Board Member</li>
      </ol>
    </div>
  </div>
  <!-- breadcrumb ends --> 
      ...
      ?>
        <div class='col-lg-6 col-md-6 col-sm-12 '>
          <div class="form-group pull-right">
            ...
            ?>
            <button class="customBtn <?php echo $class;?>" type="button" onclick="location.href='<?php echo $CommitLink;?>'" style="padding: 12px 40px !important;"><?php echo $titleCommit;?></button>
          </div>
        </div>
      <?php } ?>
    </div>
    ...
    <div class='col-lg-4 col-md-4 col-sm-4 '>
      <div class="form-group">
        <span class="contentListingLikes">
          <h2 class="likes">
            <?php 
              $likeC = $this->Custom->getBoardLikeCount($fundDeatils->id);
              if(!empty($likeC)){

                  echo $this->Html->link($likeC, array('controller'=>'BoardMembers','action'=>'likeList',base64_encode($fundDeatils->id)), array('escape'=>false,'title'=>'View','style'=>'color: rgb(3, 66, 110);'));
            ...
            &nbsp;
            <?php 
            if($this->Custom->isBoardLikedbyUser($fundDeatils->id,$userId)){
            ...
            &nbsp;
            <?php 
              $disLikeC = $this->Custom->getBoardDislikeCount($fundDeatils->id);
              if(!empty($disLikeC)){

                  echo $this->Html->link($disLikeC, array('controller'=>'BoardMembers','action'=>'dislikeList',base64_encode($fundDeatils->id)), array('escape'=>false,'title'=>'View','style'=>'color: rgb(3, 66, 110);'));
            ...
            &nbsp;
            <?php 
              if($this->Custom->isBoardDislikedbyUser($fundDeatils->id,$userId)){
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 ">
                ...
                if(!empty($skey4[0])){
                  for($ij=0; $ij<$cskey4; $ij++){
                      $slids4=$skey4[$ij];
                         ...
                         echo 'No Target Market Keywords Added';
                }
          ?>
          </ul>
        </span>
      ...
      <div class="form-group">
        ...
        <span id="selectedResult" class="form-control textArea">
          <ul>
          <?php
            $skey5= explode(',', $fundDeatils->board_oppertunity_keywords_id);
              $cskey5=count($skey5);
                if(!empty($skey5[0])){
                  for($ij=0; $ij<$cskey5; $ij++){
                      $slids5=$skey5[$ij];
                         ...
                         echo 'NO Board Membership Keywords Added';
                }
          ?>
          </ul>
        </span>
          ...
          <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
              <div class="form-group">
                ...
                <span id="selectedResult" class="form-control textArea">
                  <ul>
                  <?php
                    $skey3= explode(',', $fundDeatils->board_interest_keywords_id);
                      $cskey3=count($skey3);
                        if(!empty($skey3[0])){
                  ...
                  <div class="panel-body">
                    <div class="image-Holder">
                      <?php if(!empty($fundDeatils->image)){?>
                        <img src="<?php echo $this->request->webroot.'img/board_member/'.$fundDeatils->image;?>" alt="Fund Image">
                      <?php } ?>
                    </div>
                    </div>
                </div>
                   ...
                   <ul class="listing anchor">
                    <?php 
                        if(!empty($fundDeatils->document)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/board_member/'.$fundDeatils->document.'" target="_blank"> Document <i class="fa fa-eye"></i></a>';
                   ...
                   <ul class="listing anchor">
                     <?php 
                        if(!empty($fundDeatils->audio)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/board_member/'.$fundDeatils->audio.'" target="_blank"> Audio <i class="fa fa-eye"></i></a>';
                   ...
                   <ul class="listing anchor">
                      <?php 
                        if(!empty($fundDeatils->video)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/board_member/'.$fundDeatils->video.'" target="_blank"> Video <i class="fa fa-eye"></i></a>';
            ...
            if($userId != $fundDeatils->user_id){
              ...
              if(!empty($is_follwed_by_user)){
                  ...
                  $titleFollow='Unfollow';
                    $followLink= $this->Url->build(["controller" => "BoardMembers","action" => "unfollow",base64_encode($fundDeatils->id),base64_encode($userId)]);                      
              }else {
                $class1= 'blueBtn';
                $titleFollow='Follow';
                    $followLink= $this->Url->build(["controller" => "BoardMembers","action" => "follow",base64_encode($fundDeatils->id),base64_encode($userId)]);   
              }
              ...
              if(!empty($is_commited_by_user)){
                $class= 'greenBtn';
                  $titleCommit='Thank you. You have registered your interest.';
                    $CommitLink= $this->Url->build(["controller" => "BoardMembers","action" => "uncommit",base64_encode($fundDeatils->id),base64_encode($userId)]);                      
              }else {
                $class= 'blueBtn';
                $titleCommit='Click here to register your interest.';
                    $CommitLink= $this->Url->build(["controller" => "BoardMembers","action" => "commit",base64_encode($fundDeatils->id),base64_encode($userId)]);   
              }
