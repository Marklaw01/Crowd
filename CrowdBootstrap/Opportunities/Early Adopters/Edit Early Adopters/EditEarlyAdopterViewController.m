//
//  EditEarlyAdopterViewController.m
//  CrowdBootstrap
//
...
#import "EditEarlyAdopterViewController.h"
...
@interface EditEarlyAdopterViewController ()

@end

@implementation EditEarlyAdopterViewController
...
-(void) awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(SetViewEditingNotification:) name:kNotificationSetEarlyAdopterViewEditing
                                               object:nil];
...
-(void)SetViewEditingNotification:(NSNotification *)notification {
    if ([[notification name] isEqualToString:kNotificationSetEarlyAdopterViewEditing]) {
...
-(void)resetUISettings {
    ...
    earlyAdopterData = [[UtilityClass getEarlyAdopterDetails] mutableCopy];
    NSLog(@"Early Adopter Data: %@",earlyAdopterData) ;
    ...
    self.title = [NSString stringWithFormat:@"%@",[earlyAdopterData valueForKey:kEarlyAdopterAPI_EarlyAdopter_Title]] ;
    [self getEarlyAdopterDetails] ;
...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Created By", @"Early Adopter Title", @"Early Adopter Description", @"Early Adopter Start Date", @"Early Adopter End Date", @"Target Market", @"Early Adopter Keywords", @"Interest Keywords", @"Upload Image", @"View Document",@"Play Audio",@"Play Video"] ;
    NSArray *parametersArray = @[kEarlyAdopterAPI_CreatedBy, kEarlyAdopterAPI_EarlyAdopter_Title, kEarlyAdopterAPI_Description, kEarlyAdopterAPI_StartDate, kEarlyAdopterAPI_EndDate, kAddEarlyAdopterAPI_Target_Market_Keywords, kAddEarlyAdopterAPI_Keywords, kAddEarlyAdopterAPI_IndustryKeywords, kEarlyAdopterAPI_EarlyAdopter_Image, kEarlyAdopterAPI_EarlyAdopter_Document, kEarlyAdopterAPI_EarlyAdopter_Audio, kEarlyAdopterAPI_EarlyAdopter_Video] ;
    ...
    [self getEarlyAdopterKeywordsList];
    [self getEarlyAdopterIndustryKeywordsList];
...
- (void)checkKeywordsListFound:(int)keywordType {
    switch (keywordType) {
        ...
        case 6:
        {
            if (earlyAdopterKeywordsArray.count > 0)
                [self openTagsPopup] ;
            else
                [self presentViewController:[UtilityClass displayAlertMessage:kNoEarlyAdopterKeywordsFoundMessage] animated:YES completion:nil];
        }
            break;
        default:
            break;
...
-(NSString*)convertTagsArrayToStringforArray:(NSMutableArray*)array withTagsArray:(NSArray*)tagsArray tagType:(NSString *)type {
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kEarlyAdopterAPI_Target_Market_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kEarlyAdopterAPI_Target_Market_ID])
                    selectedValue = [obj valueForKey:kEarlyAdopterAPI_Target_Market_ID] ;
                else
                    selectedValue = [obj valueForKey:kEarlyAdopterAPI_Target_Market_ID] ;

                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag)
                        tagsStr = [dict valueForKey:kEarlyAdopterAPI_Target_Market_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kEarlyAdopterAPI_Target_Market_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
    } else if ([type isEqualToString:@"earlyAdopterkeyword"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kEarlyAdopterAPI_KeywordID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kEarlyAdopterAPI_KeywordID])
                    selectedValue = [obj valueForKey:kEarlyAdopterAPI_KeywordID] ;
                else
                    selectedValue = [obj valueForKey:kEarlyAdopterAPI_KeywordID] ;

                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kEarlyAdopterAPI_KeywordID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kEarlyAdopterAPI_KeywordID]] ;
                    isFirstTag = NO ;
                }
            }
        }
        ...
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kEarlyAdopterAPI_Industry_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kEarlyAdopterAPI_Industry_ID])
                    selectedValue = [obj valueForKey:kEarlyAdopterAPI_Industry_ID] ;
                else
                    selectedValue = [obj valueForKey:kEarlyAdopterAPI_Industry_ID] ;

                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kEarlyAdopterAPI_Industry_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kEarlyAdopterAPI_Industry_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
...
- (IBAction)CheckUncheck_ClickAction:(id)sender {
    ...
    if([btn.accessibilityLabel isEqualToString:CHECK_IMAGE ]) { // Check
        ...
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            if(selectedKeywordType == EARLY_ADOPTER_INDUSTRY_KEYWORDS_SELECTED)
                dictKeyword = [industryKeywordsArray objectAtIndex:btn.tag];
            else if (selectedKeywordType == EARLY_ADOPTER_KEYWORDS_SELECTED)
                dictKeyword = [earlyAdopterKeywordsArray objectAtIndex:btn.tag];
            else
    ...
    if(selectedKeywordType == EARLY_ADOPTER_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (NSMutableDictionary *obj in industryKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedIndustryKeywordNames addObject:[obj valueForKey:kEarlyAdopterAPI_Industry_Name]] ;
                [selectedIndustryKeywordIds addObject:[obj valueForKey:kEarlyAdopterAPI_Industry_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_INDUSTRY_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        
    } else if(selectedKeywordType == EARLY_ADOPTER_KEYWORDS_SELECTED) {
        [selectedEarlyAdopterKeywordsArray removeAllObjects];
        [selectedEarlyAdopterKeywordNames removeAllObjects];
        [selectedEarlyAdopterKeywordIds removeAllObjects];
        
        for (NSMutableDictionary *obj in earlyAdopterKeywordsArray) {
            NSString *isSelectedStr = [obj valueForKey:@"isSelected"] ;
            if([isSelectedStr isEqualToString:@"1"]) {
                [selectedEarlyAdopterKeywordsArray addObject:obj] ;
                [selectedEarlyAdopterKeywordNames addObject:[obj valueForKey:kEarlyAdopterAPI_KeywordName]] ;
                [selectedEarlyAdopterKeywordIds addObject:[obj valueForKey:kEarlyAdopterAPI_KeywordID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        ...
        for (NSMutableDictionary *obj in targetMarketKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedTargetMarketKeywordNames addObject:[obj valueForKey:kEarlyAdopterAPI_Target_Market_Name]] ;
                [selectedTargetMarketKeywordIds addObject:[obj valueForKey:kEarlyAdopterAPI_Target_Market_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_TARGET_MARKET_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
    }
}
...
-(void)tagsControl:(UIView *)tag withTagControlIndex:(NSInteger)tagControlIndex removedFromIndex:(NSInteger)index {
    ...
    if(tagControlIndex == EARLY_ADOPTER_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (int i=0; i<industryKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[industryKeywordsArray objectAtIndex:i] valueForKey:kEarlyAdopterAPI_Industry_Name]] ;
    ...
    } else if(tagControlIndex == EARLY_ADOPTER_KEYWORDS_SELECTED) {
        NSString *selectedName = [NSString stringWithFormat:@"%@",[selectedEarlyAdopterKeywordNames objectAtIndex:index]] ;
        for (int i=0; i<earlyAdopterKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[earlyAdopterKeywordsArray objectAtIndex:i] valueForKey:kEarlyAdopterAPI_KeywordName]] ;
            if([keywordName isEqualToString:selectedName]){
                [[earlyAdopterKeywordsArray objectAtIndex:i] setValue:@"0" forKey:@"isSelected"] ;
                break ;
            }
        }
        [selectedEarlyAdopterKeywordsArray removeObjectAtIndex:index] ;
        [selectedEarlyAdopterKeywordNames removeObjectAtIndex:index];
        [selectedEarlyAdopterKeywordIds removeObjectAtIndex:index];
        ...
        for (int i=0; i<targetMarketKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[targetMarketKeywordsArray objectAtIndex:i] valueForKey:kEarlyAdopterAPI_Target_Market_Name]] ;
            if([keywordName isEqualToString:selectedName]){
...
- (IBAction)Back_Click:(id)sender {
    [UtilityClass setComingFrom_EarlyAdopter_AddEditScreen:YES];
    [self.navigationController popViewControllerAnimated:YES] ;
}

- (IBAction)CloseData_ClickAction:(id)sender {
    [earlyAdopterDataView dismissPresentingPopup] ;
}
...
- (IBAction)followButton_ClickAction:(UIButton*)button {
    ...
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kEarlyAdopterAPI_EarlyAdopter_FollowedBy] ;
        [dictParam setObject:[[[UtilityClass getEarlyAdopterDetails] mutableCopy] valueForKey:kEarlyAdopterAPI_ID] forKey:kEarlyAdopterAPI_EarlyAdopterID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isFollowed == 0) {
            [ApiCrowdBootstrap followEarlyAdopterWithParameters:dictParam success:^(NSDictionary *responseDict) {
                ...
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    ...
                    [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_FOLLOW_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationNone] ;
                }
            ...
            [ApiCrowdBootstrap unfollowEarlyAdopterWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                    if(isFollowed == 1)
...
- (IBAction)Calender_ClickAction:(id)sender {
    ...
    if ([sender tag] == EARLY_ADOPTER_START_DATE_SECTION_INDEX)
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_START_DATE_SECTION_INDEX]] ;
    else
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_END_DATE_SECTION_INDEX]] ;
...
- (IBAction)Submit_ClickAction:(id)sender {
    if ((selectedSegmentControl == 100 && selectedSegment == 0)) {
        [self commitEarlyAdopter];
    } else {
        if(![self validatetextFieldsWithSectionIndex:EARLY_ADOPTER_TITLE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:EARLY_ADOPTER_START_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:EARLY_ADOPTER_END_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:EARLY_ADOPTER_DESCRIPTION_SECTION_INDEX]) return ;
        ...
        else if(selectedEarlyAdopterKeywordsArray.count < 1) {
        ...
        else [self editEarlyAdopter] ;
    }
}

- (IBAction)earlyAdopterCommitmentList_ClickAction:(id)sender {
...
//    imgVwPeople.image = [UIImage imageNamed:@"like_green"];
    ...
    if ([noOfcommit integerValue] > 0) {
        [self getEarlyAdopterCommitmentList];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    GraphicTableViewCell *cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_IMAGE_SECTION_INDEX]] ;
...
- (IBAction)DatePickerToolbarButtons_ClickAction:(id)sender {
    ...
    if([sender tag] == DONE_CLICKED) {
        if(selectedDatePickerType == EARLY_ADOPTER_START_DATE_SELECTED) {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_START_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            cell.textFld.text = [dateFormatter stringFromDate:datePickerView.date];
            [[sectionsArray objectAtIndex:EARLY_ADOPTER_START_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
        else {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_END_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            ...
            [[sectionsArray objectAtIndex:EARLY_ADOPTER_END_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
    }
    else {
        if(selectedDatePickerType == EARLY_ADOPTER_START_DATE_SELECTED) {
    ...
    if (searchBar.tag == 10) {
        ...
        switch (selectedKeywordType) {
            case 5: // Target Keywords
            {
                ...
                break;
            case 6: // Early Adopter Keywords
            {
                for(NSDictionary *keywordDict in earlyAdopterKeywordsArray)
                {
                    NSString *keywordName = [keywordDict objectForKey:@"name"];
                    NSRange range = [keywordName rangeOfString:searchText options:NSCaseInsensitiveSearch];
                    if(range.location != NSNotFound)
                ...
                NSLog(@"Early Adopter Results: %@",searchResultsForKeywords);
            }
                break;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    DobTableViewCell *startDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_START_DATE_SECTION_INDEX]] ;
    
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        textField.userInteractionEnabled = true;
        if(textField.tag == EARLY_ADOPTER_START_DATE_SECTION_INDEX || textField.tag == EARLY_ADOPTER_END_DATE_SECTION_INDEX) {
            if(textField.tag == EARLY_ADOPTER_START_DATE_SECTION_INDEX) {
                selectedDatePickerType = EARLY_ADOPTER_START_DATE_SELECTED;
            }
            else {
                selectedDatePickerType = EARLY_ADOPTER_END_DATE_SELECTED;
...
-(void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if(textField.tag == EARLY_ADOPTER_START_DATE_SECTION_INDEX || textField.tag == EARLY_ADOPTER_END_DATE_SECTION_INDEX) {
            if(textField.tag == EARLY_ADOPTER_START_DATE_SECTION_INDEX) {
                DobTableViewCell *endDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:EARLY_ADOPTER_END_DATE_SECTION_INDEX]] ;
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@"Early Adopter Description"] && textView.textColor == [UIColor lightGrayColor]){
            textView.text = @"" ;
...
-(BOOL)textViewShouldEndEditing:(UITextView *)textView {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@""]){
            textView.text = @"Early Adopter Description" ;
    ...
    if (indexPath.row == 0) {
        ...
        for (int i = 0; i < [sectionsArray count]; i++) {
            if (indexPath.section == i) {
                if (indexPath.section == EARLY_ADOPTER_DOCUMENT_SECTION_INDEX) {
                    if (![docuementFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == EARLY_ADOPTER_AUDIO_SECTION_INDEX) {
                    if (![audioFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == EARLY_ADOPTER_VIDEO_SECTION_INDEX) {
...
-(void)getEarlyAdopterKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getEarlyAdopterKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [earlyAdopterKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kEarlyAdopterAPI_KeywordList]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kEarlyAdopterAPI_KeywordList] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
                        [earlyAdopterKeywordsArray addObject:obj] ;
                    }
                }
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode) {
...
-(void)getEarlyAdopterIndustryKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getEarlyAdopterIndustryKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [industryKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kEarlyAdopterAPI_Industry_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kEarlyAdopterAPI_Industry_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getTargetMarketKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getEarlyAdopterTargetMarketKeywordsListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [targetMarketKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kEarlyAdopterAPI_Target_Market_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kEarlyAdopterAPI_Target_Market_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)commitEarlyAdopter {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kEarlyAdopterAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[earlyAdopterData valueForKey:kEarlyAdopterAPI_ID]] forKey:kEarlyAdopterAPI_EarlyAdopterID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isCommited == 0) {
            [ApiCrowdBootstrap commitEarlyAdopterWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
            ...
            [ApiCrowdBootstrap uncommitEarlyAdopterWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
...
-(void)getEarlyAdopterCommitmentList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getEarlyAdopterCommitmentListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode)  {
...
-(void)getEarlyAdopterDetails {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap viewEarlyAdopterWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"respos %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                for ( int i = 0; i < sectionsArray.count ; i++ ) {
                    NSString *key = [[sectionsArray objectAtIndex:i] valueForKey:@"key"] ;
                    if(i != EARLY_ADOPTER_INDUSTRY_KEYWORDS_SECTION_INDEX && i != EARLY_ADOPTER_TARGET_MARKET_KEYWORDS_SECTION_INDEX && i != EARLY_ADOPTER_KEYWORDS_SECTION_INDEX) {
                ...
                // Keywords Array
                if([responseDict objectForKey:kAddEarlyAdopterAPI_IndustryKeywords]) {
                    selectedIndustryKeywordsArray = [[responseDict objectForKey:kAddEarlyAdopterAPI_IndustryKeywords] mutableCopy];
                ...
                if([responseDict objectForKey:kAddEarlyAdopterAPI_Keywords]) {
                    selectedEarlyAdopterKeywordsArray = [[responseDict objectForKey:kAddEarlyAdopterAPI_Keywords] mutableCopy] ;
                    selectedEarlyAdopterKeywordNames = [self resetTagsArrayWithData:selectedEarlyAdopterKeywordsArray];
                    selectedEarlyAdopterKeywordIds = [self resetTagIdsArrayWithData:selectedEarlyAdopterKeywordsArray];
                }
                if([responseDict objectForKey:kAddEarlyAdopterAPI_Target_Market_Keywords]) {
                    selectedTargetMarketKeywordsArray = [[responseDict objectForKey:kAddEarlyAdopterAPI_Target_Market_Keywords] mutableCopy];
                ...
                // Commits
                noOfcommit = [[responseDict valueForKey:kEarlyAdopterAPI_NoOfCommits] stringValue];
                isCommited = [[responseDict valueForKey:kEarlyAdopterAPI_IsCommited] intValue];
                isLiked = [[responseDict valueForKey:kEarlyAdopterAPI_IsLiked] intValue];
                isFollowed = [[responseDict valueForKey:kEarlyAdopterAPI_IsFollowed] intValue];

                // Document
                if([responseDict objectForKey:kEarlyAdopterAPI_EarlyAdopter_Document])
                    docuementFile = [[responseDict objectForKey:kEarlyAdopterAPI_EarlyAdopter_Document] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Audio
                if([responseDict objectForKey:kEarlyAdopterAPI_EarlyAdopter_Audio])
                    audioFile = [[responseDict objectForKey:kEarlyAdopterAPI_EarlyAdopter_Audio] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Video
                if([responseDict objectForKey:kEarlyAdopterAPI_EarlyAdopter_Video])
                    videoFile = [[responseDict objectForKey:kEarlyAdopterAPI_EarlyAdopter_Video] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Image
                if([responseDict objectForKey:kEarlyAdopterAPI_EarlyAdopter_Image]) {
                    NSString *strImage = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed, [responseDict objectForKey:kEarlyAdopterAPI_EarlyAdopter_Image]];
                    NSURL *url = [NSURL URLWithString:[strImage stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                    imgData = [NSData dataWithContentsOfURL:url];
                }
                
                // Refresh Dict
                [earlyAdopterData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kEarlyAdopterAPI_EarlyAdopter_Title]] forKey:kEarlyAdopterAPI_EarlyAdopter_Title] ;
                [earlyAdopterData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kEarlyAdopterAPI_CreatedBy]] forKey:kEarlyAdopterAPI_CreatedBy] ;
                [earlyAdopterData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kEarlyAdopterAPI_StartDate]] forKey:kEarlyAdopterAPI_StartDate] ;
                [earlyAdopterData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kEarlyAdopterAPI_EndDate]] forKey:kEarlyAdopterAPI_EndDate] ;
                [earlyAdopterData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kEarlyAdopterAPI_Description]] forKey:kEarlyAdopterAPI_Description] ;
                
                [UtilityClass setEarlyAdopterDetails:[earlyAdopterData mutableCopy]] ;
...
- (void)getRecommendedUserList: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:@"early_adopter" forKey:@"type"] ;
        ...
        [ApiCrowdBootstrap getRegisteredRoleListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kEarlyAdopterAPI_UserList]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]){
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kEarlyAdopterAPI_UserList]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kEarlyAdopterAPI_UserList]] ;
                    }
            ...
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
            {
                ...
                if(![searchBarUsers.text isEqualToString:@""])
                    ...
                    usersArray = [NSMutableArray arrayWithArray:[responseDict valueForKey:kEarlyAdopterAPI_UserList]] ;
...
-(void)editEarlyAdopter {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[[sectionsArray objectAtIndex:EARLY_ADOPTER_TITLE_SECTION_INDEX] valueForKey:@"value"] forKey:kEarlyAdopterAPI_EarlyAdopter_Title] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:EARLY_ADOPTER_DESCRIPTION_SECTION_INDEX] valueForKey:@"value"] forKey:kEarlyAdopterAPI_Description] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:EARLY_ADOPTER_START_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kEarlyAdopterAPI_StartDate] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:EARLY_ADOPTER_END_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kEarlyAdopterAPI_EndDate] ;
        
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedIndustryKeywordsArray withTagsArray:industryKeywordsArray tagType:@"industry"] forKey:kAddEarlyAdopterAPI_IndustryKeywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedEarlyAdopterKeywordsArray withTagsArray:earlyAdopterKeywordsArray tagType:@"earlyAdopterkeyword"] forKey:kAddEarlyAdopterAPI_Keywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedTargetMarketKeywordsArray withTagsArray:targetMarketKeywordsArray tagType:@"targetmarket"] forKey:kAddEarlyAdopterAPI_Target_Market_Keywords] ;
        
        if(imgData)
            [dictParam setObject:imgData forKey:kEarlyAdopterAPI_EarlyAdopter_Image] ;
        else
            [dictParam setObject:@"" forKey:kEarlyAdopterAPI_EarlyAdopter_Image] ;
        
        [dictParam setObject:@"" forKey:kEarlyAdopterAPI_EarlyAdopter_Document] ;
        [dictParam setObject:@"" forKey:kEarlyAdopterAPI_EarlyAdopter_Audio] ;
        [dictParam setObject:@"" forKey:kEarlyAdopterAPI_EarlyAdopter_Video] ;
        ...
        [ApiCrowdBootstrap editEarlyAdopterWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kEditEarlyAdopter_SuccessMessage withResultType:@"0" withDuration:1] ;
                [UtilityClass setComingFrom_EarlyAdopter_AddEditScreen:YES];
                [self.navigationController popViewControllerAnimated:YES] ;
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == popupTblView) {
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            else if (selectedKeywordType == EARLY_ADOPTER_TARGET_MARKET_KEYWORDS_SELECTED)
                return targetMarketKeywordsArray.count ;
            else
                return earlyAdopterKeywordsArray.count ;
        }
        ...
        if(section <= EARLY_ADOPTER_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)
            return 1;
        else {
            if(section == EARLY_ADOPTER_IMAGE_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue])
                    return 1;
                else return 0 ;
            }
            else if(section == EARLY_ADOPTER_DOCUMENT_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == EARLY_ADOPTER_AUDIO_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(tableView == popupTblView) {
        ...
        if(selectedKeywordType == EARLY_ADOPTER_INDUSTRY_KEYWORDS_SELECTED) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kEarlyAdopterAPI_Industry_Name] ;
            if ([selectedIndustryKeywordIds containsObject:[dictKeyword valueForKey:kEarlyAdopterAPI_Industry_ID]]) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kEarlyAdopterAPI_Target_Market_Name] ;
            
            if ([selectedTargetMarketKeywordIds containsObject:[dictKeyword valueForKey:kEarlyAdopterAPI_Target_Market_ID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        } else
        {
            if (![searchBarKeywords.text isEqualToString:@""])
                ...
                dictKeyword = [earlyAdopterKeywordsArray objectAtIndex:indexPath.row];

            cell.companyNameLbl.text = [dictKeyword valueForKey:kEarlyAdopterAPI_KeywordName] ;
            
            if ([selectedEarlyAdopterKeywordIds containsObject:[dictKeyword valueForKey:kEarlyAdopterAPI_KeywordID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        if (![searchBarUsers.text isEqualToString:@""]) {
            ...
            cell.lblName.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kEarlyAdopterAPI_User_Name]] ;
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kEarlyAdopterAPI_User_Desc]];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kEarlyAdopterAPI_User_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
        ...
        if(indexPath.section == sectionsArray.count) {
        ...
        else if(indexPath.section == EARLY_ADOPTER_FOLLOW_SECTION_INDEX) {
        ...
        else if(indexPath.section == EARLY_ADOPTER_INDUSTRY_KEYWORDS_SECTION_INDEX || indexPath.section == EARLY_ADOPTER_TARGET_MARKET_KEYWORDS_SECTION_INDEX || indexPath.section == EARLY_ADOPTER_KEYWORDS_SECTION_INDEX) {
            ...
            NSMutableArray *tags ;
            if(indexPath.section == EARLY_ADOPTER_INDUSTRY_KEYWORDS_SECTION_INDEX)
                tags = [selectedIndustryKeywordNames mutableCopy] ;
            else if(indexPath.section == EARLY_ADOPTER_TARGET_MARKET_KEYWORDS_SECTION_INDEX)
                tags = [selectedTargetMarketKeywordNames mutableCopy] ;
            else
                tags = [selectedEarlyAdopterKeywordNames mutableCopy] ;
        ...
        else if(indexPath.section == EARLY_ADOPTER_DESCRIPTION_SECTION_INDEX) {
        ...
        else if(indexPath.section == EARLY_ADOPTER_START_DATE_SECTION_INDEX || indexPath.section == EARLY_ADOPTER_END_DATE_SECTION_INDEX) {
        ...
        else if(indexPath.section == EARLY_ADOPTER_IMAGE_SECTION_INDEX) {
        ...
        else if (indexPath.section == EARLY_ADOPTER_DOCUMENT_SECTION_INDEX || indexPath.section == EARLY_ADOPTER_AUDIO_SECTION_INDEX || indexPath.section == EARLY_ADOPTER_VIDEO_SECTION_INDEX) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(indexPath.section >= EARLY_ADOPTER_DOCUMENT_SECTION_INDEX && indexPath.section < sectionsArray.count) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if(indexPath.section == EARLY_ADOPTER_START_DATE_SECTION_INDEX || indexPath.section == EARLY_ADOPTER_END_DATE_SECTION_INDEX) {
            return 65;
        }
        else if (indexPath.section == EARLY_ADOPTER_FOLLOW_SECTION_INDEX)
            return 40;
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if(section <= EARLY_ADOPTER_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)return 0;
    else return 45 ;
}
