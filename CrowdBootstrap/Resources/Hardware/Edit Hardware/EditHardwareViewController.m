...
#import "EditHardwareViewController.h"
...
@interface EditHardwareViewController ()

@end

@implementation EditHardwareViewController
...
-(void) awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(SetViewEditingNotification:) name:kNotificationSetHardwareViewEditing
                                               object:nil];
...
-(void)SetViewEditingNotification:(NSNotification *)notification {
    if ([[notification name] isEqualToString:kNotificationSetHardwareViewEditing]) {
...
-(void)resetUISettings {
    ...
    self.title = [NSString stringWithFormat:@"%@",[hardwareData valueForKey:kHardwareAPI_Hardware_Title]] ;
    [self getHardwareDetails] ;
...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Created By", @"Hardware Title", @"Hardware Description", @"Hardware Availability Start Date", @"Hardware Availability End Date", @"Target Market", @"Hardware Keywords", @"Interest Keywords", @"Upload Image", @"View Document",@"Play Audio",@"Play Video"] ;
    NSArray *parametersArray = @[kHardwareAPI_CreatedBy, kHardwareAPI_Hardware_Title, kHardwareAPI_Description, kHardwareAPI_StartDate, kHardwareAPI_EndDate, kAddHardwareAPI_Target_Market_Keywords, kAddHardwareAPI_Keywords, kAddHardwareAPI_IndustryKeywords, kHardwareAPI_Hardware_Image, kHardwareAPI_Hardware_Document, kHardwareAPI_Hardware_Audio, kHardwareAPI_Hardware_Video] ;
    ...
    [self getHardwareKeywordsList];
    [self getHardwareIndustryKeywordsList];
...
- (void)checkKeywordsListFound:(int)keywordType {
    switch (keywordType) {
        ...
        case 6:
        {
            if (hardwareKeywordsArray.count > 0)
                [self openTagsPopup] ;
            else
                [self presentViewController:[UtilityClass displayAlertMessage:kNoHardwareKeywordsFoundMessage] animated:YES completion:nil];
        }
            break;
        default:
            break;
...
-(NSString*)convertTagsArrayToStringforArray:(NSMutableArray*)array withTagsArray:(NSArray*)tagsArray tagType:(NSString *)type {
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kHardwareAPI_Target_Market_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kHardwareAPI_Target_Market_ID])
                    selectedValue = [obj valueForKey:kHardwareAPI_Target_Market_ID] ;
                else
                    selectedValue = [obj valueForKey:kHardwareAPI_Target_Market_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag)
                        tagsStr = [dict valueForKey:kHardwareAPI_Target_Market_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kHardwareAPI_Target_Market_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
    } else if ([type isEqualToString:@"hardwarekeyword"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kHardwareAPI_KeywordID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kHardwareAPI_KeywordID])
                    selectedValue = [obj valueForKey:kHardwareAPI_KeywordID] ;
                else
                    selectedValue = [obj valueForKey:kHardwareAPI_KeywordID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kHardwareAPI_KeywordID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kHardwareAPI_KeywordID]] ;
                    isFirstTag = NO ;
                }
            }
        }
        ...
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kHardwareAPI_Industry_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kHardwareAPI_Industry_ID])
                    selectedValue = [obj valueForKey:kHardwareAPI_Industry_ID] ;
                else
                    selectedValue = [obj valueForKey:kHardwareAPI_Industry_ID] ;
                
                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kHardwareAPI_Industry_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kHardwareAPI_Industry_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
...
- (IBAction)CheckUncheck_ClickAction:(id)sender {
    ...
    if([btn.accessibilityLabel isEqualToString:CHECK_IMAGE ]) { // Check
        ...
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            if(selectedKeywordType == HARDWARE_INDUSTRY_KEYWORDS_SELECTED)
                dictKeyword = [industryKeywordsArray objectAtIndex:btn.tag];
            else if (selectedKeywordType == HARDWARE_KEYWORDS_SELECTED)
                dictKeyword = [hardwareKeywordsArray objectAtIndex:btn.tag];
            else
...
//        else if (selectedKeywordType == HARDWARE_KEYWORDS_SELECTED)
//            [[hardwareKeywordsArray objectAtIndex:btn.tag] setValue:@"1" forKey:@"isSelected"];
//        else
    ...
    if(selectedKeywordType == HARDWARE_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (NSMutableDictionary *obj in industryKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedIndustryKeywordNames addObject:[obj valueForKey:kHardwareAPI_Industry_Name]] ;
                [selectedIndustryKeywordIds addObject:[obj valueForKey:kHardwareAPI_Industry_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:HARDWARE_INDUSTRY_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        
    } else if(selectedKeywordType == HARDWARE_KEYWORDS_SELECTED) {
        [selectedHardwareKeywordsArray removeAllObjects];
        [selectedHardwareKeywordNames removeAllObjects];
        [selectedHardwareKeywordIds removeAllObjects];
        
        for (NSMutableDictionary *obj in hardwareKeywordsArray) {
            NSString *isSelectedStr = [obj valueForKey:@"isSelected"] ;
            if([isSelectedStr isEqualToString:@"1"]) {
                [selectedHardwareKeywordsArray addObject:obj] ;
                [selectedHardwareKeywordNames addObject:[obj valueForKey:kHardwareAPI_KeywordName]] ;
                [selectedHardwareKeywordIds addObject:[obj valueForKey:kHardwareAPI_KeywordID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:HARDWARE_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        ...
        for (NSMutableDictionary *obj in targetMarketKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedTargetMarketKeywordNames addObject:[obj valueForKey:kHardwareAPI_Target_Market_Name]] ;
                [selectedTargetMarketKeywordIds addObject:[obj valueForKey:kHardwareAPI_Target_Market_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:HARDWARE_TARGET_MARKET_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
    }
}
...
-(void)tagsControl:(UIView *)tag withTagControlIndex:(NSInteger)tagControlIndex removedFromIndex:(NSInteger)index {
    ...
    if(tagControlIndex == HARDWARE_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (int i=0; i<industryKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[industryKeywordsArray objectAtIndex:i] valueForKey:kHardwareAPI_Industry_Name]] ;
    ...
    } else if(tagControlIndex == HARDWARE_KEYWORDS_SELECTED) {
        NSString *selectedName = [NSString stringWithFormat:@"%@",[selectedHardwareKeywordNames objectAtIndex:index]] ;
        for (int i=0; i<hardwareKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[hardwareKeywordsArray objectAtIndex:i] valueForKey:kHardwareAPI_KeywordName]] ;
            if([keywordName isEqualToString:selectedName]){
                [[hardwareKeywordsArray objectAtIndex:i] setValue:@"0" forKey:@"isSelected"] ;
                break ;
            }
        }
        [selectedHardwareKeywordsArray removeObjectAtIndex:index] ;
        [selectedHardwareKeywordNames removeObjectAtIndex:index];
        [selectedHardwareKeywordIds removeObjectAtIndex:index];
        ...
        for (int i=0; i<targetMarketKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[targetMarketKeywordsArray objectAtIndex:i] valueForKey:kHardwareAPI_Target_Market_Name]] ;
            if([keywordName isEqualToString:selectedName]){
...
- (IBAction)Back_Click:(id)sender {
    [UtilityClass setComingFrom_Hardware_AddEditScreen:YES];
    [self.navigationController popViewControllerAnimated:YES] ;
}

- (IBAction)CloseData_ClickAction:(id)sender {
    [hardwareDataView dismissPresentingPopup] ;
}
...
- (IBAction)followButton_ClickAction:(UIButton*)button {
    ...
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kHardwareAPI_Hardware_FollowedBy] ;
        [dictParam setObject:[[[UtilityClass getHardwareDetails] mutableCopy] valueForKey:kHardwareAPI_ID] forKey:kHardwareAPI_HardwareID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isFollowed == 0) {
            [ApiCrowdBootstrap followHardwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
                ...
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    ...
                    [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:HARDWARE_FOLLOW_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationNone] ;
                }
            ...
            [ApiCrowdBootstrap unfollowHardwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                    if(isFollowed == 1)
...
- (IBAction)Calender_ClickAction:(id)sender {
    ...
    if ([sender tag] == HARDWARE_START_DATE_SECTION_INDEX)
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:HARDWARE_START_DATE_SECTION_INDEX]] ;
    else
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:HARDWARE_END_DATE_SECTION_INDEX]] ;
...
- (IBAction)Submit_ClickAction:(id)sender {
    if ((selectedSegmentControl == 100 && selectedSegment == 0)) {
        ...
        if(![self validatetextFieldsWithSectionIndex:HARDWARE_TITLE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:HARDWARE_START_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:HARDWARE_END_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:HARDWARE_DESCRIPTION_SECTION_INDEX]) return ;
        ...
        else if(selectedHardwareKeywordsArray.count < 1) {
        ...
        else [self editHardware] ;
    }
}

- (IBAction)hardwareCommitmentList_ClickAction:(id)sender {
    ...
    if ([noOfcommit integerValue] > 0) {
        [self getHardwareCommitmentList];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    GraphicTableViewCell *cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:HARDWARE_IMAGE_SECTION_INDEX]] ;
...
- (IBAction)DatePickerToolbarButtons_ClickAction:(id)sender {
    ...
    if([sender tag] == DONE_CLICKED) {
        if(selectedDatePickerType == HARDWARE_START_DATE_SELECTED) {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:HARDWARE_START_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            cell.textFld.text = [dateFormatter stringFromDate:datePickerView.date];
            [[sectionsArray objectAtIndex:HARDWARE_START_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
        else {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:HARDWARE_END_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            ...
            [[sectionsArray objectAtIndex:HARDWARE_END_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
    }
    else {
        if(selectedDatePickerType == HARDWARE_START_DATE_SELECTED) {
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    switch (selectedKeywordType) {
        case 5: // Target Keywords
        {
            ...
            break;
        case 6: // Hardware Keywords
        {
            for(NSDictionary *keywordDict in hardwareKeywordsArray)
            {
                NSString *keywordName = [keywordDict objectForKey:@"name"];
                NSRange range = [keywordName rangeOfString:searchText options:NSCaseInsensitiveSearch];
                if(range.location != NSNotFound)
            ...
            NSLog(@"HArdware Results: %@",searchResultsForKeywords);
        }
            break;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    DobTableViewCell *startDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:HARDWARE_START_DATE_SECTION_INDEX]] ;
    
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        textField.userInteractionEnabled = true;
        if(textField.tag == HARDWARE_START_DATE_SECTION_INDEX || textField.tag == HARDWARE_END_DATE_SECTION_INDEX) {
            if(textField.tag == HARDWARE_START_DATE_SECTION_INDEX) {
                selectedDatePickerType = HARDWARE_START_DATE_SELECTED;
            }
            else {
                selectedDatePickerType = HARDWARE_END_DATE_SELECTED;
...
-(void)textFieldDidBeginEditing:(UITextField *)textField {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if(textField.tag == HARDWARE_START_DATE_SECTION_INDEX || textField.tag == HARDWARE_END_DATE_SECTION_INDEX) {
            if(textField.tag == HARDWARE_START_DATE_SECTION_INDEX) {
                DobTableViewCell *endDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:HARDWARE_END_DATE_SECTION_INDEX]] ;
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@"Hardware Description"] && textView.textColor == [UIColor lightGrayColor]){
            textView.text = @"" ;
...
-(BOOL)textViewShouldEndEditing:(UITextView *)textView {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@""]){
            textView.text = @"Hardware Description" ;
    ...
    if (indexPath.row == 0) {
        ...
        for (int i = 0; i < [sectionsArray count]; i++) {
            if (indexPath.section == i) {
                if (indexPath.section == HARDWARE_DOCUMENT_SECTION_INDEX) {
                    if (![docuementFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == HARDWARE_AUDIO_SECTION_INDEX) {
                    if (![audioFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == HARDWARE_VIDEO_SECTION_INDEX) {
...
-(void)getHardwareKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getHardwareKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [hardwareKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kHardwareAPI_KeywordList]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kHardwareAPI_KeywordList] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
                        [hardwareKeywordsArray addObject:obj] ;
                    }
                }
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode) {
...
-(void)getHardwareIndustryKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getHardwareIndustryKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [industryKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kHardwareAPI_Industry_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kHardwareAPI_Industry_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getTargetMarketKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getHardwareTargetMarketKeywordsListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [targetMarketKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kHardwareAPI_Target_Market_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kHardwareAPI_Target_Market_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getHardwareDetails {
    if([UtilityClass checkInternetConnection]) {
        ...
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kHardwareAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[hardwareData valueForKey:kHardwareAPI_ID]] forKey:kHardwareAPI_HardwareID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap viewHardwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"respos %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                for ( int i = 0; i < sectionsArray.count ; i++ ) {
                    NSString *key = [[sectionsArray objectAtIndex:i] valueForKey:@"key"] ;
                    if(i != HARDWARE_INDUSTRY_KEYWORDS_SECTION_INDEX && i != HARDWARE_TARGET_MARKET_KEYWORDS_SECTION_INDEX && i != HARDWARE_KEYWORDS_SECTION_INDEX) {
                ...
                // Keywords Array
                if([responseDict objectForKey:kAddHardwareAPI_IndustryKeywords]) {
                    selectedIndustryKeywordsArray = [[responseDict objectForKey:kAddHardwareAPI_IndustryKeywords] mutableCopy];
                ...
                if([responseDict objectForKey:kAddHardwareAPI_Keywords]) {
                    selectedHardwareKeywordsArray = [[responseDict objectForKey:kAddHardwareAPI_Keywords] mutableCopy] ;
                    selectedHardwareKeywordNames = [self resetTagsArrayWithData:selectedHardwareKeywordsArray];
                    selectedHardwareKeywordIds = [self resetTagIdsArrayWithData:selectedHardwareKeywordsArray];
                }
                if([responseDict objectForKey:kAddHardwareAPI_Target_Market_Keywords]) {
                    selectedTargetMarketKeywordsArray = [[responseDict objectForKey:kAddHardwareAPI_Target_Market_Keywords] mutableCopy];
                ...
                // Commits
                noOfcommit = [[responseDict valueForKey:kHardwareAPI_NoOfCommits] stringValue];
                isCommited = [[responseDict valueForKey:kHardwareAPI_IsCommited] intValue];
                isLiked = [[responseDict valueForKey:kHardwareAPI_IsLiked] intValue];
                isFollowed = [[responseDict valueForKey:kHardwareAPI_IsFollowed] intValue];
                
                // Document
                if([responseDict objectForKey:kHardwareAPI_Hardware_Document])
                    docuementFile = [[responseDict objectForKey:kHardwareAPI_Hardware_Document] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Audio
                if([responseDict objectForKey:kHardwareAPI_Hardware_Audio])
                    audioFile = [[responseDict objectForKey:kHardwareAPI_Hardware_Audio] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Video
                if([responseDict objectForKey:kHardwareAPI_Hardware_Video])
                    videoFile = [[responseDict objectForKey:kHardwareAPI_Hardware_Video] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Image
                if([responseDict objectForKey:kHardwareAPI_Hardware_Image]) {
                    NSString *strImage = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed, [responseDict objectForKey:kHardwareAPI_Hardware_Image]];
                    NSURL *url = [NSURL URLWithString:[strImage stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                    imgData = [NSData dataWithContentsOfURL:url];
                }
                
                // Refresh Dict
                [hardwareData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kHardwareAPI_Hardware_Title]] forKey:kHardwareAPI_Hardware_Title] ;
                [hardwareData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kHardwareAPI_CreatedBy]] forKey:kHardwareAPI_CreatedBy] ;
                [hardwareData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kHardwareAPI_StartDate]] forKey:kHardwareAPI_StartDate] ;
                [hardwareData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kHardwareAPI_EndDate]] forKey:kHardwareAPI_EndDate] ;
                [hardwareData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kHardwareAPI_Description]] forKey:kHardwareAPI_Description] ;
                
                [UtilityClass setHardwareDetails:[hardwareData mutableCopy]] ;
...
-(void)commitHardware {
    if([UtilityClass checkInternetConnection]) {
        ...
        if (isCommited == 0) {
            [ApiCrowdBootstrap commitHardwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
            ...
            [ApiCrowdBootstrap uncommitHardwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
...
-(void)getHardwareCommitmentList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getHardwareCommitmentListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode)  {
...
- (void)getRecommendedUserList: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:@"hardware" forKey:@"type"] ;
        ...
        [ApiCrowdBootstrap getRegisteredRoleListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kHardwareAPI_UserList]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]){
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kHardwareAPI_UserList]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kHardwareAPI_UserList]] ;
                    }
            ...
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
            {
                ...
                if(![searchBarUsers.text isEqualToString:@""])
                    ...
                    usersArray = [NSMutableArray arrayWithArray:[responseDict valueForKey:kHardwareAPI_UserList]] ;
...
-(void)editHardware {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[[sectionsArray objectAtIndex:HARDWARE_TITLE_SECTION_INDEX] valueForKey:@"value"] forKey:kHardwareAPI_Hardware_Title] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:HARDWARE_DESCRIPTION_SECTION_INDEX] valueForKey:@"value"] forKey:kHardwareAPI_Description] ;
        
        [dictParam setObject:[[sectionsArray objectAtIndex:HARDWARE_START_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kHardwareAPI_StartDate] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:HARDWARE_END_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kHardwareAPI_EndDate] ;
        
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedIndustryKeywordsArray withTagsArray:industryKeywordsArray tagType:@"industry"] forKey:kAddHardwareAPI_IndustryKeywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedHardwareKeywordsArray withTagsArray:hardwareKeywordsArray tagType:@"hardwarekeyword"] forKey:kAddHardwareAPI_Keywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedTargetMarketKeywordsArray withTagsArray:targetMarketKeywordsArray tagType:@"targetmarket"] forKey:kAddHardwareAPI_Target_Market_Keywords] ;
        
        if(imgData)
            [dictParam setObject:imgData forKey:kHardwareAPI_Hardware_Image] ;
        else
            [dictParam setObject:@"" forKey:kHardwareAPI_Hardware_Image] ;
        
        [dictParam setObject:@"" forKey:kHardwareAPI_Hardware_Document] ;
        [dictParam setObject:@"" forKey:kHardwareAPI_Hardware_Audio] ;
        [dictParam setObject:@"" forKey:kHardwareAPI_Hardware_Video] ;
        ...
        [ApiCrowdBootstrap editHardwareWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kEditHardware_SuccessMessage withResultType:@"0" withDuration:1] ;
                [UtilityClass setComingFrom_Hardware_AddEditScreen:YES];
                [self.navigationController popViewControllerAnimated:YES] ;
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == popupTblView) {
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            else if (selectedKeywordType == HARDWARE_TARGET_MARKET_KEYWORDS_SELECTED)
                return targetMarketKeywordsArray.count ;
            else
                return hardwareKeywordsArray.count ;
        }
        ...
        if(section <= HARDWARE_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)
            return 1;
        else {
            if(section == HARDWARE_IMAGE_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue])
                    return 1;
                else return 0 ;
            }
            else if(section == HARDWARE_DOCUMENT_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == HARDWARE_AUDIO_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(selectedKeywordType == HARDWARE_INDUSTRY_KEYWORDS_SELECTED) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kHardwareAPI_Industry_Name] ;
            if ([selectedIndustryKeywordIds containsObject:[dictKeyword valueForKey:kHardwareAPI_Industry_ID]]) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kHardwareAPI_Target_Market_Name] ;
            
            if ([selectedTargetMarketKeywordIds containsObject:[dictKeyword valueForKey:kHardwareAPI_Target_Market_ID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        } else
        {
            if (![searchBarKeywords.text isEqualToString:@""])
                ...
                dictKeyword = [hardwareKeywordsArray objectAtIndex:indexPath.row];

            cell.companyNameLbl.text = [dictKeyword valueForKey:kHardwareAPI_KeywordName] ;
            
            if ([selectedHardwareKeywordIds containsObject:[dictKeyword valueForKey:kHardwareAPI_KeywordID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        if (![searchBarUsers.text isEqualToString:@""]) {
            ...
            cell.lblName.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kHardwareAPI_User_Name]] ;
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kHardwareAPI_User_Desc]];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kHardwareAPI_User_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
        ...
        if(indexPath.section == sectionsArray.count) {
        ...
        else if(indexPath.section == HARDWARE_FOLLOW_SECTION_INDEX) {
        ...
        else if(indexPath.section == HARDWARE_INDUSTRY_KEYWORDS_SECTION_INDEX || indexPath.section == HARDWARE_TARGET_MARKET_KEYWORDS_SECTION_INDEX || indexPath.section == HARDWARE_KEYWORDS_SECTION_INDEX) {
            ...
            NSMutableArray *tags ;
            if(indexPath.section == HARDWARE_INDUSTRY_KEYWORDS_SECTION_INDEX)
                tags = [selectedIndustryKeywordNames mutableCopy] ;
            else if(indexPath.section == HARDWARE_TARGET_MARKET_KEYWORDS_SECTION_INDEX)
                tags = [selectedTargetMarketKeywordNames mutableCopy] ;
            else
                tags = [selectedHardwareKeywordNames mutableCopy] ;
        ...
        else if(indexPath.section == HARDWARE_DESCRIPTION_SECTION_INDEX) {
        ...
        else if(indexPath.section == HARDWARE_START_DATE_SECTION_INDEX || indexPath.section == HARDWARE_END_DATE_SECTION_INDEX) {
        ...
        else if(indexPath.section == HARDWARE_IMAGE_SECTION_INDEX) {
        ...
        else if (indexPath.section == HARDWARE_DOCUMENT_SECTION_INDEX || indexPath.section == HARDWARE_AUDIO_SECTION_INDEX || indexPath.section == HARDWARE_VIDEO_SECTION_INDEX) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(indexPath.section >= HARDWARE_DOCUMENT_SECTION_INDEX && indexPath.section < sectionsArray.count) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if(indexPath.section == HARDWARE_START_DATE_SECTION_INDEX || indexPath.section == HARDWARE_END_DATE_SECTION_INDEX) {
            return 65;
        }
        else if (indexPath.section == HARDWARE_FOLLOW_SECTION_INDEX)
            return 40;
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if(section <= HARDWARE_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)return 0;
    else return 45 ;
}
