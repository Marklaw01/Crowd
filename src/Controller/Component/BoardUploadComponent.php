...
class BoardUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('BoardInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('BoardOppertunityKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('BoardMemberTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('BoardMemberFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['board_member_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('BoardCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['board_member_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('BoardCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['board_member_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/board_member/"; // Upload directory
                $imgCamp='';
                $errors='';
