//
//  AddPurchaseOrderViewController.m
//  CrowdBootstrap
//
...
#import "AddPurchaseOrderViewController.h"
...
@interface AddPurchaseOrderViewController ()

@end

@implementation AddPurchaseOrderViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Add Purchase Order";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Purchase Order Title", @"Purchase Order Description", @"Purchase Order Availability Start Date", @"Purchase Order Availability End Date", @"Target Market", @"Purchase Order Keywords", @"Interest Keywords", @"Upload Image"];
    NSArray *parametersArray = @[kPurchaseOrderAPI_Title, kPurchaseOrderAPI_Description, kPurchaseOrderAPI_StartDate, kPurchaseOrderAPI_EndDate, kAddPurchaseOrderAPI_Target_Market_Keywords, kAddPurchaseOrderAPI_Keywords, kAddPurchaseOrderAPI_IndustryKeywords, kPurchaseOrderAPI_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kPurchaseOrderAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kPurchaseOrderAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kPurchaseOrderAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
- (IBAction)Submit_ClickAction:(id)sender {
    ...
    if(![self validatetextFieldsWithSectionIndex:PURCHASE_ORDER_TITLE_SECTION_INDEX]) return ;
    else if(![self validatetextFieldsWithSectionIndex:PURCHASE_ORDER_DESCRIPTION_SECTION_INDEX]) return ;
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    switch (selectedKeywordType) {
        case 4: // Target Keywords
        {
            ...
            break;
        case 5: // Purchase Order Keywords
        {
...
-(void)createPurchaseOrder {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedIndustryKeywordsArray withTagsArray:industryKeywordsArray tagType:@"industry"] forKey:kAddPurchaseOrderAPI_IndustryKeywords] ;
        ...
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedTargetMarketKeywordsArray withTagsArray:targetMarketKeywordsArray tagType:@"targetmarket"] forKey:kAddPurchaseOrderAPI_Target_Market_Keywords] ;
        ...
        [ApiCrowdBootstrap addPurchaseOrderWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddPurchaseOrder_SuccessMessage withResultType:@"0" withDuration:1] ;
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if(indexPath.section == PURCHASE_ORDER_START_DATE_SECTION_INDEX || indexPath.section == PURCHASE_ORDER_END_DATE_SECTION_INDEX) {
            return 65;
        }
        else if(indexPath.section == PURCHASE_ORDER_DESCRIPTION_SECTION_INDEX)
            return 121 ;
