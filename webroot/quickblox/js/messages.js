    ...
    if (messages) {
      ...
      if(messages.items.length === 0) {
        ...
        messages.items.forEach(function(item, i, arr) {
          ...
          var messageSenderUserId = getUserIdById(messageSenderId);
          ...
          var messageHtml = buildMessageHTML(messageText, messageSenderLogin, messageDateSent, messageAttachmentFileId, messageId,messageSenderUserId,logeUserId);
...
//alert (JSON.stringify(item)); 
          ...
          /*if (item.read_ids.length > 1 && messageSenderId === currentUser.id) { 
            $('.chatMsg #delivered_'+messageId).fadeOut(100);
            $('.chatMsg #read_'+messageId).fadeIn(200); 
          } else if (item.delivered_ids.length > 1 && messageSenderId === currentUser.id) {
            $('.chatMsg #delivered_'+messageId).fadeIn(100);
            $('.chatMsg #read_'+messageId).fadeOut(200); 
          }*/

          if (item.read > 0 && messageSenderId === currentUser.id) { 
            $('.chatMsg #delivered_'+messageId).fadeOut(100);
            $('.chatMsg #read_'+messageId).fadeIn(200); 
          } else if (item.read == 0 && messageSenderId === currentUser.id){
            $('.chatMsg #delivered_'+messageId).fadeIn(100);
            $('.chatMsg #read_'+messageId).fadeOut(200); 
          }
...
function showMessage(userId, msg, attachmentFileId) {
  ...
  var logeUserId =currentUser.id;
  var messageSenderUserId = getUserIdById(userId);
  var messageHtml = buildMessageHTML(msg.body, userLogin, new Date(), attachmentFileId, msg.id,messageSenderUserId,logeUserId);
