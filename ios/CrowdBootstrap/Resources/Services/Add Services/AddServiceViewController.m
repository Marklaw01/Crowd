...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Service Title", @"Service Description", @"Service Availability Start Date", @"Service Availability End Date", @"Target Market", @"Service Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kServiceAPI_Service_Title, kServiceAPI_Description, kServiceAPI_StartDate, kServiceAPI_EndDate, kAddServiceAPI_Target_Market_Keywords, kAddServiceAPI_Keywords, kAddServiceAPI_IndustryKeywords, kServiceAPI_Service_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kServiceAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kServiceAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kServiceAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createService {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addServiceWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddService_SuccessMessage withResultType:@"0" withDuration:1] ;
