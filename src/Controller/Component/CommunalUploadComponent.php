...
class CommunalUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('CommunalAssetInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('CommunalAssetKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('CommunalAssetTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('CommunalAssetFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['communalasset_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('CommunalAssetCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['communalasset_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('CommunalAssetCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['communalasset_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/communal/"; // Upload directory
                $imgCamp='';
                $errors='';
