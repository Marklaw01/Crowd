//
//  EditBoardMemberViewController.m
//  CrowdBootstrap
//
...
#import "EditBoardMemberViewController.h"
...
@interface EditBoardMemberViewController ()

@end

@implementation EditBoardMemberViewController
...
-(void) awakeFromNib {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(SetViewEditingNotification:) name:kNotificationSetBoardMemberViewEditing
                                               object:nil];
...
-(void)SetViewEditingNotification:(NSNotification *)notification {
    if ([[notification name] isEqualToString:kNotificationSetBoardMemberViewEditing]) {
...
-(void)resetUISettings {
    ...
    boardMemberData = [[UtilityClass getBoardMemberDetails] mutableCopy];
    NSLog(@"Board Member Data: %@",boardMemberData) ;
    ...
    self.title = [NSString stringWithFormat:@"%@",[boardMemberData valueForKey:kBoardMemberAPI_BoardMember_Title]] ;
    [self getBoardMemberDetails] ;
...
-(void)initializeSectionArray {
    ...
    NSArray *fieldsArray = @[@"Created By", @"Board Membership Title", @"Board Membership Description", @"Board Membership Start Date", @"Board Membership End Date", @"Target Market", @"Board Membership Keywords", @"Interest Keywords", @"Upload Image", @"View Document",@"Play Audio",@"Play Video"] ;
    NSArray *parametersArray = @[kBoardMemberAPI_CreatedBy, kBoardMemberAPI_BoardMember_Title, kBoardMemberAPI_Description, kBoardMemberAPI_StartDate, kBoardMemberAPI_EndDate, kBoardMemberAPI_TargetMarket_Keywords, kBoardMemberAPI_Keywords, kBoardMemberAPI_Industry_Keywords, kBoardMemberAPI_BoardMember_Image, kBoardMemberAPI_BoardMember_Document, kBoardMemberAPI_BoardMember_Audio, kBoardMemberAPI_BoardMember_Video] ;
    ...
    [self getBoardMemberKeywordsList];
    [self getBoardMemberIndustryKeywordsList];
...
- (void)checkKeywordsListFound:(int)keywordType {
    switch (keywordType) {
        ...
        case 6:
        {
            if (boardMemberKeywordsArray.count > 0)
                [self openTagsPopup] ;
            else
                [self presentViewController:[UtilityClass displayAlertMessage:kNoBoardMemberKeywordsFoundMessage] animated:YES completion:nil];
        }
            break;
        default:
            break;
...
-(NSString*)convertTagsArrayToStringforArray:(NSMutableArray*)array withTagsArray:(NSArray*)tagsArray tagType:(NSString *)type {
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kBoardMemberAPI_Target_Market_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kBoardMemberAPI_Target_Market_ID])
                    selectedValue = [obj valueForKey:kBoardMemberAPI_Target_Market_ID] ;
                else
                    selectedValue = [obj valueForKey:kBoardMemberAPI_Target_Market_ID] ;

                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag)
                        tagsStr = [dict valueForKey:kBoardMemberAPI_Target_Market_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kBoardMemberAPI_Target_Market_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
    } else if ([type isEqualToString:@"boardMemberkeyword"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kBoardMemberAPI_KeywordID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kBoardMemberAPI_KeywordID])
                    selectedValue = [obj valueForKey:kBoardMemberAPI_KeywordID] ;
                else
                    selectedValue = [obj valueForKey:kBoardMemberAPI_KeywordID] ;

                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kBoardMemberAPI_KeywordID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kBoardMemberAPI_KeywordID]] ;
                    isFirstTag = NO ;
                }
            }
        }
        ...
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kBoardMemberAPI_Industry_ID] ;
            for (NSString *obj in array) {
                NSString *selectedValue;
                if ([obj valueForKey:kBoardMemberAPI_Industry_ID])
                    selectedValue = [obj valueForKey:kBoardMemberAPI_Industry_ID] ;
                else
                    selectedValue = [obj valueForKey:kBoardMemberAPI_Industry_ID] ;

                if([selectedValue intValue] == [value intValue]){
                    if(isFirstTag) tagsStr = [dict valueForKey:kBoardMemberAPI_Industry_ID] ;
                    else tagsStr = [NSString stringWithFormat:@"%@,%@",tagsStr,[dict valueForKey:kBoardMemberAPI_Industry_ID]] ;
                    isFirstTag = NO ;
                }
            }
        }
...
- (IBAction)CheckUncheck_ClickAction:(id)sender {
    ...
    if([btn.accessibilityLabel isEqualToString:CHECK_IMAGE ]) { // Check
        ...
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            if(selectedKeywordType == BOARD_MEMBER_INDUSTRY_KEYWORDS_SELECTED)
                dictKeyword = [industryKeywordsArray objectAtIndex:btn.tag];
            else if (selectedKeywordType == BOARD_MEMBER_KEYWORDS_SELECTED)
                dictKeyword = [boardMemberKeywordsArray objectAtIndex:btn.tag];
            else
    ...
    if(selectedKeywordType == BOARD_MEMBER_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (NSMutableDictionary *obj in industryKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedIndustryKeywordNames addObject:[obj valueForKey:kBoardMemberAPI_Industry_Name]] ;
                [selectedIndustryKeywordIds addObject:[obj valueForKey:kBoardMemberAPI_Industry_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:BOARD_MEMBER_INDUSTRY_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        
    } else if(selectedKeywordType == BOARD_MEMBER_KEYWORDS_SELECTED) {
        [selectedBoardMemberKeywordsArray removeAllObjects];
        [selectedBoardMemberKeywordNames removeAllObjects];
        [selectedBoardMemberKeywordIds removeAllObjects];
        
        for (NSMutableDictionary *obj in boardMemberKeywordsArray) {
            NSString *isSelectedStr = [obj valueForKey:@"isSelected"] ;
            if([isSelectedStr isEqualToString:@"1"]) {
                [selectedBoardMemberKeywordsArray addObject:obj] ;
                [selectedBoardMemberKeywordNames addObject:[obj valueForKey:kBoardMemberAPI_KeywordName]] ;
                [selectedBoardMemberKeywordIds addObject:[obj valueForKey:kBoardMemberAPI_KeywordID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:BOARD_MEMBER_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
        ...
        for (NSMutableDictionary *obj in targetMarketKeywordsArray) {
            ...
            if([isSelectedStr isEqualToString:@"1"]) {
                ...
                [selectedTargetMarketKeywordNames addObject:[obj valueForKey:kBoardMemberAPI_Target_Market_Name]] ;
                [selectedTargetMarketKeywordIds addObject:[obj valueForKey:kBoardMemberAPI_Target_Market_ID]] ;
            }
        }
        [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:BOARD_MEMBER_TARGET_MARKET_KEYWORDS_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationAutomatic] ;
    }
}
...
-(void)tagsControl:(UIView *)tag withTagControlIndex:(NSInteger)tagControlIndex removedFromIndex:(NSInteger)index {
    ...
    if(tagControlIndex == BOARD_MEMBER_INDUSTRY_KEYWORDS_SELECTED) {
        ...
        for (int i=0; i<industryKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[industryKeywordsArray objectAtIndex:i] valueForKey:kBoardMemberAPI_Industry_Name]] ;
    ...
    } else if(tagControlIndex == BOARD_MEMBER_KEYWORDS_SELECTED) {
        NSString *selectedName = [NSString stringWithFormat:@"%@",[selectedBoardMemberKeywordNames objectAtIndex:index]] ;
        for (int i=0; i<boardMemberKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[boardMemberKeywordsArray objectAtIndex:i] valueForKey:kBoardMemberAPI_KeywordName]] ;
            if([keywordName isEqualToString:selectedName]){
                [[boardMemberKeywordsArray objectAtIndex:i] setValue:@"0" forKey:@"isSelected"] ;
                break ;
            }
        }
        [selectedBoardMemberKeywordsArray removeObjectAtIndex:index] ;
        [selectedBoardMemberKeywordNames removeObjectAtIndex:index];
        [selectedBoardMemberKeywordIds removeObjectAtIndex:index];
        ...
        for (int i=0; i<targetMarketKeywordsArray.count; i++) {
            NSString *keywordName = [NSString stringWithFormat:@"%@",[[targetMarketKeywordsArray objectAtIndex:i] valueForKey:kBoardMemberAPI_Target_Market_Name]] ;
            if([keywordName isEqualToString:selectedName]){
...
- (IBAction)Back_Click:(id)sender {
    [UtilityClass setComingFrom_BoardMember_AddEditScreen:YES];
    [self.navigationController popViewControllerAnimated:YES] ;
}

- (IBAction)CloseData_ClickAction:(id)sender {
    [boardMemberDataView dismissPresentingPopup] ;
}
...
- (IBAction)followButton_ClickAction:(UIButton*)button {
    ...
    if([UtilityClass checkInternetConnection]) {
        ...
        if (isFollowed == 0) {
            [ApiCrowdBootstrap followBoardMemberWithParameters:dictParam success:^(NSDictionary *responseDict) {
                ...
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    ...
                    [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:BOARD_MEMBER_FOLLOW_SECTION_INDEX]] withRowAnimation:UITableViewRowAnimationNone] ;
                }
            ...
            [ApiCrowdBootstrap unfollowBoardMemberWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                    if(isFollowed == 1)
...
- (IBAction)Calender_ClickAction:(id)sender {
    ...
    if ([sender tag] == BOARD_MEMBER_START_DATE_SECTION_INDEX)
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:BOARD_MEMBER_START_DATE_SECTION_INDEX]] ;
    else
        cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:BOARD_MEMBER_END_DATE_SECTION_INDEX]] ;
...
- (IBAction)Submit_ClickAction:(id)sender {
    if ((selectedSegmentControl == 100 && selectedSegment == 0)) {
        [self commitBoardMember];
    } else {
        if(![self validatetextFieldsWithSectionIndex:BOARD_MEMBER_TITLE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:BOARD_MEMBER_START_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:BOARD_MEMBER_END_DATE_SECTION_INDEX]) return ;
        else if(![self validatetextFieldsWithSectionIndex:BOARD_MEMBER_DESCRIPTION_SECTION_INDEX]) return ;
        ...
        else if(selectedBoardMemberKeywordsArray.count < 1) {
        ...
        else [self editBoardMember] ;
    }
}

- (IBAction)boardMemberCommitmentList_ClickAction:(id)sender {
...
//    imgVwPeople.image = [UIImage imageNamed:@"like_green"];
    ...
    if ([noOfcommit integerValue] > 0) {
        [self getBoardMemberCommitmentList];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    GraphicTableViewCell *cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:BOARD_MEMBER_IMAGE_SECTION_INDEX]] ;
...
- (IBAction)DatePickerToolbarButtons_ClickAction:(id)sender {
    DobTableViewCell *startDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:BOARD_MEMBER_START_DATE_SECTION_INDEX]] ;
    DobTableViewCell *endDatecell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:BOARD_MEMBER_END_DATE_SECTION_INDEX]] ;
    [startDatecell.textFld resignFirstResponder] ;
    [endDatecell.textFld resignFirstResponder] ;
    [dateFormatter setTimeZone:[NSTimeZone timeZoneWithAbbreviation:@"GMT+0:00"]];
    
    if([sender tag] == DONE_CLICKED) {
        if(selectedDatePickerType == BOARD_MEMBER_START_DATE_SELECTED) {
            ...
            if (endDate == nil) {
                ...
                [[sectionsArray objectAtIndex:BOARD_MEMBER_START_DATE_SECTION_INDEX] setValue:startDatecell.textFld.text forKey:@"value"] ;
            } else {
            ...
            if (startDate == nil) {
                ...
                [[sectionsArray objectAtIndex:BOARD_MEMBER_END_DATE_SECTION_INDEX] setValue:endDatecell.textFld.text forKey:@"value"] ;
            } else {
                NSComparisonResult result;
...
- (IBAction)DatePickerToolbarButtons_ClickAction:(id)sender {
    ...
    if([sender tag] == DONE_CLICKED) {
        if(selectedDatePickerType == BOARD_MEMBER_START_DATE_SELECTED) {
            ...
            [[sectionsArray objectAtIndex:BOARD_MEMBER_START_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
        else {
            cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:BOARD_MEMBER_END_DATE_SECTION_INDEX]] ;
            [cell.textFld resignFirstResponder] ;
            ...
            [[sectionsArray objectAtIndex:BOARD_MEMBER_END_DATE_SECTION_INDEX] setValue:cell.textFld.text forKey:@"value"] ;
        }
    }
    else {
        if(selectedDatePickerType == BOARD_MEMBER_START_DATE_SELECTED) {
    ...
    if (searchBar.tag == 10) {
        ...
        switch (selectedKeywordType) {
            case 5: // Target Keywords
            {
                ...
                break;
            case 6: // Board Member Keywords
            {
                for(NSDictionary *keywordDict in boardMemberKeywordsArray)
                {
                    NSString *keywordName = [keywordDict objectForKey:@"name"];
                    NSRange range = [keywordName rangeOfString:searchText options:NSCaseInsensitiveSearch];
                    if(range.location != NSNotFound)
                ...
                NSLog(@"Board Member Results: %@",searchResultsForKeywords);
            }
                break;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if(textField.tag == BOARD_MEMBER_START_DATE_SECTION_INDEX || textField.tag == BOARD_MEMBER_END_DATE_SECTION_INDEX) {
            if(textField.tag == BOARD_MEMBER_START_DATE_SECTION_INDEX) {
                selectedDatePickerType = BOARD_MEMBER_START_DATE_SELECTED;
            }
            else {
                selectedDatePickerType = BOARD_MEMBER_END_DATE_SELECTED;
                NSDate *startDate = [dateFormatter dateFromString:startDatecell.textFld.text]; // start date
                if(startDate == nil)
...
-(BOOL)textViewShouldBeginEditing:(UITextView *)textView {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@"Board Member Description"] && textView.textColor == [UIColor lightGrayColor]) {
            textView.text = @"" ;
...
-(BOOL)textViewShouldEndEditing:(UITextView *)textView {
    ...
    if ((selectedSegmentControl == 100 && selectedSegment == 1) || (selectedSegmentControl == 200 && selectedSegment == 0)) {
        ...
        if([textView.text isEqualToString:@""]) {
            textView.text = @"Board Member Description" ;
    ...
    if (indexPath.row == 0) {
        ...
        for (int i = 0; i < [sectionsArray count]; i++) {
            if (indexPath.section == i) {
                if (indexPath.section == BOARD_MEMBER_DOCUMENT_SECTION_INDEX) {
                    if (![docuementFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == BOARD_MEMBER_AUDIO_SECTION_INDEX) {
                    if (![audioFile isEqualToString:@""]) {
                        [arrayForBool replaceObjectAtIndex:i withObject:[NSNumber numberWithBool:!collapsed]];
                    }
                }
                else if (indexPath.section == BOARD_MEMBER_VIDEO_SECTION_INDEX) {
...
- (void)datePickerChanged:(UIDatePicker *)datePicker {
    ...
    if (selectedDatePickerType == BOARD_MEMBER_START_DATE_SELECTED) {
        ...
        NSDate *startDate = datePicker.date; // start date
        NSDate *endDate = [dateFormatter dateFromString:endDatecell.textFld.text]; // end date
        
        if (endDate == nil) {
            startDatecell.textFld.text = [dateFormatter stringFromDate:datePicker.date];
        } else {
            NSComparisonResult result;
        ...
        NSDate *startDate = [dateFormatter dateFromString:startDatecell.textFld.text]; // start date
        NSDate *endDate = datePicker.date; // end date
        
        if (startDate == nil) {
            [endDatecell.textFld setText:[dateFormatter stringFromDate:datePicker.date]];
        } else {
            NSComparisonResult result;
...
-(void)getBoardMemberKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getBoardMemberKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [boardMemberKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kBoardMemberAPI_KeywordList]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kBoardMemberAPI_KeywordList] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
                        [boardMemberKeywordsArray addObject:obj] ;
                    }
                }
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode) {
...
-(void)getBoardMemberIndustryKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        
        [ApiCrowdBootstrap getBoardMemberIndustryKeywordsList:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [industryKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kBoardMemberAPI_Industry_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kBoardMemberAPI_Industry_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getTargetMarketKeywordsList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getBoardMemberTargetMarketKeywordsListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode )  {
                NSLog(@"response : %@", responseDict);
                [targetMarketKeywordsArray removeAllObjects] ;
                if([responseDict objectForKey:kBoardMemberAPI_Target_Market_List]) {
                    for (NSDictionary *dict in (NSArray*)[[responseDict objectForKey:kBoardMemberAPI_Target_Market_List] mutableCopy]) {
                        NSMutableDictionary *obj = [[NSMutableDictionary alloc] initWithDictionary:[dict mutableCopy]] ;
                        [obj setValue:@"0" forKey:@"isSelected"] ;
...
-(void)getBoardMemberDetails {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kBoardMemberAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[boardMemberData valueForKey:kBoardMemberAPI_ID]] forKey:kBoardMemberAPI_BoardID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        [ApiCrowdBootstrap viewBoardMemberWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"respos %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                for ( int i = 0; i < sectionsArray.count ; i++ ) {
                    NSString *key = [[sectionsArray objectAtIndex:i] valueForKey:@"key"] ;
                    if(i != BOARD_MEMBER_INDUSTRY_KEYWORDS_SECTION_INDEX && i != BOARD_MEMBER_TARGET_MARKET_KEYWORDS_SECTION_INDEX && i != BOARD_MEMBER_KEYWORDS_SECTION_INDEX) {
                ...
                // Keywords Array
                if([responseDict objectForKey:kBoardMemberAPI_Industry_Keywords]) {
                    selectedIndustryKeywordsArray = [[responseDict objectForKey:kBoardMemberAPI_Industry_Keywords] mutableCopy];
                ...
                if([responseDict objectForKey:kBoardMemberAPI_Keywords]) {
                    selectedBoardMemberKeywordsArray = [[responseDict objectForKey:kBoardMemberAPI_Keywords] mutableCopy] ;
                    selectedBoardMemberKeywordNames = [self resetTagsArrayWithData:selectedBoardMemberKeywordsArray];
                    selectedBoardMemberKeywordIds = [self resetTagIdsArrayWithData:selectedBoardMemberKeywordsArray];
                }
                if([responseDict objectForKey:kBoardMemberAPI_TargetMarket_Keywords]) {
                    selectedTargetMarketKeywordsArray = [[responseDict objectForKey:kBoardMemberAPI_TargetMarket_Keywords] mutableCopy];
                ...
                // Commits
                noOfcommit = [[responseDict valueForKey:kBoardMemberAPI_NoOfCommits] stringValue];
                isCommited = [[responseDict valueForKey:kBoardMemberAPI_IsCommited] intValue];
                isLiked = [[responseDict valueForKey:kBoardMemberAPI_IsLiked] intValue];
                isFollowed = [[responseDict valueForKey:kBoardMemberAPI_IsFollowed] intValue];

                // Document
                if([responseDict objectForKey:kBoardMemberAPI_BoardMember_Document])
                    docuementFile = [[responseDict objectForKey:kBoardMemberAPI_BoardMember_Document] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Audio
                if([responseDict objectForKey:kBoardMemberAPI_BoardMember_Audio])
                    audioFile = [[responseDict objectForKey:kBoardMemberAPI_BoardMember_Audio] stringByReplacingOccurrencesOfString:@" " withString:@"%20"] ;
                
                // Video
                if([responseDict objectForKey:kBoardMemberAPI_BoardMember_Video])
                    videoFile = [[responseDict objectForKey:kBoardMemberAPI_BoardMember_Video] stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
                
                // Image
                if([responseDict objectForKey:kBoardMemberAPI_BoardMember_Image]) {
                    NSString *strImage = [NSString stringWithFormat:@"%@%@",APIPortToBeUsed, [responseDict objectForKey:kBoardMemberAPI_BoardMember_Image]];
                    NSURL *url = [NSURL URLWithString:[strImage stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                    imgData = [NSData dataWithContentsOfURL:url];
                }
                
                // Refresh Dict
                [boardMemberData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kBoardMemberAPI_BoardMember_Title]] forKey:kBoardMemberAPI_BoardMember_Title] ;
                [boardMemberData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kBoardMemberAPI_CreatedBy]] forKey:kBoardMemberAPI_CreatedBy] ;
                [boardMemberData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kBoardMemberAPI_StartDate]] forKey:kBoardMemberAPI_StartDate] ;
                [boardMemberData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kBoardMemberAPI_EndDate]] forKey:kBoardMemberAPI_EndDate] ;
                [boardMemberData setValue:[NSString stringWithFormat:@"%@",[responseDict valueForKey:kBoardMemberAPI_Description]] forKey:kBoardMemberAPI_Description] ;
                
                [UtilityClass setBoardMemberDetails:[boardMemberData mutableCopy]] ;
...
-(void)commitBoardMember {
    if([UtilityClass checkInternetConnection]) {
        
        [UtilityClass showHudWithTitle:kHUDMessage_PleaseWait] ;
        NSMutableDictionary *dictParam =[[NSMutableDictionary alloc] init];
        [dictParam setObject:[NSString stringWithFormat:@"%d",[UtilityClass getLoggedInUserID]] forKey:kBoardMemberAPI_UserID] ;
        [dictParam setObject:[NSString stringWithFormat:@"%@",[boardMemberData valueForKey:kBoardMemberAPI_ID]] forKey:kBoardMemberAPI_BoardMemberID] ;
        NSLog(@"dictParam: %@",dictParam) ;
        
        if (isCommited == 0) {
            [ApiCrowdBootstrap commitBoardMemberWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
            ...
            [ApiCrowdBootstrap uncommitBoardMemberWithParameters:dictParam success:^(NSDictionary *responseDict) {
                [UtilityClass hideHud] ;
                NSLog(@"responseDict %@", responseDict);
                if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                    if(isCommited == 0)
...
-(void)getBoardMemberCommitmentList {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap getBoardMemberCommitmentListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            [UtilityClass hideHud] ;
            NSLog(@"responseDict: %@",responseDict) ;
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode)  {
...
- (void)getRecommendedUserList: (NSString *)searchedString {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:@"board_member" forKey:@"type"] ;
        ...
        [ApiCrowdBootstrap getRegisteredRoleListWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode) {
                if([responseDict valueForKey:kBoardMemberAPI_UserList]) {
                    ...
                    if(![searchBarUsers.text isEqualToString:@""]){
                        searchResultsForRegisteredUsers = [NSMutableArray arrayWithArray:[responseDict valueForKey:kBoardMemberAPI_UserList]] ;
                    }
                    else {
                        [usersArray addObjectsFromArray:(NSArray*)[responseDict valueForKey:kBoardMemberAPI_UserList]] ;
                    }
            ...
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
            {
                ...
                if(![searchBarUsers.text isEqualToString:@""])
                    ...
                    usersArray = [NSMutableArray arrayWithArray:[responseDict valueForKey:kBoardMemberAPI_UserList]] ;
...
-(void)editBoardMember {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[[sectionsArray objectAtIndex:BOARD_MEMBER_TITLE_SECTION_INDEX] valueForKey:@"value"] forKey:kBoardMemberAPI_BoardMember_Title] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:BOARD_MEMBER_DESCRIPTION_SECTION_INDEX] valueForKey:@"value"] forKey:kBoardMemberAPI_Description] ;
        
        [dictParam setObject:[[sectionsArray objectAtIndex:BOARD_MEMBER_START_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kBoardMemberAPI_StartDate] ;
        [dictParam setObject:[[sectionsArray objectAtIndex:BOARD_MEMBER_END_DATE_SECTION_INDEX] valueForKey:@"value"] forKey:kBoardMemberAPI_EndDate] ;
        
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedIndustryKeywordsArray withTagsArray:industryKeywordsArray tagType:@"industry"] forKey:kAddBoardMemberAPI_IndustryKeywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedBoardMemberKeywordsArray withTagsArray:boardMemberKeywordsArray tagType:@"boardMemberkeyword"] forKey:kAddBoardMemberAPI_Keywords] ;
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedTargetMarketKeywordsArray withTagsArray:targetMarketKeywordsArray tagType:@"targetmarket"] forKey:kBoardMemberAPI_TargetMarket_Keywords] ;
        
        if(imgData)
            [dictParam setObject:imgData forKey:kBoardMemberAPI_BoardMember_Image] ;
        else
            [dictParam setObject:@"" forKey:kBoardMemberAPI_BoardMember_Image] ;
        
        [dictParam setObject:@"" forKey:kBoardMemberAPI_BoardMember_Document] ;
        [dictParam setObject:@"" forKey:kBoardMemberAPI_BoardMember_Audio] ;
        [dictParam setObject:@"" forKey:kBoardMemberAPI_BoardMember_Video] ;
        ...
        [ApiCrowdBootstrap editBoardMemberWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kEditBoardMember_SuccessMessage withResultType:@"0" withDuration:1] ;
                [UtilityClass setComingFrom_BoardMember_AddEditScreen:YES];
                [self.navigationController popViewControllerAnimated:YES] ;
            }
            else if([[responseDict valueForKey:@"code"] intValue] == kErrorCode )
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == popupTblView) {
        if (![searchBarKeywords.text isEqualToString:@""])
            ...
            else if (selectedKeywordType == BOARD_MEMBER_TARGET_MARKET_KEYWORDS_SELECTED)
                return targetMarketKeywordsArray.count ;
            else
                return boardMemberKeywordsArray.count ;
        }
        ...
        if(section <= BOARD_MEMBER_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)
            return 1;
        else {
            if(section == BOARD_MEMBER_IMAGE_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue])
                    return 1;
                else return 0 ;
            }
            else if(section == BOARD_MEMBER_DOCUMENT_SECTION_INDEX ) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else if(section == BOARD_MEMBER_AUDIO_SECTION_INDEX) {
                if ([[arrayForBool objectAtIndex:section] boolValue]) return 1;
                else  return 0 ;
            }
            else{
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(tableView == popupTblView) {
        ...
        if(selectedKeywordType == BOARD_MEMBER_INDUSTRY_KEYWORDS_SELECTED) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kBoardMemberAPI_Industry_Name] ;
            if ([selectedIndustryKeywordIds containsObject:[dictKeyword valueForKey:kBoardMemberAPI_Industry_ID]]) {
            ...
            cell.companyNameLbl.text = [dictKeyword valueForKey:kBoardMemberAPI_Target_Market_Name] ;
            if ([selectedTargetMarketKeywordIds containsObject:[dictKeyword valueForKey:kBoardMemberAPI_Target_Market_ID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        } else
        {
            if (![searchBarKeywords.text isEqualToString:@""])
                ...
                dictKeyword = [boardMemberKeywordsArray objectAtIndex:indexPath.row];
            
            cell.companyNameLbl.text = [dictKeyword valueForKey:kBoardMemberAPI_KeywordName] ;
            
            if ([selectedBoardMemberKeywordIds containsObject:[dictKeyword valueForKey:kBoardMemberAPI_KeywordID]]) {
                [cell.checkboxBtn setBackgroundImage:[UIImage imageNamed:CHECK_IMAGE] forState:UIControlStateNormal] ;
                cell.checkboxBtn.accessibilityLabel = CHECK_IMAGE ;
                [dictKeyword setValue:@"1" forKey:@"isSelected"] ;
            } else {
        ...
        if (![searchBarUsers.text isEqualToString:@""]) {
            ...
            cell.lblName.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kBoardMemberAPI_User_Name]] ;
            cell.lblDesc.text = [NSString stringWithFormat:@"%@",[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kBoardMemberAPI_User_Desc]];
            
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@",APIPortToBeUsed,[[searchResultsForRegisteredUsers objectAtIndex:indexPath.row] valueForKey:kBoardMemberAPI_User_Image]]] placeholderImage:[UIImage imageNamed:kPlaceholderImage_Contractor]] ;
        ...
        if(indexPath.section == sectionsArray.count) {
        ...
        else if(indexPath.section == BOARD_MEMBER_FOLLOW_SECTION_INDEX) {
            ...
            NSMutableArray *tags ;
            if(indexPath.section == BOARD_MEMBER_INDUSTRY_KEYWORDS_SECTION_INDEX)
                tags = [selectedIndustryKeywordNames mutableCopy] ;
            else if(indexPath.section == BOARD_MEMBER_TARGET_MARKET_KEYWORDS_SECTION_INDEX)
                tags = [selectedTargetMarketKeywordNames mutableCopy] ;
            else
                tags = [selectedBoardMemberKeywordNames mutableCopy] ;
        ...
        else if(indexPath.section == BOARD_MEMBER_DESCRIPTION_SECTION_INDEX) {
        ...
        else if(indexPath.section == BOARD_MEMBER_START_DATE_SECTION_INDEX || indexPath.section == BOARD_MEMBER_END_DATE_SECTION_INDEX) {
        ...
        else if(indexPath.section == BOARD_MEMBER_IMAGE_SECTION_INDEX) {
        ...
        else if (indexPath.section == BOARD_MEMBER_DOCUMENT_SECTION_INDEX || indexPath.section == BOARD_MEMBER_AUDIO_SECTION_INDEX || indexPath.section == BOARD_MEMBER_VIDEO_SECTION_INDEX) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView == popupTblView) {
        ...
        if(indexPath.section >= BOARD_MEMBER_DOCUMENT_SECTION_INDEX && indexPath.section < sectionsArray.count) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == popupTblView)
        ...
        if(indexPath.section == BOARD_MEMBER_START_DATE_SECTION_INDEX || indexPath.section == BOARD_MEMBER_END_DATE_SECTION_INDEX) {
            return 65;
        }
        else if (indexPath.section == BOARD_MEMBER_FOLLOW_SECTION_INDEX)
            return 40;
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if(section <= BOARD_MEMBER_INDUSTRY_KEYWORDS_SECTION_INDEX || section == sectionsArray.count)return 0;
    else return 45 ;
}
