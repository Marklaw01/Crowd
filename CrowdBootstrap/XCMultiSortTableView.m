...
@interface XCMultiTableView () <UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate , UITextFieldDelegate>
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UITapGestureRecognizer *vertexTapRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(vertexTap:)];
        [vertexView addGestureRecognizer:vertexTapRecognizer];
        ...
        vertextLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:15] ;
...
- (UITableViewCell *)contentTableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        UITextField *textFld = [[UITextField alloc] initWithFrame:CGRectMake(0, 0, cellW, 40)] ;
        textFld.text = [NSString stringWithFormat:@"%@", [ary objectAtIndex:i]];
        textFld.font = [UIFont fontWithName:@"HelveticaNeue" size:14] ;
        textFld.keyboardType = UIKeyboardTypeNumberPad ;
        textFld.textAlignment = NSTextAlignmentCenter ;
        //textFld.layer.borderColor = [[UIColor grayColor] CGColor] ;
        //textFld.layer.borderWidth = 2 ;
        textFld.delegate = self ;
        textFld.inputAccessoryView = numberToolbar ;
        textFld.tag = indexPath.section ;
        textFld.accessibilityValue = [NSString stringWithFormat:@"%ld",(long)indexPath.row] ;
        textFld.accessibilityLabel = [NSString stringWithFormat:@"%ld",(long)i] ;
        if(self.isEditableEnable){
            if(indexPath.row < 7) textFld.userInteractionEnabled = YES ;
            else textFld.userInteractionEnabled = NO ;
        }
        else textFld.userInteractionEnabled = NO ;
        //[textFld sizeToFit];
       ...
       /* UILabel *label = [[UILabel alloc] initWithFrame:CGRectZero];
        ...
        switch (alignPosition) {
            case AlignHorizontalPositionLeft:
            {
                CGFloat labelW = CGRectGetWidth(textFld.frame);
                textFld.center = CGPointMake(labelW * 0.5, cellH * 0.5);
            }
                break;
            case AlignHorizontalPositionRight:
            {
                CGFloat labelW = CGRectGetWidth(textFld.frame);
                textFld.center = CGPointMake(cellW - labelW * 0.5, cellH * 0.5);
            }
                break;
            default:
            {
                textFld.center = CGPointMake(cellW / 2.0f, cellH / 2.0f);
            }
                break;
        }
       ...
       /* UIColor *color = [self bgColorInSection:indexPath.section InRow:indexPath.row InColumn:i];
        ...
        // Commented by Richa -- To add gradient on cell
        
        CAGradientLayer *gradient = [CAGradientLayer layer];
        gradient.frame = cell.bounds;
        gradient.colors = [NSArray arrayWithObjects:(id)[[UIColor colorWithRed:214.0f/255.0f green:223.0f/255.0f blue:255.0f/255.0f alpha:1]CGColor], (id)[[UIColor colorWithRed:232.0f/255.0f green:237.0f/255.0f blue:255.0f/255.0f alpha:1]CGColor], nil];
...
-(void)vertexTap:(UITapGestureRecognizer *)recognizer {
    [self.delegate vertexTapped:vertextLabel.text] ;
}
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if([content intValue]){
        if([content intValue] > 24) return NO;
        else{
            NSIndexPath *indexPath = [NSIndexPath indexPathForRow:[textField.accessibilityValue intValue] inSection:textField.tag] ;
            
            [self.delegate updateGridViewContent:indexPath withIndex:[textField.accessibilityLabel intValue] withContent:content] ;
        }
