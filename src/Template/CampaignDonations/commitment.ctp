              ...
              <div role="tabpanel" class="campaigns-section">
                ...
                <div class="tab-content">
                 <!-- 1Tab-->
                  ...
                  <div role="tabpanel" class="tab-pane active whiteBg" id="commitments">
                     <ul class="contentListing campaignListing"> 
                      
                       <?php $c=0; 
                       $countData= count($commitments);
                       foreach ($commitments as $commitment): $c++;?>
                            <li>
                             <div class="listingIcon"><i class="icon"><img src="<?php echo $this->request->webroot;?>images/campaign.png" alt=""></i></div>
                             <div class="listingContent">
                               <div class="headingBar">
                                 <h2 class="heading"><?php echo $commitment->campaign->campaigns_name; ?>
                                 <span>
                                    <?php 
                                      $startup_id = $commitment->campaign->startup_id;
                                      $startups = $this->Custom->getStartupsDetailByID($startup_id);
                                          echo $startups[0]->name;
                                    ?>
                                 </span></h2>
                                 <h2 class="amount">Target Amount: $<?php echo $this->Number->precision($commitment->campaign->target_amount,2); ?>
                                 <span>
                                 FUNDS RAISED SO FAR: 
                                 <?php //echo $this->Number->precision($commitment->campaign->fund_raised_so_far,2); ?>
                                  
                                  $<?php $DonatedAmount = $this->Custom->getCampaignTotalDonatedAmountByID($commitment->campaign->id);
                                      echo $this->Number->precision($DonatedAmount,2);
                                  ?>
                                 </span></h2>
                               </div>
                               <p><?php echo $commitment->campaign->summary; ?></p>
                               <p class="date"> Due Date: <?php echo $commitment->campaign->due_date; ?></p>
                               <div class="links alignRight">
                                   <?php 
                                      echo $this->Html->link('<i class="fa fa-eye"></i> View', array('controller'=>'campaigns',  'action' => 'view', base64_encode($commitment->campaign->id)), array('escape'=>false,'class'=>'smallCurveBtn blueBtn customBtn'));
                                  ?>
                               </div>
                            </div>
                            </li> 
