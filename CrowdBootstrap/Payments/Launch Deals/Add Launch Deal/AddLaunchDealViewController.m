...
#import "AddLaunchDealViewController.h"
...
@interface AddLaunchDealViewController ()

@end

@implementation AddLaunchDealViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Launch Deals";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Launch Deal Title", @"Launch Deal Description", @"Launch Deal Availability Start Date", @"Launch Deal Availability End Date", @"Target Market", @"Launch Deal Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kLaunchDealAPI_Title, kLaunchDealAPI_Description, kLaunchDealAPI_StartDate, kLaunchDealAPI_EndDate, kAddLaunchDealAPI_Target_Market_Keywords, kAddLaunchDealAPI_Keywords, kAddLaunchDealAPI_IndustryKeywords, kLaunchDealAPI_Image] ;
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kLaunchDealAPI_Target_Market_ID] ;
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kLaunchDealAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kLaunchDealAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createLaunchDeal {
    if([UtilityClass checkInternetConnection]) {
        ...
        [ApiCrowdBootstrap addLaunchDealWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddLaunchDeal_SuccessMessage withResultType:@"0" withDuration:1] ;
