  ...
  <div class="row">
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 ">
      <div class="form-group pull-right">
        <?php 
            if($UserId != $JobsDetails->user_id){
              // Commit and Uncommit button
              if(empty($jobApply)){
                    $titleComiit='Apply';
                    $comittLink= $this->Url->build(["controller" => "Opportunities","action" => "applyForJob",base64_encode($JobsDetails->id)]);
                    // echo $this->Html->link(__('Commit'), ['controller' => 'CampaignDonations','action' => 'add',base64_encode($campaign->id)]) ;
              }else{
                    $titleComiit='Already Applied';
                    $comittLink= $this->Url->build(["controller" => "Opportunities","action" => "viewJobDetails",base64_encode($JobsDetails->id)]);
              ...
              if(!empty($jobFollow)){
                  $titleFollow='Unfollow';
                    $followLink= $this->Url->build(["controller" => "Opportunities","action" => "unfollowJob",base64_encode($JobsDetails->id)]);                      
              }else {
                $titleFollow='Follow';
                    $followLink= $this->Url->build(["controller" => "Opportunities","action" => "followJob",base64_encode($JobsDetails->id)]);   
              }
