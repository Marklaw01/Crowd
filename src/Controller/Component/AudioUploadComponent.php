...
class AudioUploadComponent extends Component
{
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('AudioInterestKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('AudioKeywords');
		...
		* * * * */
		...
		{
			$JobPostingKeywords = TableRegistry::get('AudioTargetMarkets');
		...
		* * * * */
		public function isFollowedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('AudioFollowers');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['audio_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function isCommitedbyUser($fundId=null,$userId=null)
		{
			...
			$FundFollowers = TableRegistry::get('AudioCommitments');
			$FundFollower= $FundFollowers->find('all',['conditions'=>['audio_id' => $fundId,'user_id' => $userId]])->first();
		...
		* * * * */
		public function getCommitCount($fundId=null)
		{
			...
			$FundDislikes = TableRegistry::get('AudioCommitments');
			$likelists= $FundDislikes->find('all',['conditions'=>['audio_id' => $fundId]]);
	    ...
	    ****/
	    ...
	    public function uploadImage($files = null) 
	    {
		    ...
		    if(!empty($files)){
                ...
                $path = WWW_ROOT . "img/audio_video/"; // Upload directory
                $imgCamp='';
                $errors='';
