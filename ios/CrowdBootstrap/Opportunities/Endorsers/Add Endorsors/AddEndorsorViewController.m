...
#import "AddEndorsorViewController.h"
...
@interface AddEndorsorViewController ()

@end

@implementation AddEndorsorViewController
...
-(void)resetUISettings {
    ...
    self.title = @"Endorser Requirements";
    [self initializeSectionArray] ;
}

-(void)initializeSectionArray {
    
    NSArray *fieldsArray = @[@"Endorsement Title", @"Endorsement Description", @"Endorsement Start Date", @"Endorsement End Date", @"Target Market", @"Endorsement Keywords", @"Interest Keywords", @"Upload Image"] ;
    NSArray *parametersArray = @[kEndorsorAPI_Endorsor_Title, kEndorsorAPI_Description, kEndorsorAPI_StartDate, kEndorsorAPI_EndDate, kAddEndorsorAPI_Target_Market_Keywords, kAddEndorsorAPI_Keywords, kAddEndorsorAPI_IndustryKeywords, kEndorsorAPI_Endorsor_Image] ;
...
-(NSString*)convertTagsArrayToStringforArray:(NSMutableArray*)array withTagsArray:(NSArray*)tagsArray tagType:(NSString *)type {
    ...
    if ([type isEqualToString:@"targetmarket"]) {
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kEndorsorAPI_Target_Market_ID] ;
    ...
    } else if ([type isEqualToString:@"endorserKeyword"]) {
        for (NSDictionary *dict in tagsArray) {
            NSString *value = [dict valueForKey:kEndorsorAPI_KeywordID] ;
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kEndorsorAPI_KeywordID] ;
                if([selectedValue intValue] == [value intValue]){
        ...
        for (NSDictionary *dict in tagsArray) {
            ...
            for (NSString *obj in array) {
                NSString *selectedValue = [obj valueForKey:kEndorsorAPI_Industry_ID] ;
                if([selectedValue intValue] == [value intValue]){
...
-(void)createEndorsor {
    if([UtilityClass checkInternetConnection]) {
        ...
        [dictParam setObject:[self convertTagsArrayToStringforArray:selectedEndorsorKeywordsArray withTagsArray:endorsorKeywordsArray tagType:@"endorserKeyword"] forKey:kAddEndorsorAPI_Keywords] ;
        ...
        [ApiCrowdBootstrap addEndorsorWithParameters:dictParam success:^(NSDictionary *responseDict) {
            ...
            if([[responseDict valueForKey:@"code"] intValue] == kSuccessCode ) {
                ...
                [UtilityClass showNotificationMessgae:kAddEndorsor_SuccessMessage withResultType:@"0" withDuration:1] ;
