    ...
    <div class='col-lg-4 col-md-4 col-sm-4 '>
      <div class="form-group">
        <span class="contentListingLikes">
          <h2 class="likes">
            <?php 
              $likeC = $this->Custom->getLaunchDealLikeCount($fundDeatils->id);
              if(!empty($likeC)){

                  echo $this->Html->link($likeC, array('controller'=>'LaunchDeals','action'=>'likeList',base64_encode($fundDeatils->id)), array('escape'=>false,'title'=>'View','style'=>'color: rgb(3, 66, 110);'));
            ...
            &nbsp;
            <?php 
            if($this->Custom->isLaunchDealLikedbyUser($fundDeatils->id,$userId)){
            ...
            &nbsp;
            <?php 
              $disLikeC = $this->Custom->getLaunchDealDislikeCount($fundDeatils->id);
              if(!empty($disLikeC)){

                  echo $this->Html->link($disLikeC, array('controller'=>'LaunchDeals','action'=>'dislikeList',base64_encode($fundDeatils->id)), array('escape'=>false,'title'=>'View','style'=>'color: rgb(3, 66, 110);'));
            ...
            &nbsp;
            <?php 
              if($this->Custom->isLaunchDealDislikedbyUser($fundDeatils->id,$userId)){
                  ...
                  <div class="panel-body">
                    <div class="image-Holder">
                      <?php if(!empty($fundDeatils->image)){?>
                        <img src="<?php echo $this->request->webroot.'img/launchdeal/'.$fundDeatils->image;?>" alt="Fund Image">
                      <?php } ?>
                    </div>
                    </div>
                </div>
                   ...
                   <ul class="listing anchor">
                    <?php 
                        if(!empty($fundDeatils->document)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/launchdeal/'.$fundDeatils->document.'" target="_blank"> Document <i class="fa fa-eye"></i></a>';
                   ...
                   <ul class="listing anchor">
                     <?php 
                        if(!empty($fundDeatils->audio)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/launchdeal/'.$fundDeatils->audio.'" target="_blank"> Audio <i class="fa fa-eye"></i></a>';
                   ...
                   <ul class="listing anchor">
                      <?php 
                        if(!empty($fundDeatils->video)){
                          ...
                          echo '<li><a href="'.$this->request->webroot.'img/launchdeal/'.$fundDeatils->video.'" target="_blank"> Video <i class="fa fa-eye"></i></a>';
            ...
            if($userId != $fundDeatils->user_id){
              ...
              if(!empty($is_follwed_by_user)){
                  ...
                  $titleFollow='Unfollow';
                    $followLink= $this->Url->build(["controller" => "LaunchDeals","action" => "unfollow",base64_encode($fundDeatils->id),base64_encode($userId)]);                      
              }else {
                $class1= 'blueBtn';
                $titleFollow='Follow';
                    $followLink= $this->Url->build(["controller" => "LaunchDeals","action" => "follow",base64_encode($fundDeatils->id),base64_encode($userId)]);   
              }
              ...
              if(!empty($is_commited_by_user)){
                  ...
                  $titleCommit='Thank you. You have registered your interest.';
                    $CommitLink= $this->Url->build(["controller" => "LaunchDeals","action" => "uncommit",base64_encode($fundDeatils->id),base64_encode($userId)]);                      
              }else {
                $class= 'blueBtn';
                $titleCommit='Click here to register your interest.';
                    $CommitLink= $this->Url->build(["controller" => "LaunchDeals","action" => "commit",base64_encode($fundDeatils->id),base64_encode($userId)]);   
              }
